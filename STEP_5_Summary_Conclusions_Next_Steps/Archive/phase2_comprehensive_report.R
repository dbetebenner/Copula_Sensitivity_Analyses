...
phase1_results <- fread(
  "STEP_1_Family_Selection/results/phase1_copula_family_comparison.csv"
)
phase1_selection <- fread(
  "STEP_1_Family_Selection/results/phase1_selection_table.csv"
)
...
report$phase1 <- list(
  ...
  winner_stats = phase1_results[
    family == phase2_families[1],
    .(
      mean_aic = mean(aic),
      mean_tau = mean(tau),
...
if (
  file.exists(
    "STEP_4_Deep_Dive_Reporting/results/phase2_t_copula_deep_dive.RData"
  )
) {
...
by_span <- phase1_results[,
  .SD[which.min(aic)],
  by = .(condition_id, grade_span)
][,
  .N,
  by = .(grade_span, family)
]
...
params_by_span <- phase1_results[
  family == report$executive$phase1_winner,
  .(
...
cat(
  "PARAMETERS BY GRADE SPAN (",
  report$executive$phase1_winner,
  ")\n",
  sep = ""
)
cat("---------------------------------------\n")
...
if (!is.null(report$phase2$t_copula)) {
  ...
  cat(
    "  Correlation (rho):",
    round(report$phase2$t_copula$true_parameters$rho, 4),
    "\n"
  )
  cat(
  ...
  cat(
    "  Kendall's tau:",
    round(report$phase2$t_copula$true_parameters$tau, 4),
    "\n"
  )
  cat(
    "  Tail dependence:",
    round(report$phase2$t_copula$true_parameters$tail_dep, 4),
    "\n\n"
  )
  ...
  stability_table <- data.table(
    ...
    tail_dep_mean = sapply(
      report$phase2$t_copula$tail_dep_results,
      function(x) {
        round(x$mean, 4)
      }
    ),
  ...
  if ("4000" %in% sample_sizes) {
    ...
    cat(
      "  Expected tau precision (SD):",
      round(sd(result_4k$tau_samples), 4),
      "\n"
    )
...
if (length(report$phase2) > 1) {
  # More than just t_copula
...
if (report$executive$phase1_winner == "t") {
  ...
  cat(
    "1. Use",
    report$executive$phase1_winner,
    "copula based on Phase 1 results.\n\n"
  )
}
...
if (
  !is.null(report$phase2$t_copula) &&
    "1000" %in% names(report$phase2$t_copula$df_results)
) {
  ...
  cat(
    "   (expected SD ≈",
    round(report$phase2$t_copula$tail_dep_results[["2000"]]$sd, 4),
    ")\n\n"
  )
...
cat(
  "  - Develop copula-based SGP methodology for international assessments\n\n"
)
...
fwrite(
  table2,
  "STEP_4_Deep_Dive_Reporting/results/TABLE2_parameters_by_span.csv"
)
...
if (!is.null(report$phase2$t_copula)) {
  ...
  fwrite(
    table3,
    "STEP_4_Deep_Dive_Reporting/results/TABLE3_parameter_stability.csv"
  )
...
if (!dir.exists("STEP_4_Deep_Dive_Reporting/results/FINAL_FIGURES")) {
  dir.create(
    "STEP_4_Deep_Dive_Reporting/results/FINAL_FIGURES",
    recursive = TRUE
  )
}
...
if (length(figure_files) > 0) {
  ...
  cat(
    "  Copied",
    length(figure_files),
    "figures to STEP_4_Deep_Dive_Reporting/results/FINAL_FIGURES/\n"
  )
}
