...
if (file.exists("dataset_configs_local.R")) {
  ...
  cat(
    "Note: dataset_configs_local.R not found. Using placeholder state identifiers.\n"
  )
...
# ============================================================================
if (!exists("DATASETS_TO_RUN")) {
  DATASETS_TO_RUN <- "dataset_2"
}
...
if (PERFORMANCE_MODE == "fast") {
  ...
  if (!exists("UNCERTAINTY_GRID_SIZE")) {
    UNCERTAINTY_GRID_SIZE <- 100
  }
  if (!exists("SKIP_COMONOTONIC")) {
    SKIP_COMONOTONIC <- TRUE
  }
...
if (!exists("N_BOOTSTRAP_GOF")) {
  N_BOOTSTRAP_GOF <- 100
} # Default: 100 bootstraps for testing
...
if (!exists("GENERATE_UNCERTAINTY_PLOTS")) {
  GENERATE_UNCERTAINTY_PLOTS <- TRUE
}
...
if (!exists("N_BOOTSTRAP_UNCERTAINTY")) {
  N_BOOTSTRAP_UNCERTAINTY <- 50
}
...
if (!exists("BOOTSTRAP_ALL_FAMILIES")) {
...
if (!exists("UNCERTAINTY_GRID_SIZE")) {
  UNCERTAINTY_GRID_SIZE <- GRID_SIZE
}
...
if (GENERATE_UNCERTAINTY_PLOTS && GENERATE_CONTOUR_PLOTS) {
  ...
  cat(
    "  Families:",
    ifelse(BOOTSTRAP_ALL_FAMILIES, "ALL (5 parametric)", "BEST ONLY"),
    "\n"
  )
  cat("  Note: Bootstrap runs sequentially within each parallel worker\n")
  n_families <- ifelse(BOOTSTRAP_ALL_FAMILIES, 5, 1)
  est_time_min <- (N_BOOTSTRAP_UNCERTAINTY *
    n_families *
    (UNCERTAINTY_GRID_SIZE / 300)^2 *
    0.5) /
    60
  est_time_max <- (N_BOOTSTRAP_UNCERTAINTY *
    n_families *
    (UNCERTAINTY_GRID_SIZE / 300)^2 *
    0.8) /
    60
  cat(
    "  Est. additional time: ~",
    round(est_time_min, 1),
    "-",
    round(est_time_max, 1),
    " min per condition\n\n"
  )
}
...
if (!exists("USE_EXHAUSTIVE_ALL_DATASETS")) {
...
if (!exists("TEST_N_CONDITIONS_PER_DATASET")) {
  TEST_N_CONDITIONS_PER_DATASET <- 2
}
if (!exists("QUICK_TEST_MODE")) {
  QUICK_TEST_MODE <- FALSE
}
if (!exists("QUICK_TEST_N_CONDITIONS")) {
  QUICK_TEST_N_CONDITIONS <- 1
}
if (!exists("QUICK_TEST_PREFER_SMALLEST")) {
  QUICK_TEST_PREFER_SMALLEST <- TRUE
}
...
if (QUICK_TEST_MODE) {
  ...
  cat(
    "  Expected conditions: 966 total (dataset_1=510, dataset_2=194, dataset_3=80, dataset_4=182)\n"
  )
  cat(
    "  Estimated runtime on EC2 (parallel): ~3-4 hours total with 180+ workers\n"
  )
  if (TEST_MODE) {
    cat("\n")
    ...
    cat(
      "  Will analyze only",
      TEST_N_CONDITIONS_PER_DATASET,
      "condition(s) per dataset\n"
    )
  ...
  cat(
    "Strategic Subset Mode: Using representative sampling (~16 conditions/dataset)\n"
  )
  if (TEST_MODE) {
    cat(
      "  TEST MODE: Will analyze only",
      TEST_N_CONDITIONS_PER_DATASET,
      "condition(s) per dataset\n"
    )
  }
...
if (!exists("BATCH_MODE")) {
  BATCH_MODE <- FALSE
}
if (!exists("EC2_MODE")) {
  EC2_MODE <- FALSE
}
if (!exists("SKIP_COMPLETED")) {
  SKIP_COMPLETED <- TRUE
}
...
if (!exists("SKIP_COMPLETED_CONDITIONS")) {
  SKIP_COMPLETED_CONDITIONS <- TRUE
}
...
if (IS_EC2) {
  ...
  instance_type <- tryCatch(
    {
      system(
        "ec2-metadata --instance-type 2>/dev/null | cut -d ' ' -f 2",
        intern = TRUE
      )
    },
  ...
  if (is.na(n_cores_available)) {
    n_cores_available <- 1
  } # Fallback for systems where detectCores fails
...
if (!exists("EXPERIMENT_TO_RUN_STEP2")) {
...
#
if (!exists("STEP2_MAX_CONDITIONS")) {
  STEP2_MAX_CONDITIONS <- NULL
}
if (!exists("STEP2_SAMPLE_STRATEGY")) {
  STEP2_SAMPLE_STRATEGY <- "stratified"
}
if (!exists("STEP2_SEED")) {
  STEP2_SEED <- 42
}
if (!exists("STEP2_MEMORY_PER_WORKER_GB")) {
  STEP2_MEMORY_PER_WORKER_GB <- NULL
}
if (!exists("STEP2_TOTAL_MEMORY_GB")) {
  STEP2_TOTAL_MEMORY_GB <- NULL
}
...
if (exists("EC2_MODE") && isTRUE(EC2_MODE)) {
  if (is.null(STEP2_MEMORY_PER_WORKER_GB)) {
...
if (!is.null(STEP2_MAX_CONDITIONS)) {
  cat(
    "  Subset mode:",
    STEP2_MAX_CONDITIONS,
    "conditions per dataset (",
    STEP2_SAMPLE_STRATEGY,
    ")\n"
  )
} else {
...
if (!dir.exists(FUNCTIONS_DIR)) {
  stop(
    "ERROR: functions/ directory not found. Are you running from the project root?"
  )
}
...
if (should_run_step(1)) {
  ...
  cat(
    "Paper Section: Background → TAMP and Copulas; Methodology → Copula Selection\n"
  )
  cat(
    "Objective: Identify which copula family best fits longitudinal education data\n"
  )
  ...
  if (
    SKIP_COMPLETED &&
      check_results_exist(
        phase1_results_file,
        "Step 1 family comparison (all datasets)"
      )
  ) {
    ...
    result_1_1 <- time_phase("Step 1.1: Family Selection (All Datasets)", {
      ...
      if (USE_PARALLEL) {
        ...
        source_with_path(
          ...
          "Step 1.1: Family Selection (Parallel)"
        )
      } else {
...
for (dataset_idx in seq_along(datasets_to_analyze)) {
  ...
  cat(
    "  Results suffix:",
    ifelse(RESULTS_SUFFIX == "", "(none)", RESULTS_SUFFIX),
    "\n\n"
  )
  ...
  if (should_run_step(3)) {
    ...
    cat(
      "Objective: Validate copula-kernel growth regime inference against longitudinal truth\n"
    )
    ...
    if (SKIP_COMPLETED && file.exists(step3_summary)) {
      ...
      if (!result_3$success) {
        ...
        cat(
          "Recommend investigating before proceeding to TIMSS application.\n\n"
        )
    ...
    if (file.exists(step3_summary)) {
      ...
      cat(
        "  Subgroup:",
        s3_summary$subgroup_id[1],
        "(n =",
        s3_summary$n_subgroup[1],
        ")\n"
      )
    ...
    if (file.exists(phase_b_file)) {
      ...
      cat(
        "  Median |error|:",
        round(median(abs(s3_sys$median_diff)), 2),
        "SGP points\n"
      )
      cat(
        "  Mean |error|:  ",
        round(mean(abs(s3_sys$median_diff)), 2),
        "SGP points\n\n"
      )
    }
    ...
    pause_for_review(
      paste0(
        "Review Step 3 results (Growth Regime Inference):\n",
  ...
  if (should_run_step(2)) {
    if (!exists("SKIP_COMPLETED_STEP2")) {
    ...
    cat(
      "Paper Section: Application → SGPc Sensitivity (Sklar-Theoretic Extension)\n"
    )
    cat("Objective: Quantify practical impact of copula choice on SGPcs\n")
    cat(
      "Mode:",
      if (USE_PARALLEL_STEP2) "PARALLEL (mirai)" else "SEQUENTIAL",
      "\n"
    )
    cat(
      "Estimated time:",
      if (USE_PARALLEL_STEP2) "60-120 minutes" else "3-6 hours",
      "\n\n"
    )

    cat(
      "NOTE: Parameter stability (grade span, sample size, content area, cohort)\n"
    )
    cat("      already analyzed in Phase 1 across 966 conditions.\n")
    cat(
      "      Old experiments archived in: STEP_2_Copula_Sensitivity_Analyses/deprecated/\n\n"
    )
    ...
    if (SKIP_COMPLETED_STEP2 && variants_complete) {
      ...
      result_2_1 <- time_phase("Step 2.1: Compute SGPc Variants", {
        ...
        assign(
          "STEP2_MEMORY_PER_WORKER_GB",
          STEP2_MEMORY_PER_WORKER_GB,
          envir = .GlobalEnv
        )
        assign(
          "STEP2_TOTAL_MEMORY_GB",
          STEP2_TOTAL_MEMORY_GB,
          envir = .GlobalEnv
        )
    ...
    canonical_val_complete <- file.exists(
      "STEP_2_SGPc_Sensitivity/results/canonical_validation_report.md"
    )
  ...
  if (should_run_step(4)) {
    ...
    cat(
      "Objective: Deploy copula-kernel growth regime inference on TIMSS data\n"
    )
  ...
  if (should_run_step(5)) {
    ...
    if (file.exists(step5_runner)) {
      ...
      cat(
        "See STEP_5_Summary_Conclusions_Next_Steps/README.md for planned content.\n\n"
      )
    }
  } else {
    cat(
    ...
    if (length(sgpc_files) > 0) {
      ...
      if (length(sgpc_list) > 0) {
        ...
        if (length(sgpc_cols) > 0) {
          ...
          all_sgpc[,
            merge_key := paste(ID, YEAR, GRADE, CONTENT_AREA, sep = "_")
          ]
          state_data[,
            merge_key := paste(ID, YEAR, GRADE, CONTENT_AREA, sep = "_")
          ]
          ...
          sgpc_to_merge <- all_sgpc[,
            c(
              list(merge_key = merge_key[1]),
          ...
          for (col in sgpc_cols) {
            ...
            if (length(valid_vals) > 0) {
              cat(sprintf(
                "  %-20s: n=%s, mean=%.1f, sd=%.1f\n",
                col,
                format(length(valid_vals), big.mark = ","),
                mean(valid_vals),
                sd(valid_vals)
              ))
            }
          ...
          if ("SGP" %in% names(state_data)) {
            ...
            for (col in sgpc_cols) {
              ...
              if (sum(valid_idx) > 100) {
                corr <- cor(
                  state_data[[col]][valid_idx],
                  state_data$SGP[valid_idx]
                )
                cat(sprintf(
                  "  %-20s: r=%.4f (n=%s)\n",
                  col,
...
if (should_run_step(2)) {
  ...
  aggregate_complete <- file.exists(
    "STEP_2_SGPc_Sensitivity/results/sgpc_key_comparisons.csv"
  )
  ...
  vis_complete <- file.exists(
    "STEP_2_SGPc_Sensitivity/results/visualizations/scatter_emp_vs_best.pdf"
  )
  ...
  pub_fig_complete <- file.exists(
    "STEP_2_SGPc_Sensitivity/results/visualizations/sgpc_summary_grid.pdf"
  )

  if (SKIP_COMPLETED_STEP2 && pub_fig_complete) {
    cat("✓ Skipping Step 2.5 (publication figure already created)\n\n")
  } else if (
    file.exists("STEP_2_SGPc_Sensitivity/create_publication_figure.R")
  ) {
  ...
  cat(
    "  - Publication figure: visualizations/sgpc_summary_grid.{pdf,svg,png}\n\n"
  )

  pause_for_review(
    paste0(
      "Review Step 2 SGPc sensitivity results:\n",
      "  - STEP_2_SGPc_Sensitivity/results/SGPC_SENSITIVITY_REPORT.md\n",
...
if (should_run_step(1)) {
  ...
  if (file.exists(output_file)) {
    ...
    cat(
      ...
      length(ALL_DATASET_RESULTS$step1),
      "datasets...\n"
    )
  ...
  if (!is.null(step1_combined) && nrow(step1_combined) > 0) {
    ...
    n_unique_conditions <- uniqueN(step1_combined[, paste(
      dataset_id,
      condition_id,
      sep = "_"
    )])
    ...
    cat(
      "  Total unique conditions (across all datasets):",
      n_unique_conditions,
      "\n"
    )
    ...
    if (
      SKIP_COMPLETED &&
        check_results_exist(phase1_decision_file, "Step 1 decision")
    ) {
    ...
    if (file.exists(phase1_decision_file)) {
      ...
      cat(
        "Selected families for Step 2+:",
        paste(phase2_families, collapse = ", "),
        "\n\n"
      )
      cat("RATIONALE:\n", rationale, "\n\n")
      ...
      if (
        file.exists(
          "STEP_1_Family_Selection/results/dataset_all/phase1_selection_table.csv"
        )
      ) {
        selection_table <- fread(
          "STEP_1_Family_Selection/results/dataset_all/phase1_selection_table.csv"
        )
      ...
      pause_for_review(
        paste0(
          "Review Step 1 combined results:\n",
...
if (should_run_step(2) && length(ALL_DATASET_RESULTS$step2) > 0) {
  cat(
    "Combining STEP 2 results from",
    length(ALL_DATASET_RESULTS$step2),
    "datasets...\n"
  )
...
if (should_run_step(3) && length(ALL_DATASET_RESULTS$step3) > 0) {
  cat(
    "Combining STEP 3 results from",
    length(ALL_DATASET_RESULTS$step3),
    "datasets...\n"
  )
...
if (should_run_step(4) && length(ALL_DATASET_RESULTS$step4) > 0) {
  cat(
    "Combining STEP 4 results from",
    length(ALL_DATASET_RESULTS$step4),
    "datasets...\n"
  )
...
cat(
  "  Steps run:",
  ifelse(is.null(STEPS_TO_RUN), "ALL", paste(STEPS_TO_RUN, collapse = ", ")),
  "\n"
)
cat("  Batch mode:", BATCH_MODE, "\n")
cat("  Log file:", LOG_FILE, "\n\n")
...
cat("-----------------\n")
if (should_run_step(1)) {
  cat("  Step 1: STEP_1_Family_Selection/results/\n")
}
if (should_run_step(2)) {
  cat("  Step 2: STEP_2_Copula_Sensitivity_Analyses/results/ (CORE)\n")
}
if (should_run_step(3)) {
  cat("  Step 3: STEP_3_LIwLD/results/\n")
}
if (should_run_step(4)) {
  cat("  Step 4: STEP_4_Deep_Dive_Reporting/results/\n")
}
cat("\n")
