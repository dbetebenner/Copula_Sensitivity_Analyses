...
create_bootstrap_ecdf <- function(
  data,
  n_sample,
  n_bootstrap = 100,
  knots,
  ...
  cat(
    "    Sample SDs   - Range: [",
    round(min(sample_sds), 2),
    ",",
    round(max(sample_sds), 2),
    "], SD:",
    round(sd(sample_sds), 2),
    "\n"
  )
...
plot_ecdf_bootstrap <- function(
  ...
  grid.text(
    "Full Data ECDF",
    x = legend_x + unit(2.5, "lines"),
    y = legend_y - unit(0, "lines"),
    just = "left",
    gp = gpar(cex = 0.7)
  )
  grid.lines(
    x = unit.c(legend_x, legend_x + unit(2, "lines")),
...
for (i in seq_along(sample_sizes)) {
  ...
  mae_25 <- mean(
    abs(boot_matrix[idx_25, ] - y_full_smooth[idx_25]),
    na.rm = TRUE
  )
  mae_50 <- mean(
    abs(boot_matrix[idx_50, ] - y_full_smooth[idx_50]),
    na.rm = TRUE
  )
  mae_75 <- mean(
    abs(boot_matrix[idx_75, ] - y_full_smooth[idx_75]),
    na.rm = TRUE
  )
...
cat(
  "- mean_band_width: Average width of 90% confidence bands across all scale scores\n"
)
