...
cat(
  "This should take ~",
  round(elapsed_seq / (n_cores_bootstrap * 0.8) / 60, 1),
  " minutes (",
  round(n_cores_bootstrap * 0.8, 1),
  "x speedup)\n\n"
)
...
if (exists("perform_gof_test_parallel")) {
  ...
  cat(
    ...
    round(elapsed_par, 1),
    "seconds (",
    round(elapsed_par / 60, 2),
    "minutes)\n"
  )
  cat("  T-copula p-value:", round(fit_par$results$t$gof_pvalue, 4), "\n")
  cat(
    "  T-copula statistic:",
    round(fit_par$results$t$gof_statistic, 4),
    "\n\n"
  )
...
if (exists("perform_gof_test_parallel") && !is.na(elapsed_par)) {
  ...
  if (elapsed_par < elapsed_seq * 0.7) {
    ...
    cat(
      "  Time:",
      round(elapsed_full, 1),
      "seconds (",
      round(elapsed_full / 60, 2),
      "minutes)\n\n"
    )
    ...
    cat(
      "  Estimated sequential time for 6 families: ",
      round(elapsed_seq * 6 / 60, 1),
      " min\n",
      sep = ""
    )
    cat(
      "  Actual parallel time for 6 families:      ",
      round(elapsed_full / 60, 1),
      " min\n",
      sep = ""
    )
    cat(
      "  Overall speedup:         ",
      round((elapsed_seq * 6) / elapsed_full, 2),
      "x\n\n",
      sep = ""
    )
    ...
    cat(
      "  Time per condition:      ",
      round(ec2_time_per_condition, 2),
      " min\n",
      sep = ""
    )
    cat(
      "  Total time (6×7 nested): ",
      round(ec2_total_time, 1),
      " min\n",
      sep = ""
    )
