...
for (i in seq_along(sample_sizes)) {
  ...
  pseudo_obs <- pobs(
    cbind(pairs_data$SCALE_SCORE_PRIOR, pairs_data$SCALE_SCORE_CURRENT),
    ties.method = "random"
  )
  ...
  for (b in 1:N_BOOTSTRAP) {
    if (b %% 10 == 1) {
      cat("  Bootstrap", b, "-", min(b + 9, N_BOOTSTRAP), "...\n")
    }
    ...
    boot_cop_fixed <- tCopula(
      param = boot_fit@estimate[1],
      dim = 2,
      df = round(boot_fit@estimate[2]),
      df.fixed = TRUE
    )
  ...
  cat(
    "Bootstrap range:   ",
    sprintf("%10.6f", min(bootstrap_stats)),
    "-",
  ...
  cat(
    "Obs/Boot mean:     ",
    sprintf("%.2f", observed_stat / mean(bootstrap_stats)),
    "x\n"
  )
  cat(
    "Obs/Boot max:      ",
    sprintf("%.2f", observed_stat / max(bootstrap_stats)),
    "x\n"
  )
}
...
plot(
  NULL,
  xlim = c(0.5, length(sample_sizes) + 0.5),
