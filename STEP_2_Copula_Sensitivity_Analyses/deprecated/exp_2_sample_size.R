...
# Generate test configurations dynamically based on actual data availability
years_available <- unique(STATE_DATA_LONG$YEAR)
years_numeric <- sort(as.numeric(as.character(years_available)))

cat("Generating dynamic TEST_CONFIGS...\n")
cat("  Available years:", paste(range(years_numeric), collapse = "-"), "\n")

TEST_CONFIGS <- list()

# Config 1: 1-year span (G4->G5)
valid_1yr <- years_numeric[(years_numeric + 1) %in% years_numeric]
if (length(valid_1yr) > 0) {
  year_1yr <- max(valid_1yr)
  n_check <- STATE_DATA_LONG[
    GRADE == 4 & YEAR == as.character(year_1yr) & CONTENT_AREA == "MATHEMATICS",
    .N
  ]
  if (n_check >= 100) {
    TEST_CONFIGS[[length(TEST_CONFIGS) + 1]] <- list(
      ...
      grade_prior = 4,
      grade_current = 5,
      year_prior = as.character(year_1yr),
      content = "MATHEMATICS"
    )
  }
}

# Config 2: 4-year span (G4->G8)
valid_4yr <- years_numeric[(years_numeric + 4) %in% years_numeric]
if (length(valid_4yr) > 0) {
  year_4yr <- max(valid_4yr)
  n_check <- STATE_DATA_LONG[
    GRADE == 4 & YEAR == as.character(year_4yr) & CONTENT_AREA == "MATHEMATICS",
    .N
  ]
  if (n_check >= 100) {
    TEST_CONFIGS[[length(TEST_CONFIGS) + 1]] <- list(
      ...
      grade_prior = 4,
      grade_current = 8,
      year_prior = as.character(year_4yr),
      content = "MATHEMATICS"
    )
  }
}

cat("✓ Generated", length(TEST_CONFIGS), "valid test configurations\n")
if (length(TEST_CONFIGS) == 0) {
  stop("No valid test configurations found in dataset")
}
cat("\n")
...
for (config in TEST_CONFIGS) {
  ...
  output_dir <- file.path("STEP_2_Copula_Sensitivity_Analyses/results", "exp_2_sample_size", config$name)
  ...
  extra_copulas <- list()
  canonical_copula <- get_canonical_copula(CANONICAL_PARAMS, year_span, config$content)
  if (!is.null(canonical_copula)) {
    extra_copulas[[paste0("canonical_", tolower(config$content), "_span", year_span)]] <- canonical_copula
  }
...
if (length(sgpc_sensitivity_all) > 0) {
  ...
  sgpc_output_dir <- "STEP_2_Copula_Sensitivity_Analyses/results/exp_2_sample_size"
