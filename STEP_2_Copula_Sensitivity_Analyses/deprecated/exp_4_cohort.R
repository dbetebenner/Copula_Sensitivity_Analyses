...
for (config in COHORT_CONFIGS) {
  ...
  # NOTE: Configuration already has valid years from dynamic generation above
  # (lines 60-99), so we don't need to call resolve_year_prior() anymore.
  ...
  output_dir <- file.path("STEP_2_Copula_Sensitivity_Analyses/results", "exp_4_cohort", config$name)
