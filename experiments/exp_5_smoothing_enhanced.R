...
SMOOTHING_METHODS <- list(
  ...
  list(
    name = "ispline_pit_optimal",
    label = "I-spline (PIT-optimized)",
    type = "ispline_pit"
  ),
  list(
    name = "ispline_tail_aware",
    label = "I-spline (Tail-aware)",
    type = "ispline_tail"
  ),
  list(
    name = "no_smoothing",
    label = "No Smoothing (Mid-Ranks)",
    type = "ranks"
  ),
...
for (method in SMOOTHING_METHODS) {
  ...
  if (method$type == "ispline") {
    ...
    framework_current <- list(
      smooth_ecdf_full = smooth_current,
      method = "hyman"
    )
  } else if (method$type == "kernel") {
    # Kernel smoothing
    bw_prior <- 1.06 *
      sd(pairs_full$SCALE_SCORE_PRIOR) *
      length(pairs_full$SCALE_SCORE_PRIOR)^(-1 / 5)
    bw_current <- 1.06 *
      sd(pairs_full$SCALE_SCORE_CURRENT) *
    ...
    smooth_current <- function(x) {
      pnorm(x, mean = mu_current, sd = sigma_current)
    }

    framework_prior <- list(
      smooth_ecdf_full = smooth_prior,
      method = "parametric"
    )
    framework_current <- list(
      smooth_ecdf_full = smooth_current,
...
for (method in SMOOTHING_METHODS) {
  ...
  cat(
    "  PIT Diagnostics (Prior)  - CvM:",
    round(pit_diag_prior$cvm, 6),
    ", KS p-value:",
    round(pit_diag_prior$ks_pval, 4),
    "\n"
  )
  cat(
    "  PIT Diagnostics (Current) - CvM:",
    round(pit_diag_current$cvm, 6),
    ", KS p-value:",
    round(pit_diag_current$ks_pval, 4),
    "\n"
  )
