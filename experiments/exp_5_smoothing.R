...
x_grid <- seq(
  min(c(pairs_full$SCALE_SCORE_PRIOR, pairs_full$SCALE_SCORE_CURRENT)),
...
for (method_config in SMOOTHING_METHODS) {
  ...
  if (method_config$type == "ispline") {
    ...
    cat(
      "Normal parameters - Prior: μ =",
      round(fit_prior$parameters["mu"], 2),
      ", σ =",
      round(fit_prior$parameters["sigma"], 2),
      "\n"
    )
    cat(
      "Normal parameters - Current: μ =",
      round(fit_current$parameters["mu"], 2),
      ", σ =",
      round(fit_current$parameters["sigma"], 2),
      "\n\n"
    )
...
for (method_name in names(all_results)) {
  ...
  comparison_data[[length(comparison_data) + 1]] <- data.table(
    ...
    gaussian_tau = if (!is.null(result$true_copula$results$gaussian)) {
      result$true_copula$results$gaussian$kendall_tau
    } else {
      NA
    },
    clayton_tau = if (!is.null(result$true_copula$results$clayton)) {
      result$true_copula$results$clayton$kendall_tau
    } else {
      NA
    },
    gumbel_tau = if (!is.null(result$true_copula$results$gumbel)) {
      result$true_copula$results$gumbel$kendall_tau
    } else {
      NA
    },
    frank_tau = if (!is.null(result$true_copula$results$frank)) {
      result$true_copula$results$frank$kendall_tau
    } else {
      NA
    }
  )
...
barplot(
  comparison_table$empirical_tau,
  ...
  ylim = c(
    min(comparison_table$empirical_tau) * 0.95,
    max(comparison_table$empirical_tau) * 1.05
  )
)
legend(
  "topright",
  legend = comparison_table$method,
  fill = rainbow(nrow(comparison_table)),
  cex = 0.7,
  bg = "white"
)
grid()
...
barplot(
  comparison_table$gaussian_tau,
  names.arg = 1:nrow(comparison_table),
  col = rainbow(nrow(comparison_table)),
  main = expression("Gaussian Copula" ~ tau ~ "by Method"),
  ylab = expression("Kendall's" ~ tau),
  ylim = c(
    min(comparison_table$gaussian_tau, na.rm = TRUE) * 0.95,
...
cat(
  "- Variability in tau across methods:",
  round(tau_sd, 4),
  "(",
  round(tau_cv, 2),
  "% CV)\n"
...
if (tau_cv < 1) {
  ...
  cat(
    "           Choice of smoothing method matters. Further investigation needed.\n"
  )
}
