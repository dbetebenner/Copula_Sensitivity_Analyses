...
create_longitudinal_pairs <- function(
  data,
  grade_prior,
  grade_current,
  year_prior,
  ...
  if ("SCHOOL_NUMBER" %in% names(data)) {
    ...
    stop(
      "CRITICAL ERROR: SCHOOL_NUMBER column not found in data.\n",
  ...
  if ("DISTRICT_NUMBER" %in% names(data)) {
    ...
    stop(
      "CRITICAL ERROR: DISTRICT_NUMBER column not found in data.\n",
      "  This column is REQUIRED for Step 2 group-level analyses.\n",
  ...
  cat(
    "  Time span:",
    as.numeric(year_current) - as.numeric(year_prior),
    "years\n"
  )
...
resolve_year_prior <- function(
  ...
  years_prior <- data[
    GRADE == grade_prior & CONTENT_AREA == content_prior,
    unique(YEAR)
  ]
  years_current <- data[
    GRADE == grade_current & CONTENT_AREA == content_current,
    unique(YEAR)
  ]
  ...
  if (length(years_prior_num) == 0 || length(years_current_num) == 0) {
    warning(sprintf(
      "No valid numeric years found for grade %d -> %d, content %s -> %s",
      grade_prior,
      grade_current,
      content_prior,
      content_current
  ...
  valid_prior <- years_prior_num[
    years_prior_num + grade_span %in% years_current_num
  ]
  if (length(valid_prior) == 0) {
    warning(sprintf(
      "No valid year_prior found where year_prior + %d exists for grade %d -> %d",
      grade_span,
      grade_prior,
      grade_current
    ))
  ...
  if (!is.null(year_prior)) {
    ...
    if (!is.na(year_prior_num) && year_prior_num %in% valid_prior) {
      ...
      warning(sprintf(
        "Requested year_prior %d not valid. Valid years: %s. Using max: %d",
        year_prior_num,
...
create_multiple_pairs <- function(data, configurations) {
  ...
  for (i in seq_along(configurations)) {
    ...
    pairs_list[[i]] <- create_longitudinal_pairs(
      ...
      min_valid_score = if (!is.null(cfg$min_valid_score)) {
        cfg$min_valid_score
      } else {
        200
      }
...
get_available_configurations <- function(
  data,
  min_grade_span = 1,
  max_grade_span = 5,
  min_pairs = 100
  ...
  available <- data[
    !is.na(SCALE_SCORE) & SCALE_SCORE >= 200,
    .(N = .N),
  ...
  for (i in 1:nrow(configs)) {
    prior_exists <- nrow(available[
      GRADE == configs$grade_prior[i] &
        YEAR == configs$year_prior[i] &
        CONTENT_AREA == configs$content_area[i]
    ]) >
      0

    current_exists <- nrow(available[
      GRADE == configs$grade_current[i] &
  ...
  cat(
    "Grade spans:",
    paste(
      sort(unique(configs$grade_current - configs$grade_prior)),
      collapse = ", "
    ),
    "\n"
  )
  cat(
    "Content areas:",
    paste(unique(configs$content_area), collapse = ", "),
    "\n\n"
  )
