...
#'
get_completed_conditions <- function(results_dir = "STEP_1_Family_Selection/results") {
  ...
  for (ds_dir in dataset_dirs) {
    ...
    condition_dirs <- list.dirs(contour_dir, recursive = FALSE, full.names = TRUE)
...
#'
make_condition_key <- function(dataset_id, year_current, grade_prior, grade_current, content_area) {
  paste(dataset_id, year_current, grade_prior, grade_current, content_area, sep = "|")
}
...
get_remaining_conditions <- function(all_conditions, completed_conditions) {
  ...
  if (skipped_count > 0) {
    message("Checkpoint: Skipping ", skipped_count, " already-completed conditions")
  }
...
#'
print_checkpoint_summary <- function(results_dir = "STEP_1_Family_Selection/results",
  ...
  if (nrow(completed) > 0) {
    ...
    for (i in seq_len(nrow(content_summary))) {
      cat(sprintf("  %s: %d\n", content_summary$content_area[i], content_summary$N[i]))
    }
    cat("\n")
  }
...
#'
is_condition_completed <- function(results_dir, dataset_id, year_current, 
                                   grade_prior, grade_current, content_area) {
  
  condition_dir <- file.path(
    results_dir, dataset_id, "contour_plots",
    sprintf("%s_G%d_G%d_%s", year_current, grade_prior, grade_current, content_area)
  )
