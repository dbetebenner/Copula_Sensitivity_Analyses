...
#'
#' @export
sgpc_engine <- function(u, v, copula, 
 ...
 # Input validation
  ...
  if (any(!na_mask)) {
    ...
    cond_cdf_valid <- .dispatch_sgpc(u_valid, v_valid, copula, grid_cache, grid_size)
...
#'
#' @export
create_sgpc_grid <- function(emp_copula, 
                              grid_size = 200,
  ...
  C_matrix <- matrix(C_values, nrow = grid_size, ncol = grid_size, byrow = FALSE)
  ...
  for (j in 1:grid_size) {
    for (i in 2:(grid_size - 1)) {
      conditional_matrix[i, j] <- (C_matrix[i + 1, j] - C_matrix[i - 1, j]) / (2 * du)
    }
    # Boundary points: one-sided differences
    conditional_matrix[1, j] <- (C_matrix[2, j] - C_matrix[1, j]) / du
    conditional_matrix[grid_size, j] <- (C_matrix[grid_size, j] - C_matrix[grid_size - 1, j]) / du
  }
...
sgpc_interpolate <- function(u, v, grid_cache) {
  ...
  if (is.null(grid_cache) || !all(c("u_grid", "v_grid", "conditional_matrix") %in% names(grid_cache))) {
...
sgpc_batch <- function(u, v, 
  ...
  for (family in parametric_families) {
    ...
    if (!is.null(copula_results[[family]]) && !is.null(copula_results[[family]]$copula)) {
  ...
  if (include_comonotonic) {
    result[["SGPc_comonotonic"]] <- sgpc_engine(u, v, "comonotonic", scale = scale)
  }
