...
assert_equal <- function(a, b, tol = 1e-6, msg = "") {
  if (!isTRUE(all.equal(a, b, tolerance = tol))) {
    stop(sprintf(
      "Assertion failed: %s\n  Expected: %s\n  Got: %s",
      msg,
      paste(head(b, 5), collapse = ", "),
      paste(head(a, 5), collapse = ", ")
    ))
  }
}
...
assert_in_range <- function(x, lower, upper, msg = "") {
  ...
  if (length(x_valid) > 0 && (any(x_valid < lower) || any(x_valid > upper))) {
    stop(sprintf(
      "Assertion failed: %s values outside [%g, %g]",
      msg,
      lower,
      upper
    ))
...
run_test("Parametric Gaussian copula", {
  ...
  assert_equal(
    ...
    expected,
    tol = 1e-6,
    msg = "Gaussian cCopula alignment"
  )
})
...
run_test("Comonotonic copula", {
  ...
  assert_equal(
    ...
    u,
    tol = 1e-10,
    msg = "Comonotonic = prior percentile"
  )
...
run_test("Empirical copula grid creation", {
  ...
  assert_true(
    "conditional_matrix" %in% names(grid),
    "Grid has conditional_matrix"
  )
...
run_test("Grid interpolation accuracy", {
  ...
  assert_true(
    max_diff < 0.01,
    sprintf("Close at grid points (max diff: %.4f)", max_diff)
  )
})
...
run_test("Validate pseudo-observations", {
  # Valid
  assert_true(
    validate_pseudo_obs(c(0.3, 0.5, 0.7), c(0.4, 0.6, 0.8)),
    "Valid obs"
  )
  ...
  suppressWarnings({
    assert_true(
      !validate_pseudo_obs(c(0.3, 0.5), c(0.4, 0.6, 0.8)),
      "Different lengths"
    )
  })
  ...
  suppressWarnings({
    assert_true(
      !validate_pseudo_obs(c(0.3, 1.5, 0.7), c(0.4, 0.6, 0.8)),
      "Out of range"
    )
  })
...
run_test("Performance benchmark (100k obs)", {
  ...
  assert_true(
    t_param < 5,
    sprintf("Parametric < 5 seconds (got %.2f)", t_param)
  )
