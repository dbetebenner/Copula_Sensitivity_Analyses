...
bootstrap_ispline_ecdfs <- function(
  data,
  n_sample,
  n_bootstrap,
  framework,
...
select_smoothing_by_pit <- function(
  scale_scores,
  knot_configs = list(
    c(0.5),
    c(0.33, 0.67),
  ...
  cvm_uniform <- function(u) {
    ...
    cvm <- (1 / (12 * length(u))) +
      sum((u_sorted - (2 * i - 1) / (2 * length(u)))^2)
    return(cvm)
  }
  ...
  ad_uniform <- function(u) {
    ...
    i <- 1:n_u
    ad <- -n_u -
      sum((2 * i - 1) * (log(u_sorted) + log(1 - rev(u_sorted)))) / n_u
    return(ad)
  }
  ...
  for (i in seq_along(knot_configs)) {
    ...
    if (use_tail_aware) {
      ...
      knots <- unique(sort(c(
        tail_lower[tail_lower < min(knots)],
        knots,
    ...
    framework <- tryCatch(
      {
        create_ispline_framework(scale_scores, knot_percentiles = knots)
      },
      error = function(e) {
        return(NULL)
      }
...
create_ispline_framework_enhanced <- function(
  scale_scores,
...
print_ispline_framework <- function(framework, label = NULL) {
  ...
  cat(
    "Knot locations:",
    paste(round(framework$knot_locations, 2), collapse = ", "),
    "\n"
  )
