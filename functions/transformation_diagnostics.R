...
compute_tail_diagnostics <- function(U, V, empirical_baseline = NULL) {
  ...
  if (!is.null(empirical_baseline)) {
    ...
    result$preserves_tail_lower = result$tail_distortion_lower <
      (0.20 * empirical_baseline$lower_10)
    result$preserves_tail_upper = result$tail_distortion_upper <
      (0.20 * empirical_baseline$upper_90)
  }
...
compute_utility_diagnostics <- function(framework, n_test = 1000) {
  ...
  return(list(
    ...
    n_knots = ifelse(
      !is.null(framework$n_knots),
      framework$n_knots,
      ifelse(
        !is.null(framework$knot_locations),
        length(framework$knot_locations),
        NA
      )
    )
  ))
...
classify_transformation_method <- function(
  uniformity,
  dependence,
  tail,
  copula_results,
  ...
  tier1_pass <- tier1_copula_correct &&
    tier1_tau_preserved &&
    tier1_tail_preserved
  ...
  tier2_pass <- tier2_ks_liberal &&
    tier2_cvm_acceptable &&
    tier2_no_excessive_ties
...
generate_method_report <- function(
  method_name,
  all_diagnostics,
  output_file = NULL
) {
  report <- c(
    ...
    paste(
      ...
      all_diagnostics$classification$use_in_phase2
    ),
    "",
    "TIER 1 (CRITICAL):",
    paste(
      "  Selects correct copula:",
      all_diagnostics$classification$tier1_copula_correct
    ),
    paste(
      "  Preserves tau:",
      all_diagnostics$classification$tier1_tau_preserved
    ),
    paste(
      "  Preserves tails:",
      all_diagnostics$classification$tier1_tail_preserved
    ),
    "",
    "TIER 2 (IMPORTANT):",
    paste(
      "  Passes K-S (liberal):",
      all_diagnostics$classification$tier2_ks_liberal
    ),
    paste(
      "  CvM acceptable:",
      all_diagnostics$classification$tier2_cvm_acceptable
    ),
    paste(
      "  No excessive ties:",
      all_diagnostics$classification$tier2_no_excessive_ties
    ),
    "",
    "UNIFORMITY:",
    sprintf(
    ...
    "DEPENDENCE:",
    sprintf(
      "  Kendall tau: %.4f (bias: %.4f)",
      all_diagnostics$dependence$kendall_tau,
      ifelse(
        !is.null(all_diagnostics$dependence$tau_bias),
        all_diagnostics$dependence$tau_bias,
        NA
      )
    ),
    "",
    "TAIL STRUCTURE:",
    sprintf(
      "  Lower 10%%: %.4f (distortion: %.4f)",
      all_diagnostics$tail$lower_10,
      ifelse(
        !is.null(all_diagnostics$tail$tail_distortion_lower),
        all_diagnostics$tail$tail_distortion_lower,
        NA
      )
    ),
    sprintf(
      "  Upper 90%%: %.4f (distortion: %.4f)",
      all_diagnostics$tail$upper_90,
      ifelse(
        !is.null(all_diagnostics$tail$tail_distortion_upper),
        all_diagnostics$tail$tail_distortion_upper,
        NA
      )
    ),
    "",
...
tail_calibration_check <- function(
  U_empirical,
  U_smoothed,
  ...
  upper_tail_emp <- sapply(tail_quantiles, function(q) {
    mean(U_empirical >= (1 - q))
  })
  upper_tail_smooth <- sapply(tail_quantiles, function(q) {
    mean(U_smoothed >= (1 - q))
  })
...
plot_tail_calibration <- function(
  tail_cal,
  method_name,
  ...
  # Lower tail
  plot(
    tail_cal$curves$q_grid,
    tail_cal$curves$lower_emp,
    type = "l",
    lwd = 2,
    col = "black",
    xlab = "Quantile q",
  ...
  text(
    max(tail_cal$curves$q_grid) * 0.7,
    ...
    cex = 0.8,
    col = ifelse(tail_cal$passes_tier1, "darkgreen", "darkred")
  )
...
bootstrap_parameter_stability <- function(
  U_prior,
  U_current,
  copula_family = "t",
  n_bootstrap = 200,
  ...
  true_nu <- if (copula_family == "t" && !is.null(true_fit$df)) {
    true_fit$df
  } else {
    NA
  }
  ...
  bootstrap_once <- function(i) {
    ...
    tryCatch(
      {
        boot_fit <- fit_copula_from_uniform(
          U_prior_boot,
          U_current_boot,
          copula_family
        )
        list(
          ...
          nu = if (copula_family == "t" && !is.null(boot_fit$df)) {
            boot_fit$df
          } else {
            NA
          },
  ...
  nu_iqr <- if (copula_family == "t" && length(boot_nu) > 0) {
    IQR(boot_nu)
  } else {
    NA
  }
  ...
  return(list(
    ...
    nu_iqr = nu_iqr,
    nu_cv = nu_cv,
    nu_ci = if (length(boot_nu) > 0) {
      quantile(boot_nu, probs = c(0.025, 0.975))
    } else {
      c(NA, NA)
    },
