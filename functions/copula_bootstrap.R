...
fit_copula_from_pairs <- function(
  scores_prior,
  scores_current,
  framework_prior,
  framework_current,
  ...
  if (use_empirical_ranks) {
    ...
    if (is.null(framework_prior) || is.null(framework_current)) {
      stop(
        "framework_prior and framework_current required when use_empirical_ranks=FALSE"
      )
    }
    ...
    tryCatch(
      {
        if (family == "gaussian") {
          ...
          results[[family]] <- list(
            ...
            bic = -2 *
              fit@loglik +
              log(nrow(pseudo_obs)) * length(fit@estimate),
          ...
          tail_dep_val <- 2 *
            pt(-sqrt((df + 1) * (1 - rho) / (1 + rho)), df = df + 1)
  ...
  if (!is.null(n_bootstrap_gof)) {
    for (family in names(results)) {
      if (family == "comonotonic") {
        ...
        gof_test <- perform_gof_test(
          fitted_copula = results[[family]]$copula,
          pseudo_obs = pseudo_obs,
          n_bootstrap = n_bootstrap_gof,
          family = family
        )
  ...
  if (save_copula_data) {
    if (is.null(output_dir)) {
      warning(
        "save_copula_data=TRUE but output_dir not specified. Skipping data save."
      )
    } else {
      ...
      saveRDS(
        pseudo_obs,
        file = file.path(output_dir, "pseudo_observations.rds")
      )
      ...
      saveRDS(
        original_scores,
        file = file.path(output_dir, "original_scores.rds")
      )
      ...
      if (!is.null(empirical_copulas)) {
        saveRDS(
          empirical_copulas,
          file = file.path(output_dir, "empirical_copulas.rds")
        )
        cat(sprintf(
          "  Saved %d empCopula objects (raw + bernstein)\n",
      ...
      if (requireNamespace("ks", quietly = TRUE)) {
        ...
        if (exists("calculate_empirical_copula_grid")) {
          ...
          saveRDS(
            empirical_grid,
            file = file.path(output_dir, "empirical_copula_grid.rds")
          )
        }
      }
...
fit_empirical_copulas <- function(pseudo_obs, methods = c("raw", "bernstein")) {
  require(copula)
...
bootstrap_copula_estimation <- function(
  ...
  if (use_parallel) {
    ...
    cat(
      ...
      n_cores,
      "cores via PSOCK socket cluster)\n\n"
    )
  } else {
    ...
    tryCatch(
      {
        if (sampling_method == "paired") {
          ...
          if (with_replacement) {
            sample_ids <- sample(
              1:nrow(pairs_data),
              size = min(n_sample_prior, n_sample_current),
              replace = TRUE
            )
          } else {
            sample_ids <- sample(
          ...
          scores_prior_boot <- sample(
            pairs_data$SCALE_SCORE_PRIOR,
            size = n_sample_prior,
            replace = with_replacement
          )
          scores_current_boot <- sample(
  ...
  if (use_parallel && n_cores > 1) {
    ...
    for (b in 1:n_bootstrap) {
      if (b %% 10 == 0 || b == 1) {
        cat("  Bootstrap iteration", b, "/", n_bootstrap, "\n")
      }
...
summarize_bootstrap_copulas <- function(bootstrap_results, true_copula = NULL) {
  ...
  summary_dt[,
    selection_freq := as.numeric(best_family_counts[family]) /
