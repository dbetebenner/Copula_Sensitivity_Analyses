...
calculate_dependence_measures <- function(copula_fit, pseudo_obs = NULL) {
  ...
  if (!is.null(pseudo_obs)) {
    measures$empirical_kendall <- cor(
      pseudo_obs[, 1],
      pseudo_obs[, 2],
      method = "kendall"
    )
    measures$empirical_spearman <- cor(
      pseudo_obs[, 1],
      pseudo_obs[, 2],
      method = "spearman"
    )
    measures$empirical_pearson <- cor(
      pseudo_obs[, 1],
      pseudo_obs[, 2],
      method = "pearson"
    )
...
#' @param filename PDF filename
plot_parameter_stability <- function(
  results_by_size,
  sample_sizes,
  true_value = NULL,
  family = "gaussian",
  ...
  polygon(
    c(sample_sizes, rev(sample_sizes)),
    c(q05, rev(q95)),
    col = rgb(0, 0, 1, 0.2),
    border = NA
  )
...
create_sensitivity_report <- function(
  bootstrap_results,
  true_copula,
  ...
  # Save summary table
  fwrite(summary_dt, file = paste0(output_prefix, "_summary.csv"))
