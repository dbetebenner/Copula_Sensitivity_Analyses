...
dataset_3_knots_boundaries_ELA_2014 <- SGP::createKnotsBoundaries(Copula_Sensitivity_Data_Set_3[
  CONTENT_AREA == "ELA" & YEAR <= 2014
])
dataset_3_knots_boundaries_MATHEMATICS_2014 <- SGP::createKnotsBoundaries(Copula_Sensitivity_Data_Set_3[
  CONTENT_AREA == "MATHEMATICS" & YEAR <= 2014
])
dataset_3_knots_boundaries_ELA_2015 <- SGP::createKnotsBoundaries(Copula_Sensitivity_Data_Set_3[
  CONTENT_AREA == "ELA" & YEAR >= 2015
])
dataset_3_knots_boundaries_MATHEMATICS_2015 <- SGP::createKnotsBoundaries(Copula_Sensitivity_Data_Set_3[
  CONTENT_AREA == "MATHEMATICS" & YEAR >= 2015
])
dataset_3_knots_boundaries[['ELA']] <- dataset_3_knots_boundaries_ELA_2014[[
  'ELA'
]]
dataset_3_knots_boundaries[[
  'MATHEMATICS'
]] <- dataset_3_knots_boundaries_MATHEMATICS_2014[['MATHEMATICS']]
dataset_3_knots_boundaries[[
  'ELA.2015'
]] <- dataset_3_knots_boundaries_ELA_2015[['ELA']]
dataset_3_knots_boundaries[[
  'MATHEMATICS.2015'
]] <- dataset_3_knots_boundaries_MATHEMATICS_2015[['MATHEMATICS']]

# Embed knots and boundaries into SGPstateData
SGPstateData[["DATASET_3"]][["Achievement"]][[
  "Knots_Boundaries"
]] <- dataset_3_knots_boundaries
...
for (span_config in seq_along(dataset_3_config)) {
  ...
  variables.to.save <- intersect(
    names(dataset_3_SGP_Object@Data),
    c("SGP", "SGP_ORDER_1", "SGP_ORDER_2", "SGP_ORDER", "SCALE_SCORE_PRIOR")
  )
  Copula_Sensitivity_Data_Set_3_SGP[,
    (variables.to.save) := dataset_3_SGP_Object@Data[,
      (variables.to.save),
      with = FALSE
    ]
  ]
  setnames(
    Copula_Sensitivity_Data_Set_3_SGP,
    variables.to.save,
    paste0(variables.to.save, "_SPAN_", span_config, "_YEAR")
  )
...
save(
  Copula_Sensitivity_Data_Set_3_SGP,
  file = "../../Data/Copula_Sensitivity_Data_Set_3_SGP.Rdata"
)
