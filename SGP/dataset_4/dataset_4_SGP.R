...
dataset_4_knots_boundaries <- SGP::createKnotsBoundaries(
  Copula_Sensitivity_Data_Set_4
)

# Embed knots and boundaries into SGPstateData
SGPstateData[["dataset_4"]][["Achievement"]][[
  "Knots_Boundaries"
]] <- dataset_4_knots_boundaries
...
for (span_config in seq_along(dataset_4_config)) {
  ...
  variables.to.save <- intersect(
    names(dataset_4_SGP_Object@Data),
    c("SGP", "SGP_ORDER_1", "SGP_ORDER_2", "SGP_ORDER", "SCALE_SCORE_PRIOR")
  )
  Copula_Sensitivity_Data_Set_4_SGP[,
    (variables.to.save) := dataset_4_SGP_Object@Data[,
      (variables.to.save),
      with = FALSE
    ]
  ]
...
save(
  Copula_Sensitivity_Data_Set_4_SGP,
  file = "../../Data/Copula_Sensitivity_Data_Set_4_SGP.Rdata"
)
