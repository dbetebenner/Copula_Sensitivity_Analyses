...
if (file.exists("functions/transformation_diagnostics.R")) {
  ...
  stop(
    "Cannot find functions directory. Please run from workspace root or STEP_3_Application_Implementation directory."
  )
}
...
test_operational_fitness <- function(
  method_fits,
  test_scores,
  n_calls_forward = 1e5,
  n_calls_inverse = 1e5,
  ...
  for (method_name in names(method_fits)) {
    ...
    test_x <- sample(
      test_scores,
      size = min(n_calls_forward, length(test_scores)),
      replace = TRUE
    )
    ...
    test_sample <- sample(
      test_scores,
      size = min(n_roundtrip, length(test_scores))
    )
    ...
    roundtrip_results <- tryCatch(
      {
        ...
        list(
          ...
          success_rate = sum(!is.na(X_roundtrip) & !is.infinite(X_roundtrip)) /
            length(X_roundtrip)
        )
      },
      error = function(e) {
...
grade_operational_fitness <- function(fitness_results, score_range = 1000) {
  ...
  fitness_results[
    is.na(speed_forward_per_sec) | is.na(speed_inverse_per_sec),
    grade := "FAIL"
  ]
...
create_fitness_report <- function(fitness_graded) {
  ...
  for (i in 1:nrow(fitness_graded)) {
    ...
    report <- c(
      ...
      "  Performance:",
      sprintf(
        "    Forward:  %s calls/sec %s",
        format(
          method$speed_forward_per_sec,
          big.mark = ",",
          scientific = FALSE
        ),
      ...
      sprintf(
        "    Inverse:  %s calls/sec %s",
        format(
          method$speed_inverse_per_sec,
          big.mark = ",",
          scientific = FALSE
        ),
...
if (interactive() || !exists("SKIP_EXP6_EXECUTION")) {
  ...
  test_scores_prior <- test_data$SCALE_SCORE_PRIOR[
    !is.na(test_data$SCALE_SCORE_PRIOR)
  ]
  test_scores_current <- test_data$SCALE_SCORE_CURRENT[
    !is.na(test_data$SCALE_SCORE_CURRENT)
  ]
  ...
  cat(sprintf(
    "Score range: [%.1f, %.1f] (range = %.1f)\n",
    min(test_scores_prior),
    max(test_scores_prior),
    score_range
  ))
  ...
  cat("  Bernstein CDF...\n")
  if (
    file.exists("STEP_3_Application_Implementation/methods/bernstein_cdf.R")
  ) {
  ...
  kernel_quantile <- function(p, data = test_scores_prior, bandwidth = bw) {
    sapply(p, function(pi) {
      uniroot(
        function(x) kernel_cdf(x, data, bandwidth) - pi,
