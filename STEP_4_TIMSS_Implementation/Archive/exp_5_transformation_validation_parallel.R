...
cat(
  ...
  "transformation methods (includes Bernstein)\n"
)
...
clusterEvalQ(cl, {
  ...
  if (
    file.exists(
      "STEP_3_Application_Implementation/methods/csem_aware_smoother.R"
    )
  ) {
  ...
  tryCatch(
    {
      ...
      copula_summary <- list(
        ...
        aic_delta_from_empirical = aic_values[best_copula_family] -
          empirical_baseline$copula_fits$results[[
            empirical_baseline$best_family
          ]]$aic
      )
...
all_results_raw <- parLapply(
  cl,
  TRANSFORMATION_METHODS,
  process_transformation_method
)
...
cat(
  "Methods per minute:",
  round(length(TRANSFORMATION_METHODS) / as.numeric(runtime), 1),
  "\n\n"
)
...
summary_table <- rbindlist(lapply(names(all_results), function(method_name) {
  ...
  data.table(
    ...
    copula_correct = (res$copula_results$best_family ==
      empirical_baseline$best_family),
...
fwrite(
  summary_table,
  file.path(results_dir, "exp5_transformation_validation_summary.csv")
)
...
cat(
  "  -",
  file.path(results_dir, "exp5_transformation_validation_full.RData"),
  "\n\n"
)
