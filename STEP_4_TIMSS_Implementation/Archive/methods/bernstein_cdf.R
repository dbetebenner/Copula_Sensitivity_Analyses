...
fit_bernstein_cdf <- function(
  ...
  F_forward <- function(x) {
    ...
    approx(x_grid, F_fitted, xout = x_norm, yleft = 0, yright = 1, rule = 2)$y
  }
...
tune_bernstein_degree <- function(
  scores_norm,
  n,
  ...
  cv_scores <- sapply(degree_candidates, function(d) {
    fold_errors <- sapply(1:n_folds, function(fold) {
      ...
      tryCatch(
        {
          fit <- fit_bernstein_cdf(
            scores_norm[train_idx] * 100, # Denormalize temporarily
            degree = d,
            tune_by_cv = FALSE
          )
