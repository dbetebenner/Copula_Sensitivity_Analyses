...
fit_csem_aware <- function(scores, csem = 1.0, method = "pool_adjacent") {
  require(stats)
...
needs_csem_smoothing <- function(scores) {
  ...
  if (all(scores == floor(scores))) {
    ...
    heaping_5 <- if (length(mod5_counts) > 0) {
      max(mod5_counts) / n > 0.30
    } else {
      FALSE
    }
    heaping_10 <- if (length(mod10_counts) > 0) {
      max(mod10_counts) / n > 0.20
    } else {
      FALSE
    }
  ...
  needs_csem <- high_discretization ||
    boundary_inflation ||
    large_gaps ||
    heaping
...
compare_csem_smoothing <- function(scores, csem = NULL) {
  ...
  return(list(
    ...
    recommendation = if (cvm_csem < cvm_standard) {
      "Use CSEM-aware"
    } else {
      "Use standard"
    }
