...
if (!exists("all_results")) {
  load(
    "STEP_3_Application_Implementation/results/exp5_transformation_validation_full.RData"
  )
}
...
dir.create(
  "STEP_3_Application_Implementation/results/figures/exp5_transformation_validation",
  showWarnings = FALSE,
  recursive = TRUE
)
...
create_method_dashboard <- function(
  method_name,
  results,
  empirical_U,
  empirical_V
  ...
  points(
    sample(empirical_U, min(1000, length(empirical_U))),
  ...
  legend(
    "topleft",
    legend = c("This method", "Empirical ranks"),
...
pdf(
  "STEP_3_Application_Implementation/results/figures/exp5_transformation_validation/uniformity_forest_plot.pdf",
  width = 10,
  height = 8
)
...
plot(
  ks_pvals,
  y_pos,
  xlim = c(0, max(ks_pvals, 0.1)),
  pch = 19,
  col = colors,
  cex = 1.5,
  yaxt = "n",
  xlab = "K-S Test p-value",
  ylab = "",
...
pdf(
  "STEP_3_Application_Implementation/results/figures/exp5_transformation_validation/tail_concentration_comparison.pdf",
  width = 12,
  height = 6
)
...
axis(
  1,
  at = 1:length(lower_10),
  labels = names(lower_10),
  las = 2,
  cex.axis = 0.7
)
...
axis(
  1,
  at = 1:length(upper_90),
  labels = names(upper_90),
  las = 2,
  cex.axis = 0.7
)
...
pdf(
  "STEP_3_Application_Implementation/results/figures/exp5_transformation_validation/copula_selection_results.pdf",
  width = 10,
  height = 8
)
...
aic_deltas <- sapply(all_results, function(x) {
  x$copula_results$aic_delta_from_empirical
})
...
barplot(
  aic_deltas,
  names.arg = rep("", length(aic_deltas)),
  col = bar_colors,
  border = "white",
  main = expression(paste(
    "Copula Selection: ",
    Delta,
    "AIC from Empirical Best"
  )),
...
axis(
  1,
  at = 1:length(aic_deltas),
  labels = names(aic_deltas),
  las = 2,
  cex.axis = 0.7
)
...
pdf(
  "STEP_3_Application_Implementation/results/figures/exp5_transformation_validation/tradeoff_space.pdf",
  width = 10,
  height = 8
)
...
classifications <- sapply(all_results, function(x) {
...
plot(
  ...
  xlim = c(0, max(ks_pvals)),
  ylim = range(aic_deltas)
)
...
pdf(
  "STEP_3_Application_Implementation/results/figures/exp5_transformation_validation/key_methods_comparison.pdf",
  width = 14,
  height = 10
)
...
for (i in 1:6) {
  if (i <= length(key_methods)) {
    ...
    text(
      0.05,
      0.95,
      sprintf("K-S p = %.3f", res$uniformity$combined_ks_pval),
      adj = c(0, 1),
      cex = 0.8
    )
    ...
    text(
      0.05,
      0.88,
      paste("Copula:", res$copula_results$best_family),
      adj = c(0, 1),
      cex = 0.8,
      col = ifelse(
        res$copula_results$best_family == empirical_baseline$best_family,
        "darkgreen",
        "red"
      )
    )
