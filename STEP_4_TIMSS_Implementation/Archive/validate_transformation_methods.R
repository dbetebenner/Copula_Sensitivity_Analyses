...
fit_gauss_ranks <- fitCopula(
  cop_gauss_ranks,
  cbind(U_ranks, V_ranks),
  method = "ml"
)

cop_frank_ranks <- frankCopula(dim = 2)
fit_frank_ranks <- fitCopula(
  cop_frank_ranks,
  cbind(U_ranks, V_ranks),
  method = "ml"
)
...
results_ranks <- data.frame(
  ...
  LogLik = c(
    fit_gauss_ranks@loglik,
    fit_frank_ranks@loglik,
    fit_t_ranks@loglik
  ),
  AIC = c(
    -2 * fit_gauss_ranks@loglik + 2,
    -2 * fit_frank_ranks@loglik + 2,
    -2 * fit_t_ranks@loglik + 4
  ),
  Tau = c(
    tau(fit_gauss_ranks@copula),
...
cat(
  "  U: D =",
  round(ks_u_ispline$statistic, 6),
  ", p-value =",
...
cat(
  "  V: D =",
  round(ks_v_ispline$statistic, 6),
  ", p-value =",
...
fit_gauss_ispline <- fitCopula(
  cop_gauss_ispline,
  cbind(U_ispline, V_ispline),
  method = "ml"
)

cop_frank_ispline <- frankCopula(dim = 2)
fit_frank_ispline <- fitCopula(
  cop_frank_ispline,
  cbind(U_ispline, V_ispline),
  method = "ml"
)
...
results_ispline <- data.frame(
  Family = c("Gaussian", "Frank", "t"),
  LogLik = c(
    fit_gauss_ispline@loglik,
    fit_frank_ispline@loglik,
    fit_t_ispline@loglik
  ),
  AIC = c(
    -2 * fit_gauss_ispline@loglik + 2,
    -2 * fit_frank_ispline@loglik + 2,
    -2 * fit_t_ispline@loglik + 4
  ),
  Tau = c(
    tau(fit_gauss_ispline@copula),
...
cat(
  "  Empirical ranks winner:",
  results_ranks$Family[1],
  "(AIC:",
  round(results_ranks$AIC[1], 1),
  ")\n"
)
cat(
  "  I-spline winner:",
  results_ispline$Family[1],
  "(AIC:",
  round(results_ispline$AIC[1], 1),
  ")\n\n"
)
...
cat(
  "  Empirical ranks - Lower:",
  round(lower_tail_ranks, 4),
  "(",
  round(lower_tail_ranks / 0.01, 2),
  "x)\n"
)
cat(
  "  Empirical ranks - Upper:",
  round(upper_tail_ranks, 4),
  "(",
  round(upper_tail_ranks / 0.01, 2),
  "x)\n"
)
cat(
  "  I-spline - Lower:",
  round(lower_tail_ispline, 4),
  "(",
  round(lower_tail_ispline / 0.01, 2),
  "x)\n"
)
cat(
  "  I-spline - Upper:",
  round(upper_tail_ispline, 4),
  "(",
  round(upper_tail_ispline / 0.01, 2),
  "x)\n\n"
)
...
cat(
  ...
  ifelse(ranks_uniform, "✓ YES", "✗ NO"),
  "\n"
)
cat(
  "2. Improved I-spline produces uniform U,V:",
  ifelse(ispline_uniform, "✓ YES", "✗ NO"),
  "\n"
)
cat(
  "3. Both methods select same copula:",
  ifelse(same_winner, "✓ YES", "✗ NO"),
  "\n"
)
cat(
  "4. Winner is t-copula (expected):",
...
if (
  ...
  if (!ispline_uniform) {
    cat(
      "  Issue: I-spline still not uniform - need MORE knots or tail-aware knots\n"
    )
  }
  if (!same_winner) {
    cat(
      "  Issue: Methods select different copulas - transformation distortion still present\n"
    )
  }
