...
save_multi_format <- function(plot, base_path, width, height, dpi = 300) {
  ...
  ggsave(paste0(base_path, ".svg"), plot, width = width, height = height, bg = "transparent")
  cat("    Saved:", paste0(base_path, ".svg"), "\n")
  
  # PNG with transparent background
  ggsave(paste0(base_path, "@2x.png"), plot, width = width, height = height, 
         dpi = dpi * 2, bg = "transparent")
...
if (!is.null(sgp_data_path) && file.exists(sgp_data_path)) {
  ...
  cat("  Contains SGP columns: SGP_ORDER_1 =", "SGP_ORDER_1" %in% names(STATE_DATA_LONG),
      ", SGP =", "SGP" %in% names(STATE_DATA_LONG), "\n")
} else {
  # Fallback to base data file
load(current_dataset$local_path)
  STATE_DATA_LONG <- get(current_dataset$rdata_object_name)
  cat("  Loaded base data file (no SGP columns):", current_dataset$local_path, "\n")
}
...
if (has_sgp_order_1) {
  ...
  cat("  SGP_ORDER_1 valid values:", n_valid_order_1, 
...
if (has_sgp_best) {
  ...
  cat("  SGP (best) valid values:", n_valid_best, 
...
sgpc_emp <- sgpc_engine(u_obs, v_obs, empirical_copulas$bernstein, scale = "percentile")
...
cat("  Comonotonic SGPc: mean =", round(mean(sgpc_como, na.rm = TRUE), 1), "\n\n")
...
if (!is.null(panel_result_t)) {
  ...
  cat("    P(|diff|>10):", round(panel_result_t$statistics$pct_diff_gt_10 * 100, 1), "%\n")
  ...
  save_multi_format(panel_result_t$plot, 
                    file.path(output_dir, "test1_panel_t_heatmap_with_sgp"), 
                    width = 7, height = 12)  # Taller to accommodate summary table
  cat("\n")
} else {
...
if (!is.null(panel_result_como)) {
  cat("  Panel created successfully\n")
  cat("  Statistics:\n")
  cat("    KS distance:", round(panel_result_como$statistics$ks_distance, 4), "\n")
  cat("    Correlation:", round(panel_result_como$statistics$correlation, 4), "\n")
  cat("    P(|diff|>10):", round(panel_result_como$statistics$pct_diff_gt_10 * 100, 1), "%\n")
  ...
  save_multi_format(panel_result_como$plot, 
...
if (!is.null(combined_result_t$combined_plot)) {
  ...
  save_multi_format(combined_result_t$combined_plot, 
                    file.path(output_dir, "test3_combined_t_heatmap_with_sgp"), 
                    width = 15, height = 10)  # Taller to accommodate summary table
  cat("\n")
} else {
...
if (!is.null(combined_result_como$combined_plot)) {
  ...
  save_multi_format(combined_result_como$combined_plot, 
...
if (!is.null(sgp_order_1_values) && sum(!is.na(sgp_order_1_values)) > 10) {
  ...
  if (!is.null(emp_vs_sgp_result)) {
    ...
    cat("    Mean SGPc:", round(emp_vs_sgp_result$statistics$mean_sgpc, 1), "\n")
    cat("    Mean SGP:", round(emp_vs_sgp_result$statistics$mean_sgp, 1), "\n")
    cat("    Correlation:", round(emp_vs_sgp_result$statistics$correlation, 4), "\n")
    cat("    KS distance:", round(emp_vs_sgp_result$statistics$ks_distance, 4), "\n")
    ...
    save_multi_format(emp_vs_sgp_result$plot, 
...
cat("    - Diagonal cells: Large POSITIVE deviation (comonotonic over-represents)\n")
cat("    - Off-diagonal cells: Large NEGATIVE deviation (comonotonic under-represents)\n")
cat("\n")
cat("The heatmap shows how the parametric copula deviates from the empirical.\n")
...
if (has_valid_sgp_order_1 || has_valid_sgp_best) {
  ...
  if (has_valid_sgp_order_1) cat("    - SGP_ORDER_1 (Teal dashed line): Single prior b-spline SGP\n")
  if (has_valid_sgp_best) cat("    - SGP (Gold dashed line): Best available (typically 2 priors)\n")
  cat("\n")
  cat("  In the ECDF plot (top):\n")
  cat("    - Solid black: Empirical SGPc\n")
  cat("    - Solid magenta: Parametric SGPc\n")
  if (has_valid_sgp_order_1) cat("    - Dashed teal: SGP_ORDER_1 (traditional, 1 prior)\n")
  if (has_valid_sgp_best) cat("    - Dashed gold: SGP (traditional, best available)\n")
  cat("\n")
  cat("  In the SGPc vs SGP comparison plot (TEST 5):\n")
  cat("    - Direct comparison between empirical copula SGPc and traditional SGP\n")
  cat("    - Decile cross-tabulation shows agreement/disagreement patterns\n")
} else {
  cat("  ✗ No valid SGP data was found for this condition - plots show SGPc only\n")
