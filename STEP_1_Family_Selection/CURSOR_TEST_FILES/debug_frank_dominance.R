...
abline(
  lm(SCALE_SCORE_CURRENT ~ SCALE_SCORE_PRIOR, data = pairs_full),
  col = "red",
  lwd = 2
)
grid()
...
plot(
  pairs_full$SCALE_SCORE_PRIOR,
  pairs_full$SCALE_SCORE_CURRENT,
  xlim = range(pairs_full$SCALE_SCORE_PRIOR[
    pairs_full$SCALE_SCORE_PRIOR < low_cutoff
  ]),
  ylim = range(pairs_full$SCALE_SCORE_CURRENT[
    pairs_full$SCALE_SCORE_PRIOR < low_cutoff
  ]),
  pch = 20,
  cex = 0.5,
...
plot(
  ...
  pairs_full$SCALE_SCORE_CURRENT,
  xlim = range(pairs_full$SCALE_SCORE_PRIOR[
    pairs_full$SCALE_SCORE_PRIOR > high_cutoff
  ]),
  ylim = range(pairs_full$SCALE_SCORE_CURRENT[
    pairs_full$SCALE_SCORE_PRIOR > high_cutoff
  ]),
...
cat(
  "  Prior mean:",
  round(mean(extreme_low_5$SCALE_SCORE_PRIOR), 1),
  " (percentile: 2.5)\n"
)
...
cat(
  "  Prior mean:",
  round(mean(extreme_high_5$SCALE_SCORE_PRIOR), 1),
  " (percentile: 97.5)\n"
)
...
cat(
  "  Current percentile:",
  round(
    100 * mean(extreme_high_5$SCALE_SCORE_CURRENT > quantile_current[4]),
    1
  ),
  "% still in top 5%\n"
)
cat(
  "  Current percentile:",
  round(
    100 * mean(extreme_high_5$SCALE_SCORE_CURRENT > quantile_current[3]),
    1
  ),
  "% still in top 10%\n\n"
)
...
text(
  0.5,
  max(hist(U, breaks = 50, plot = FALSE)$counts) * 1.1,
  paste("K-S p =", format.pval(ks_u$p.value, digits = 2)),
  col = "red"
)
...
text(
  0.5,
  max(hist(V, breaks = 50, plot = FALSE)$counts) * 1.1,
  paste("K-S p =", format.pval(ks_v$p.value, digits = 2)),
  col = "red"
)

# Q-Q plots - should be straight line if uniform
qqplot(
  qunif(ppoints(length(U))),
  U,
...
chiplot_func <- function(u, v, n_points = 100) {
  ...
  chi_vals <- sapply(h_seq, function(h) {
    ...
    chi <- (prob_both - prob_u * prob_v) /
      sqrt(prob_u * (1 - prob_u) * prob_v * (1 - prob_v))
    return(chi)
  })
...
grid()
text(0.05, 0.095, paste("n =", sum(U < 0.1 & V < 0.1)), col = "blue", font = 2)
...
grid()
text(0.95, 0.905, paste("n =", sum(U > 0.9 & V > 0.9)), col = "red", font = 2)
...
plot(
  ...
  ylim = c(
    min(chi_result$chi, na.rm = TRUE) * 1.1,
...
text(
  0.7,
  max(chi_result$chi, na.rm = TRUE) * 0.9,
...
cat(
  "  Lower tail (both < 10%):",
  round(lower_tail_conc, 4),
  " (expected under independence: 0.0100)\n"
)
cat(
  "  Upper tail (both > 90%):",
  round(upper_tail_conc, 4),
  " (expected under independence: 0.0100)\n"
)
cat(
  "  Lower tail concentration ratio:",
  round(lower_tail_conc / 0.01, 2),
  "x\n"
)
cat(
  "  Upper tail concentration ratio:",
  round(upper_tail_conc / 0.01, 2),
  "x\n\n"
)

cat(
  "Final chi value (h=0.99):",
  round(tail(na.omit(chi_result$chi), 1), 4),
  "\n\n"
)
...
cat(
  "  t-copula: rho =",
  round(fit_t@estimate[1], 4),
  ", df =",
  round(fit_t@estimate[2], 2),
  "\n\n"
)
...
test_points$dens_gauss <- dCopula(
  as.matrix(test_points[, 1:2]),
  cop_gauss_fitted
)
test_points$dens_frank <- dCopula(
  as.matrix(test_points[, 1:2]),
  cop_frank_fitted
)
...
points(
  U[sample(length(U), 1000)],
  V[sample(length(V), 1000)],
  pch = 20,
  cex = 0.3,
  col = rgb(0, 0, 0, 0.3)
)
...
cat(
  "- Ratios > 1.2 in center (0.5, 0.5) → Frank better at central dependence\n"
)
cat(
  "- Red regions in difference plots show where Frank assigns more density\n\n"
)
...
ll_gauss_center <- sum(dCopula(
  pseudo_obs[center, ],
  cop_gauss_fitted,
  log = TRUE
))
ll_gauss_tails <- sum(dCopula(
  pseudo_obs[tails, ],
  cop_gauss_fitted,
  log = TRUE
))
ll_gauss_moderate <- sum(dCopula(
  pseudo_obs[moderate, ],
  cop_gauss_fitted,
  log = TRUE
))
...
ll_frank_center <- sum(dCopula(
  pseudo_obs[center, ],
  cop_frank_fitted,
  log = TRUE
))
ll_frank_tails <- sum(dCopula(
  pseudo_obs[tails, ],
  cop_frank_fitted,
  log = TRUE
))
ll_frank_moderate <- sum(dCopula(
  pseudo_obs[moderate, ],
  cop_frank_fitted,
  log = TRUE
))
...
cat(
  "Frank's total advantage over t:",
  round(ll_frank_total - ll_t_total, 1),
  "\n"
)
cat(
  "Frank's advantage in center:",
  round(ll_frank_center - ll_t_center, 1),
  " (",
  round(
    100 * (ll_frank_center - ll_t_center) / (ll_frank_total - ll_t_total),
    1
  ),
  "%)\n"
)
cat(
  "Frank's advantage in tails:",
...
cat(
  "  Gaussian simulation:",
  round(cor(sim_gauss[, 1], sim_gauss[, 2], method = "kendall"), 4),
  "\n"
)
cat(
  "  Frank simulation:",
  round(cor(sim_frank[, 1], sim_frank[, 2], method = "kendall"), 4),
  "\n"
)
cat(
  "  t simulation:",
  round(cor(sim_t[, 1], sim_t[, 2], method = "kendall"), 4),
  "\n\n"
)
...
plot(
  ...
  ylim = c(0, 1),
  main = paste0(
    "t-Copula Simulation (tau = ",
    round(cor(sim_t[, 1], sim_t[, 2], method = "kendall"), 3),
    ", df = ",
    round(fit_t@estimate[2], 1),
    ")"
  ),
  xlab = "U",
  ylab = "V"
...
cat(
  "  Empirical tau:",
  cor(U_empirical, V_empirical, method = "kendall"),
  "\n\n"
)
...
fit_gauss_emp <- fitCopula(
  cop_gauss_emp,
  cbind(U_empirical, V_empirical),
  method = "ml"
)

cat("Fitting Frank copula...\n")
cop_frank_emp <- frankCopula(dim = 2)
fit_frank_emp <- fitCopula(
  cop_frank_emp,
  cbind(U_empirical, V_empirical),
  method = "ml"
)
...
empirical_results <- data.frame(
  ...
  LogLik = c(
    fit_gauss_emp@loglik,
    fit_frank_emp@loglik,
    fit_t_emp@loglik,
    fit_clay_emp@loglik,
    fit_gumb_emp@loglik
  ),
  AIC = c(
    -2 * fit_gauss_emp@loglik + 2,
  ...
  Tau = c(
    tau(fit_gauss_emp@copula),
    tau(fit_frank_emp@copula),
    tau(fit_t_emp@copula),
    tau(fit_clay_emp@copula),
    tau(fit_gumb_emp@copula)
  )
)

empirical_results$Delta_AIC <- empirical_results$AIC -
  min(empirical_results$AIC)
