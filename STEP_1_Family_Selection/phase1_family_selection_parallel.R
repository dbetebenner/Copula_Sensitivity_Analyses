...
if (
  exists("TEST_MODE", envir = .GlobalEnv) &&
    get("TEST_MODE", envir = .GlobalEnv)
) {
...
if (exists("N_BOOTSTRAP_GOF", envir = .GlobalEnv)) {
  ...
  cat(
    "Goodness-of-Fit Testing: ENABLED (N =",
    N_BOOTSTRAP_GOF_VALUE,
    "bootstrap samples)\n"
  )
} else {
...
if (exists("GENERATE_UNCERTAINTY_PLOTS", envir = .GlobalEnv)) {
  GENERATE_UNCERTAINTY_PLOTS_VALUE <- get(
...
if (exists("GENERATE_CONTOUR_PLOTS", envir = .GlobalEnv)) {
  GENERATE_CONTOUR_PLOTS_VALUE <- get(
...
if (exists("N_BOOTSTRAP_UNCERTAINTY", envir = .GlobalEnv)) {
  N_BOOTSTRAP_UNCERTAINTY_VALUE <- get(
...
if (exists("BOOTSTRAP_ALL_FAMILIES", envir = .GlobalEnv)) {
  BOOTSTRAP_ALL_FAMILIES_VALUE <- get(
    "BOOTSTRAP_ALL_FAMILIES",
    envir = .GlobalEnv
  )
} else {
...
if (exists("UNCERTAINTY_GRID_SIZE", envir = .GlobalEnv)) {
  UNCERTAINTY_GRID_SIZE_VALUE <- get(
...
if (
  GENERATE_UNCERTAINTY_PLOTS_VALUE &&
  ...
  cat(
    "  Families:",
    ifelse(BOOTSTRAP_ALL_FAMILIES_VALUE, "ALL (5 parametric)", "BEST ONLY"),
    "\n"
  )
...
if (!exists("DATASETS", envir = .GlobalEnv)) {
  stop(
    "ERROR: DATASETS not found in .GlobalEnv. Ensure dataset_configs.R is sourced first."
  )
}
...
cat(
  "  Datasets available:",
  paste(names(DATASETS_CONFIG), collapse = ", "),
  "\n"
)
...
init_packages <- everywhere(
  {
    ...
    cat(
      "[DAEMON] Starting initialization at",
    ...
    tryCatch(
      {
        ...
        cat(
          "[DAEMON] Thread management: Single-threaded mode enabled (prevents oversubscription)\n"
        )
      },
      error = function(e) {
...
# Check results
n_success <- sum(sapply(init_packages_results, function(x) {
...
if (n_fail > 0) {
  cat(
    "  WARNING: Some daemons failed package initialization (",
    n_fail,
    "/",
    length(init_packages_results),
    ")\n"
  )
  # Show failed results
  ...
  cat(
    "  ✓ Packages and functions loaded on all",
    length(init_packages_results),
    "daemons\n"
  )
}
...
cat(
  "  Package initialization complete. Data export deferred until process_condition is defined.\n\n"
)
...
if (SKIP_COMONOTONIC_VALUE) {
  ...
  cat(
    "Copula families: gaussian, t, clayton, gumbel, frank (comonotonic SKIPPED)\n"
  )
} else {
  COPULA_FAMILIES <- c(
    "gaussian",
...
if (exists("USE_EXHAUSTIVE_ALL_DATASETS", envir = .GlobalEnv)) {
  USE_EXHAUSTIVE_CONDITIONS <- get(
    "USE_EXHAUSTIVE_ALL_DATASETS",
    envir = .GlobalEnv
  )
}
...
if (exists("ALL_DATASET_CONFIGS", envir = .GlobalEnv)) {
  ...
  for (ds_id in names(all_dataset_configs)) {
    ...
    if (USE_EXHAUSTIVE_CONDITIONS) {
      ...
      ds_conditions <- generate_exhaustive_conditions(
        ds_config,
        max_year_span = 4
      )
    ...
    for (i in seq_along(ds_conditions)) {
      ...
      cond$transition_year <- if (ds_config$has_transition) {
        ds_config$transition_year
      } else {
        NA
      }
  ...
  cat(
    "  (This prevents memory spikes from all workers loading dataset_1 simultaneously)\n\n"
  )
  # === END RANDOMIZATION ===
  ...
  if (USE_EXHAUSTIVE_CONDITIONS) {
    cat(
      "Using EXHAUSTIVE conditions for",
...
if (!exists("MULTI_DATASET_MODE") || !MULTI_DATASET_MODE) {
  ...
  if (
    ...
    for (i in seq_along(CONDITIONS)) {
      ...
      cond$current_scaling_type <- get_scaling_type(
        current_dataset,
        year_current
      )
      cond$scaling_transition_type <- get_scaling_transition_type(
...
if (
  exists("QUICK_TEST_MODE", envir = .GlobalEnv) &&
    get("QUICK_TEST_MODE", envir = .GlobalEnv)
) {
  ...
  if (prefer_smallest && exists("MULTI_DATASET_MODE") && MULTI_DATASET_MODE) {
    ...
    ds4_conditions_idx <- which(sapply(CONDITIONS, function(c) {
      c$dataset_id == smallest_dataset
    }))

    if (length(ds4_conditions_idx) > 0) {
      cat("Filtering to", smallest_dataset, "(smallest, ~1.6M rows)...\n")
      cat(
        "  Available conditions from",
        smallest_dataset,
        ":",
        length(ds4_conditions_idx),
        "\n"
      )
      ...
      cat(
        "WARNING: No conditions found for",
        smallest_dataset,
        ", using first",
        n_quick_test,
        "from pool\n"
  ...
  if (
    ...
    for (ds_id in names(all_dataset_configs)) {
      ds_conditions_idx <- which(sapply(CONDITIONS, function(c) {
        c$dataset_id == ds_id
      }))
      ...
      selected_conditions <- c(
        selected_conditions,
        CONDITIONS[ds_conditions_idx]
      )
      cat(
        "  ",
        ds_id,
        ": selected",
        length(ds_conditions_idx),
        "of",
        sum(sapply(CONDITIONS, function(c) c$dataset_id == ds_id)),
        "conditions\n"
      )
    }
    ...
    if (original_count_test > n_conditions_test) {
      ...
      if (n_conditions_test >= 4 && USE_EXHAUSTIVE_CONDITIONS) {
        spans_available <- sapply(CONDITIONS, function(c) {
          c$year_span %||% c$span
        })
        selected_indices <- c()
...
if (
  !exists("get_completed_conditions") &&
    file.exists("functions/checkpoint_utils.R")
) {
...
if (exists("SKIP_COMPLETED_CONDITIONS", envir = .GlobalEnv)) {
  SKIP_COMPLETED_CONDITIONS_VALUE <- get(
...
if (
  SKIP_COMPLETED_CONDITIONS_VALUE &&
    length(CONDITIONS) > 0 &&
    exists("get_completed_conditions")
) {
  cat("\n")
...
process_condition <- function(
  ...
  write_condition_progress <- function(output_dir, status, error_msg = NULL) {
    ...
    for (step_name in names(step_timings)) {
      ...
      pct <- if (overall_elapsed > 0) {
        (result$elapsed_secs / overall_elapsed) * 100
      } else {
        0
      }
  ...
  tryCatch(
    {
      ...
      for (family in copula_families) {
        if (!is.null(copula_fits$results[[family]])) {
          ...
          degrees_freedom <- if (
            family %in% c("t", "t_df5", "t_df10", "t_df15")
          ) {
            param_2
          } else {
            NA_real_
          ...
          family_results[[family]] <- data.table(
            ...
            scaling_transition_type = if (
              !is.null(cond$scaling_transition_type)
            ) {
            ...
            condition_id = i,
            year_span = if (!is.null(cond$year_span)) {
              cond$year_span
            } else {
              cond$span
            },
            ...
            gof_pvalue = if (!is.null(fit$gof_pvalue)) {
              fit$gof_pvalue
            } else {
              NA_real_
            },
      ...
      if (GENERATE_CONTOUR_PLOTS && !is.null(copula_fits$pseudo_obs)) {
        ...
        if (exists("generate_condition_plots")) {
          ...
          dataset_config <- if (
            exists("DATASETS", envir = .GlobalEnv) &&
              !is.null(DATASETS[[dataset_id]])
          ) {
            DATASETS[[dataset_id]]
          ...
          condition_info <- list(
            ...
            scale_note = if (!is.null(dataset_config)) {
              dataset_config$notes
            } else {
              NA
            },
          ...
          if (
            isTRUE(GENERATE_UNCERTAINTY_PLOTS_VALUE) &&
            ...
            bootstrap_info <- sprintf(
              "%d samples × %d families",
              N_BOOTSTRAP_UNCERTAINTY_VALUE,
            ...
            bootstrap_results <- tryCatch(
              ...
              error = function(e) {
                warning(sprintf(
                  "Bootstrap failed for condition %d: %s",
                  i,
                  e$message
                ))
                NULL
          ...
          tryCatch(
            ...
            error = function(e) {
              warning(sprintf(
                "Failed to generate plots for condition %d: %s",
                i,
                e$message
              ))
            }
          ...
          record_step(
            ...
            sprintf(
              "grid=%d, formats=%s",
              GRID_SIZE_VALUE,
              paste(EXPORT_FORMATS_VALUE, collapse = ",")
            )
          )
        }
      }
      ...
      if (isTRUE(CALCULATE_SGPC_VALUE) && !is.null(copula_fits$pseudo_obs)) {
          ...
          if (file.exists(empirical_copulas_file)) {
            ...
            if (!is.null(emp_copulas)) {
              ...
              if (!is.null(emp_copulas$bernstein)) {
                sgpc_bernstein <- sgpc_engine(
                  u_sgpc,
                  v_sgpc,
                  emp_copulas$bernstein,
                  scale = "percentile"
                )
        ...
        if (!is.null(plot_output_dir)) {
          ...
          if (length(sgpc_cols) > 1) {
            cor_matrix <- cor(
              sgpc_results[, ..sgpc_cols],
              use = "pairwise.complete.obs"
            )
            saveRDS(
              cor_matrix,
              file.path(sgpc_output_dir, "sgpc_correlations.rds")
            )
          }
        }
      ...
      if (!is.null(plot_output_dir) && dir.exists(plot_output_dir)) {
        tryCatch(
          ...
          error = function(e) {
            warning(sprintf(
              "Failed to write condition progress: %s",
              e$message
            ))
          }
        )
      ...
      complete_msg <- sprintf(
        "[%s] COMPLETE %3d/%d: %-4s G%d->G%d (%.1f min, n=%d, best=%s)\n",
      ...
      if (exists("sgpc_results")) {
        rm(sgpc_results)
      }
      if (exists("copula_fits")) {
        rm(copula_fits)
      }
    ...
    error = function(e) {
      ...
      elapsed <- as.numeric(difftime(end_time, overall_start, units = "mins"))
      error_msg <- sprintf(
        "[%s] ERROR    %3d/%d: %-4s G%d->G%d (%.1f min) - %s\n",
        format(end_time, "%H:%M:%S"),
        i,
        total_conditions,
        cond$content,
...
cat(
  "Copula families:",
  paste(COPULA_FAMILIES, collapse = ", "),
  "\n",
  file = progress_file,
  append = TRUE
)
...
if (SKIP_PROCESSING) {
  ...
  init_data <- everywhere(
    {
      ...
      cat(
        "[DAEMON] Starting config initialization at",
        format(Sys.time(), "%H:%M:%S"),
        "\n"
      )
      ...
      if (!exists("process_condition")) {
        cat(
          "[DAEMON ERROR] process_condition not found in daemon environment\n"
        )
      ...
      cat(
        "[DAEMON] DATASETS_CONFIG available with",
        length(DATASETS_CONFIG),
        "datasets\n"
      )
      ...
      config_vars <- c(
        "N_BOOTSTRAP_GOF_VALUE",
        "CALCULATE_SGPC_VALUE",
        "GENERATE_UNCERTAINTY_PLOTS_VALUE",
        "GENERATE_CONTOUR_PLOTS_VALUE",
        "N_BOOTSTRAP_UNCERTAINTY_VALUE",
        "BOOTSTRAP_ALL_FAMILIES_VALUE",
        "GRID_SIZE_VALUE",
        "UNCERTAINTY_GRID_SIZE_VALUE",
        "SKIP_COMONOTONIC_VALUE",
        "COMPARISON_FAMILIES_VALUE",
        "EXPORT_FORMATS_VALUE",
        "EXPORT_DPI_VALUE",
        "EXPORT_VERBOSE_VALUE"
      )
  ...
  n_success <- sum(sapply(init_data_results, function(x) {
    isTRUE(x) || identical(x, TRUE)
  }))
  ...
  if (n_fail > 0) {
    cat(
      "  WARNING: Some daemons failed config initialization (",
      n_fail,
      "/",
      length(init_data_results),
      ")\n"
    )
  ...
  mirai_results <- mirai_map(
    ...
    .f = function(
      ...
      if (
        !is.null(dataset_id) &&
          exists("DATASETS_CONFIG") &&
          dataset_id %in% names(DATASETS_CONFIG)
      ) {
        ...
        if (USE_SGP_DATA_VALUE && !is.null(ds_config$local_path_sgp)) {
          ds_path <- if (IS_EC2_VALUE) {
            ds_config$ec2_path_sgp
          } else {
        ...
        current_dataset_id <- if (exists(".LOADED_DATASET_ID")) {
          .LOADED_DATASET_ID
        } else {
          NULL
        }
  ...
  cat(
    ...
    round(duration / total_conditions, 2),
    "minutes\n",
    file = progress_file,
    append = TRUE
  )
  ...
  cat(
    ...
    round(duration / length(CONDITIONS), 2),
    "minutes\n\n"
  )
...
generate_run_summary <- function(results_dir, all_results, duration_mins) {
  summary_file <- file.path(
    results_dir,
    sprintf("RUN_SUMMARY_%s.txt", format(Sys.Date(), "%Y-%m-%d"))
  )

  # Find all condition.progress files
  progress_files <- list.files(
    results_dir,
    pattern = "condition\\.progress$",
    recursive = TRUE,
    full.names = TRUE
  )
  ...
  if (length(step_timings) > 0) {
    ...
    for (step_name in names(step_timings)) {
      ...
      cat(sprintf(
        "%-25s  avg=%6.1fs  med=%6.1fs  max=%6.1fs  [%5.1f%%]\n",
        paste0(step_name, ":"),
        avg_secs,
        med_secs,
        max_secs,
  ...
  best_families <- sapply(
    all_results[sapply(all_results, function(x) x$success)],
...
if (exists("MULTI_DATASET_MODE") && MULTI_DATASET_MODE) {
  ...
  cat(
    "\nTotal conditions tested (all datasets):",
    uniqueN(paste(results_dt$dataset_id, results_dt$condition_id)),
    "\n"
  )
...
best_families <- results_dt[,
  .(best_family = family[which.min(aic)]),
  by = condition_id
]
...
if (exists("MULTI_DATASET_MODE") && MULTI_DATASET_MODE) {
  ...
  family_by_dataset <- results_dt[,
    .(best_family = family[which.min(aic)]),
    by = .(dataset_id, condition_id)
  ]
  family_by_dataset_summary <- family_by_dataset[,
    .N,
    by = .(dataset_id, best_family)
  ]
...
cat(
  "Combined file: STEP_1_Family_Selection/results/dataset_all/phase1_copula_family_comparison_all_datasets.csv\n\n"
)
