...
step1_csv <- file.path(
  normalizePath(
    file.path(margins_dir, "../../../../STEP_1_Family_Selection/results"),
    mustWork = FALSE
  ),
  step1_dataset_id,
...
if (n_pop_stayers_actual > max_pop_scatter_dots) {
  ...
  cat(
    "  Subsampled",
    max_pop_scatter_dots,
    "of",
    format(n_pop_stayers_actual, big.mark = ","),
    "population stayers for scatter\n"
  )
} else {
...
if (n_stayers_actual > max_sub_scatter_dots) {
  ...
  cat(
    "  Subsampled",
    max_sub_scatter_dots,
    "of",
    n_stayers_actual,
    "subgroup stayers for scatter\n"
  )
} else {
...
sub_prior_scores <- c(
  sub_stayers$SCALE_SCORE_PRIOR,
  sub_leavers$SCALE_SCORE_PRIOR
)
sub_current_scores <- c(
  sub_stayers$SCALE_SCORE_CURRENT,
  sub_entrants$SCALE_SCORE_CURRENT
)
...
write_dat(
  round(x_norm, 6),
  round(pdf_prior_pop / max_pdf_bot, 6),
  "pdf_bottom_pop_norm.dat"
)
write_dat(
  round(x_norm, 6),
  round(d_x$y / max_pdf_bot, 6),
  "pdf_bottom_sub_norm.dat"
)
...
write_dat(
  round(pdf_current_pop / max_pdf_left, 6),
  round(y_norm, 6),
  "pdf_left_pop_norm.dat"
)
write_dat(
  round(d_y$y / max_pdf_left, 6),
  round(y_norm, 6),
  "pdf_left_sub_norm.dat"
)
...
pop_scatter_tex <- sprintf(
  "  \\psdot[dotsize=1.5pt,fillcolor=populationRef,linecolor=populationRef,fillstyle=solid,opacity=0.3,dotstyle=o](%s,%s)%%",
  round(u_pop_scatter, 4),
  round(v_pop_scatter, 4)
)
write_tex_lines(pop_scatter_tex, "scatter_population.tex")
cat("  Population:", n_pop_scatter, "dots\n")

# Subgroup foreground dots (teal, larger dots)
sub_scatter_tex <- sprintf(
  "  \\psdot[dotsize=2.0pt,fillcolor=subgroupColor,linecolor=black,fillstyle=solid,opacity=0.6,dotstyle=o](%s,%s)%%",
  round(u_sub_scatter, 4),
  round(v_sub_scatter, 4)
)
write_tex_lines(sub_scatter_tex, "scatter_subgroup.tex")
cat("  Subgroup:", n_sub_scatter, "dots\n")

# Variant _2: smaller, more transparent dots so contour lines are prominent
pop_scatter_tex_2 <- sprintf(
  "  \\psdot[dotsize=1.0pt,fillcolor=populationRef,linecolor=populationRef,fillstyle=solid,opacity=0.2,dotstyle=o](%s,%s)%%",
  round(u_pop_scatter, 4),
  round(v_pop_scatter, 4)
)
...
sub_scatter_tex_2 <- sprintf(
  "  \\psdot[dotsize=1.5pt,fillcolor=subgroupColor,linecolor=subgroupColor,fillstyle=solid,opacity=0.4,dotstyle=o](%s,%s)%%",
  round(u_sub_scatter, 4),
  round(v_sub_scatter, 4)
)
...
rug_bot <- c(
  sprintf(
    "  \\psline[linecolor=stayerColor,linewidth=0.3pt](%s,0)(%s,1)%%",
    round(u_rug_stayers, 4),
    round(u_rug_stayers, 4)
  ),
  sprintf(
    "  \\psline[linecolor=leaverColor,linewidth=0.5pt](%s,0)(%s,1)%%",
    round(u_sub_leavers, 4),
    round(u_sub_leavers, 4)
  )
)
...
rug_left <- c(
  sprintf(
    "  \\psline[linecolor=stayerColor,linewidth=0.3pt](0,%s)(1,%s)%%",
    round(v_rug_stayers, 4),
    round(v_rug_stayers, 4)
  ),
  sprintf(
    "  \\psline[linecolor=entrantColor,linewidth=0.5pt](0,%s)(1,%s)%%",
    round(v_sub_entrants, 4),
    round(v_sub_entrants, 4)
  )
)
...
cat(
  "  Bottom strip: subgroup n =",
  length(u_sub_all),
  " leavers n =",
  length(u_sub_leavers),
  "\n"
)
cat(
  "  Left strip:   subgroup n =",
  length(v_sub_all),
  " entrants n =",
  length(v_sub_entrants),
  "\n"
)
cat(
  "  Leaver proportion range: [",
...
param_lines <- c(
  ...
  sprintf(
    "\\def\\nPopEntrants{%s}",
    format(nrow(pop_entrants), big.mark = ",")
  ),
  sprintf(
    "\\def\\popAlpha{%.2f}",
  ...
  sprintf(
    ...
    round(
      n_stayers_actual /
        max(
          n_stayers_actual + n_leavers_actual,
          n_stayers_actual + n_entrants_actual
        ) *
        100,
      0
    )
  ...
  sprintf(
    "\\def\\exampleURound{%s}",
    formatC(round(u_example, 2), format = "f", digits = 2)
  ),
  sprintf(
    "\\def\\exampleVRound{%s}",
    formatC(round(v_example, 2), format = "f", digits = 2)
  ),
  "",
  ...
  if (step1_available) {
    c(
      ...
      sprintf(
        "\\def\\copulaSecondFamily{%s}",
        paste0(
          toupper(substring(copula_second, 1, 1)),
...
trace_lines <- c(
  ...
  sprintf(
    "\\def\\nTotalU{%s}",
    format(n_stayers_actual + n_leavers_actual, big.mark = ",")
  ),
  sprintf(
    "\\def\\nTotalV{%s}",
    format(n_stayers_actual + n_entrants_actual, big.mark = ",")
  ),
  sprintf(
    "\\def\\linkageRate{%s}",
    round(
  ...
  sprintf(
    "\\def\\copulaRho{%s}",
    round(
      cor(sub_stayers$SCALE_SCORE_PRIOR, sub_stayers$SCALE_SCORE_CURRENT),
      3
    )
  ),
...
cat(
  "  Rug marks: bottom =",
  length(u_sub_stayers) + length(u_sub_leavers),
  " left =",
  length(v_sub_stayers) + length(v_sub_entrants),
  "\n"
)
