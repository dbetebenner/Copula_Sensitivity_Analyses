...
# Main builder
# ---------------------------------------------------------------------------
run_step3_analytic_explanation <- function(
  cfg = STEP3_ANALYTIC_EXPLANATION_CONFIG,
  verbose = TRUE
) {
  script_dir <- .locate_script_dir()
  step3_dir <- normalizePath(
  ...
  for (src in required_sources) {
    if (!file.exists(src)) {
      stop("Required source file missing: ", src)
    }
    source(src)
  }
  ...
  if (!requireNamespace("copula", quietly = TRUE)) {
    stop(
      "Package 'copula' is required. Install with install.packages('copula')."
    )
  }
  ...
  .create_kernel_cache_safe <- function(
    copula_obj,
    u_grid_size = 201L,
    v_grid_size = 201L,
    boundary_buffer = 0.005,
  ...
  v_grid <- seq(
    cfg$boundary_buffer,
    1 - cfg$boundary_buffer,
    length.out = cfg$v_grid_size
  )
  ...
  w1_reduction_pct <- 100 *
    (1 - (d_inferred$wasserstein1 / d_uniform$wasserstein1))
  ...
  summary_csv <- file.path(
    output_dir,
    paste0(cfg$output_basename, "_summary.csv")
  )
  utils::write.csv(summary_df, summary_csv, row.names = FALSE)

  grid_csv <- file.path(
    output_dir,
    paste0(cfg$output_basename, "_grid_search.csv")
  )
  ...
  plot_fn <- function() {
    ...
    polygon(
      c(d_u$x, rev(d_u$x)),
      c(d_u$y, rep(0, length(d_u$y))),
      col = adjustcolor(STEP3_FIG_COLORS$prior, alpha.f = 0.20),
      border = NA
    )
    lines(d_u$x, d_u$y, lwd = cfg$line_width, col = STEP3_FIG_COLORS$prior)
    polygon(
      c(d_v$x, rev(d_v$x)),
      c(d_v$y, rep(0, length(d_v$y))),
      col = adjustcolor(STEP3_FIG_COLORS$current, alpha.f = 0.16),
      border = NA
    )
    ...
    mtext(
      "No student-level pair IDs are available",
      side = 3,
      line = 0.3,
      cex = 0.80,
    ...
    lines(
      v_grid,
      F_uniform,
      lwd = cfg$line_width - 0.1,
      lty = 3,
      col = STEP3_FIG_COLORS$uniform
    )
    lines(
      v_grid,
      est$F_pred,
      lwd = cfg$line_width,
      lty = 2,
      col = STEP3_FIG_COLORS$inferred
    )
    legend(
      "bottomright",
      legend = c(
        "Observed F_obs(v)",
        "Uniform H(p)=p prediction",
        "Inferred H_S prediction"
      ),
      col = c(
        STEP3_FIG_COLORS$observed,
        STEP3_FIG_COLORS$uniform,
        STEP3_FIG_COLORS$inferred
      ),
    ...
    text(
      0.03,
      0.84,
      labels = "F_H(v) = E_U[ H(F_0(v | U)) ]",
      adj = c(0, 1),
      cex = 0.76,
      col = STEP3_FIG_COLORS$body
    )
    box(col = STEP3_FIG_COLORS$border)
    ...
    text(
      est$m_hat * 100,
      log10(est$kappa_hat),
      labels = "  optimum (m_hat, kappa_hat)",
      pos = 4,
      cex = 0.78
    )
    mtext(
      "Color = log10 distance",
      side = 3,
      line = 0.2,
      cex = 0.72,
      col = "grey35"
    ...
    polygon(
      c(p_grid * 100, rev(p_grid * 100)),
      c(d_inf, rep(0, length(d_inf))),
      col = adjustcolor(STEP3_FIG_COLORS$inferred, alpha.f = 0.20),
      border = NA
    )
    lines(
      p_grid * 100,
    ...
    mtext(
      sprintf(
        ...
        d_uniform$wasserstein1,
        d_inferred$wasserstein1,
        w1_reduction_pct
      ),
      side = 1,
      outer = TRUE,
  ...
  if (requireNamespace("svglite", quietly = TRUE)) {
    export_formats <- c(export_formats, "svg")
  }
  if (requireNamespace("ragg", quietly = TRUE)) {
    export_formats <- c(export_formats, "png")
  }
