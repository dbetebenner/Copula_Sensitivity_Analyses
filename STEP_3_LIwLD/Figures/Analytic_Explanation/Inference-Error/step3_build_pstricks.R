  ...
  if (!file.exists(pdf_file)) {
    ...
    if (file.exists(log_file)) {
      ...
      if (length(err_lines) > 0) {
        ...
        fatal <- grep(
          "Fatal|Emergency|cannot find|not found|No file",
          log_lines,
          value = TRUE
        )
        if (length(fatal) > 0) {
          cat("    Log warnings:\n")
          for (msg in fatal[1:min(5, length(fatal))]) {
            cat("      ", msg, "\n")
          }
        }
      }
...
for (p in graphic_panels) {
  ok <- compile_pstricks(p, verbose = VERBOSE) && ok
}
for (p in text_panels) {
  ok <- compile_xelatex(p, verbose = VERBOSE) && ok
}
