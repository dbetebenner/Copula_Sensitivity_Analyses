...
write_dat(
  ...
  "panel_B_cdf_uniform.dat"
)
write_dat(
  round(dat$v_grid, 6),
  round(dat$est$F_pred, 6),
  "panel_B_cdf_inferred.dat"
)
...
metrics <- c(
  ...
  sprintf(
    "\\newcommand{\\inferredRegimeMean}{%.1f}",
  ...
  sprintf(
    "\\newcommand{\\wOneUniform}{%.4f}",
    dat$distances$uniform$wasserstein1
  ),
  sprintf(
    "\\newcommand{\\wOneInferred}{%.4f}",
    dat$distances$inferred$wasserstein1
  ),
...
axes <- c(
  ...
  sprintf(
    "\\newcommand{\\inferredMeanVline}{%s}",
    round(dat$est$regime$mean * 100, 1)
  ),
  sprintf(
    "\\newcommand{\\trueMeanVline}{%s}",
    round(dat$true_regime$mean * 100, 1)
  )
)
