...
# Export individual graphic panels as SVG (for presentations)
# Uses dvisvgm --eps on the .ps intermediates (kept after cleanup)
cat("\n--- Step 5b: Export individual panel SVGs ---\n")
if (Sys.which("dvisvgm") != "") {
  panel_svgs <- paste0("step3_panel_", c("A", "B1", "B2", "C"), "_graphic")
  for (stem in panel_svgs) {
    ps_file <- file.path(pstricks_dir, paste0(stem, ".ps"))
    svg_file <- file.path(output_dir, paste0(stem, ".svg"))
    if (file.exists(ps_file)) {
      system2(
        "dvisvgm",
        c("--eps", "--no-fonts", paste0("--output=", svg_file), ps_file),
        stdout = FALSE,
        stderr = FALSE
      )
      if (file.exists(svg_file)) cat("  SVG:", svg_file, "\n")
    } else {
      cat("  SKIP:", stem, "(no .ps intermediate found)\n")
    }
  }
} else {
  cat("  Panel SVGs: skipped (dvisvgm not found)\n")
}
