...
write_dat(round(p_grid * 100, 4), round(d_bridge, 6), "panel_A_density_bridge.dat")
...
write_dat(round(true_means_conditions, 2), round(bridge_estimates, 2),
...
write_dat(round(bridge_error_density$x, 4), round(bridge_error_density$y, 6),
...
write_dat(round(indep_u, 4), round(indep_v, 4), "panel_C_independent_sample.dat")
...
N_grid <- c(seq(50, 500, by = 25), seq(600, 1000, by = 50),
            seq(1200, 2000, by = 200), seq(2500, 5000, by = 500))
...
for (i in seq_along(subgroup_estimates)) {
  panel_e_lines <- c(panel_e_lines,
    paste(i, subgroup_estimates[i], subgroup_truth[i],
...
metrics <- c(
  ...
  sprintf("\\providecommand{\\samplingHWpaired}{%.1f}",     3.92 * k_paired / sqrt(500) * (1 + 2.5/sqrt(500)) / 2),
  sprintf("\\providecommand{\\samplingHWindep}{%.1f}",      round(sampling_hw_indep, 1)),
  sprintf("\\providecommand{\\ciWidthPairedFiveH}{%.1f}",   ci_width_paired[which(N_grid == 500)]),
  sprintf("\\providecommand{\\ciWidthIndepFiveH}{%.1f}",    ci_width_indep[which(N_grid == 500)])
)
...
axes <- c(
  ...
  sprintf("\\providecommand{\\panelBymax}{%s}",    ceiling(y_max_b_density * 10) / 10),
...
for (i in seq_along(true_means_conditions)) {
  ...
  marker_lines <- c(marker_lines,
    sprintf("\\psline[linecolor=%s,linewidth=0.8pt]{->}(%.2f,%d)(%.2f,%d)%%",
            col, tm, y_pos, be, y_pos),
