...
plot_copula_comparison_panel <- function(primary_est,
                                         alt_est,
                                         true_sgpc = NULL,
  ...
  pA <- ggplot(cdf_primary, aes(x = v, y = cdf, color = source, linetype = source)) +
    geom_line(linewidth = 0.8) +
    scale_color_manual(values = c("Observed" = STEP3_COLORS$observed,
                                  "Predicted" = STEP3_COLORS$predicted)) +
    scale_linetype_manual(values = c("Observed" = "solid", "Predicted" = "dashed")) +
    labs(title = primary_label,
         subtitle = sprintf("W1=%.4f  CvM=%.6f",
                            primary_est$all_distances$wasserstein1,
                            primary_est$all_distances$cramer_von_mises),
         x = "v (current percentile)", y = "CDF") +
    theme_publication() +
  ...
  pB <- ggplot(cdf_alt, aes(x = v, y = cdf, color = source, linetype = source)) +
    geom_line(linewidth = 0.8) +
    scale_color_manual(values = c("Observed" = STEP3_COLORS$observed,
                                  "Predicted" = STEP3_COLORS$predicted)) +
    scale_linetype_manual(values = c("Observed" = "solid", "Predicted" = "dashed")) +
    labs(title = alt_label,
         subtitle = sprintf("W1=%.4f  CvM=%.6f",
  ...
  pC <- ggplot(df_regime, aes(x = sgpc, y = density, color = source, linetype = source)) +
    geom_line(linewidth = 0.9)

  # Add true SGPc kernel density if available
  if (!is.null(true_sgpc) && length(true_sgpc) > 10) {
    kd <- density(true_sgpc / 100, from = 0.01, to = 0.99, bw = "SJ", n = 200)
    df_actual <- data.frame(sgpc = kd$x * 100, density = kd$y, source = "Actual (truth)")
    pC <- pC +
      geom_line(data = df_actual, aes(x = sgpc, y = density),
  ...
  pC <- pC +
    ...
    labs(title = "Inferred Regime Densities",
         x = "SGPc", y = "Density") +
    theme_publication() +
    theme(legend.position = "bottom", legend.title = element_blank(),
          plot.title = element_text(size = 10))
  ...
  delta_txt <- if (!is.null(sensitivity)) {
    paste0(
      "Copula sensitivity delta (", alt_label, " \u2212 ", primary_label, "):\n",
      sprintf("  \u0394 Median SGPc: %+.2f SGP points\n", sensitivity$delta_median_sgpc),
      sprintf("  \u0394 Mean SGPc:   %+.2f SGP points\n", sensitivity$delta_mean_sgpc),
      sprintf("\nPrimary: median=%.1f  mean=%.1f  (diff from truth: %.1f / %.1f)\n",
              sensitivity$primary_median_sgpc, sensitivity$primary_mean_sgpc,
              sensitivity$primary_median_diff, sensitivity$primary_mean_diff),
      sprintf("Alt:     median=%.1f  mean=%.1f  (diff from truth: %.1f / %.1f)",
              sensitivity$alt_median_sgpc, sensitivity$alt_mean_sgpc,
              sensitivity$alt_median_diff, sensitivity$alt_mean_diff)
    )
  } else {
    "No copula sensitivity data available."
  }
  pD <- ggplot() +
    annotate("text", x = 0.5, y = 0.5, label = delta_txt,
             hjust = 0.5, vjust = 0.5, size = 3.2, family = "mono") +
    xlim(0, 1) + ylim(0, 1) +
...
plot_linkage_fraction_curve <- function(precision_dt,
                                        n_bucket_focus = NULL,
  ...
  if (nrow(dt) < 2) {
    cat("  plot_linkage_fraction_curve: fewer than 2 linkage fractions for N=",
        n_bucket_focus, ". Skipping.\n")
    return(invisible(NULL))
  }
  ...
  curve_dt <- dt[, .(
    ...
    n_pools   = .N
  ), by = linkage_fraction][order(linkage_fraction)]

  # Compute linkage premium: CI relative to fully paired (lf=1.0)
  paired_row <- curve_dt[linkage_fraction == 1.0]
  if (nrow(paired_row) > 0 &&
      is.finite(paired_row$median_ci) && paired_row$median_ci > 0 &&
      is.finite(paired_row$mean_ci)   && paired_row$mean_ci   > 0) {
  ...
  ci_long <- data.table::melt(
    curve_dt, id.vars = "linkage_fraction",
    measure.vars = c("median_ci", "mean_ci"),
    variable.name = "measure", value.name = "ci_width"
  )
  ci_long[, measure_label := fifelse(measure == "median_ci", "Median SGPc", "Mean SGPc")]

  colors <- if (exists("STEP3_COLORS")) {
    c("Median SGPc" = STEP3_COLORS$predicted,
      "Mean SGPc"   = STEP3_COLORS$true_value)
  } else {
    c("Median SGPc" = "#3B9AB2", "Mean SGPc" = "#E1AF00")
  }

  p <- ggplot(ci_long, aes(x = linkage_fraction, y = ci_width,
    ...
    labs(
      title = paste0("Linkage Fraction Curve (N = ",
  ...
  if ("median_premium" %in% names(curve_dt)) {
    ...
    if (length(max_premium) > 0 && is.finite(max_premium)) {
      p <- p + annotate("text",
        x = 0.0, y = curve_dt[linkage_fraction == 0.0, median_ci] * 1.05,
  ...
  save_plot_multi(p, filename, output_dir,
...
#'
#' @export
plot_churn_decomposition <- function(churn_bk, condition_label = NULL,
                                      filename = "phasea_07a_churn_decomposition",
                                      output_dir = ".") {

  cb <- churn_bk$condition
  if (is.null(cb) || nrow(cb) == 0) return(invisible(NULL))
  ...
  fill_colors <- c("Stayers" = "#2C7BB6", "Leavers" = "#D7191C", "Entrants" = "#FDAE61")

  title_text <- "Churn Decomposition: Stayers / Leavers / Entrants"
  if (!is.null(condition_label)) title_text <- paste0(title_text, "\n", condition_label)

  sub_text <- sprintf(
    paste0("n_S = %s  |  n_L = %s  |  n_E = %s  |  ",
           "\u03b1 = %.3f  |  \u03b2 = %.3f  |  Type: %s"),
    format(cb$n_stayers, big.mark = ","),
    format(cb$n_leavers, big.mark = ","),
    format(cb$n_entrants, big.mark = ","),
    cb$alpha, cb$beta, cb$churn_type
  )
  ...
  p <- ggplot(bar_dt, aes(x = wave, y = n, fill = group)) +
    ...
    labs(title = title_text, subtitle = sub_text, x = NULL, y = "Number of Students") +
    theme_publication() +
    theme(legend.position = "bottom")
...
#'
#' @export
plot_marginal_comparison <- function(marginal_comparison, condition_label = NULL,
  ...
  prior_dt <- data.table::rbindlist(list(
    data.table::data.table(score = mc$prior_scores_all,    population = "All Students"),
    data.table::data.table(score = mc$prior_scores_stayer, population = "Stayers Only")
  ))

  # Current wave densities
  current_dt <- data.table::rbindlist(list(
    data.table::data.table(score = mc$current_scores_all,    population = "All Students"),
    data.table::data.table(score = mc$current_scores_stayer, population = "Stayers Only")
  ))

  pop_colors <- c("All Students" = "#D7191C", "Stayers Only" = "#2C7BB6")

  p_prior <- ggplot(prior_dt, aes(x = score, fill = population, color = population)) +
  ...
  p_current <- ggplot(current_dt, aes(x = score, fill = population, color = population)) +
    geom_density(alpha = 0.35, linewidth = 0.7) +
    scale_fill_manual(values = pop_colors) +
    scale_color_manual(values = pop_colors) +
    labs(
  ...
  ignorable_label <- if (isTRUE(mc$compositionally_ignorable)) {
    ...
    sprintf("Compositional drift detected (asymmetry ratio = %.1f)", mc$asymmetry_ratio %||% NA_real_)
  }
  ...
  save_plot_multi(combined, filename, output_dir, width = PLOT_WIDTH + 2, height = PLOT_HEIGHT)
  invisible(combined)
}
...
#'
#' @export
plot_regime_contrast <- function(regime_contrast, stayer_estimate,
                                  true_sgpc = NULL,
                                  condition_label = NULL,
  ...
  rc <- regime_contrast
  if (is.null(rc) || is.null(rc$regime_all)) return(invisible(NULL))
  ...
  stayer_density <- dbeta(x_grid, stayer_regime$params$alpha, stayer_regime$params$beta)
  ...
  density_dt <- data.table::rbindlist(list(
    data.table::data.table(sgpc = x_grid * 100, density = stayer_density, source = "Stayer Regime"),
    data.table::data.table(sgpc = x_grid * 100, density = all_density,   source = "All-Student Regime")
  ))

  source_colors <- c("Stayer Regime" = "#2C7BB6", "All-Student Regime" = "#D7191C")
  ...
  sub_text <- sprintf(
    ...
    rc$delta_median, rc$delta_mean
  )

  p <- ggplot(density_dt, aes(x = sgpc, y = density, color = source, fill = source)) +
  ...
  if (!is.null(true_sgpc) && length(true_sgpc) > 10) {
    ...
    p <- p + geom_line(data = true_dt, aes(x = sgpc, y = density),
                       inherit.aes = FALSE, color = "grey40",
                       linetype = "dashed", linewidth = 0.8)
  }

  p <- p +
    labs(title = title_text, subtitle = sub_text,
         x = "SGPc", y = "Density") +
    theme_publication() +
    theme(legend.position = "bottom", legend.title = element_blank()) +
    coord_cartesian(xlim = c(0, 100))
...
#'
#' @export
plot_churn_summary_panel <- function(churn_bk,
                                      marginal_comparison,
                                      regime_contrast,
  ...
  p1 <- ggplot(bar_dt, aes(x = wave, y = n, fill = group)) +
    ...
    scale_fill_manual(values = fill_colors, name = "") +
    labs(title = sprintf("A. Churn (\u03b1=%.2f, \u03b2=%.2f, %s)",
                         cb$alpha, cb$beta, cb$churn_type),
         x = NULL, y = "n") +
  ...
  if (!is.null(mc)) {
    ...
    p2 <- ggplot(current_dt, aes(x = score, fill = pop, color = pop)) +
      ...
      scale_fill_manual(values = pop_cols) + scale_color_manual(values = pop_cols) +
      labs(title = sprintf("B. Current Marginals (\u0393_V=%.1f)", mc$gamma_current),
           x = "Score", y = "Density") +
      theme_publication() + theme(legend.position = "bottom",
                                   legend.title = element_blank(),
                                   plot.title = element_text(size = 10))
  ...
  if (!is.null(regime_contrast) && !is.null(regime_contrast$regime_all) &&
    ...
    dens_dt <- data.table::rbindlist(list(
      data.table::data.table(sgpc = x_grid * 100,
                             d = dbeta(x_grid, sr$params$alpha, sr$params$beta), src = "Stayer"),
      data.table::data.table(sgpc = x_grid * 100,
                             d = dbeta(x_grid, ar$params$alpha, ar$params$beta), src = "All")
    ))
    src_cols <- c("Stayer" = "#2C7BB6", "All" = "#D7191C")
    p3 <- ggplot(dens_dt, aes(x = sgpc, y = d, color = src)) +
      geom_line(linewidth = 0.8) +
      scale_color_manual(values = src_cols) +
      labs(title = sprintf("C. Regime Contrast (\u0394 Median=%+.1f)",
  ...
  if (!is.null(theoretical_premium) && !is.null(empirical_premium)) {
    ...
    if (nrow(prem_dt) > 0) {
      p4 <- ggplot(prem_dt, aes(x = type, y = value)) +
        ...
        geom_text(aes(label = sprintf("%.2f", value)), vjust = -0.3, size = 3.2) +
        labs(title = "D. Linkage Premium (empirical vs theoretical)",
             x = NULL, y = "SE Multiplier") +
        theme_publication() + theme(plot.title = element_text(size = 10))
  ...
  if (!is.null(condition_label)) suptitle <- paste0(suptitle, ": ", condition_label)
  ...
  save_plot_multi(combined, filename, output_dir,
                  width = PLOT_WIDTH + 2, height = PLOT_HEIGHT + 3)
  invisible(combined)
}
