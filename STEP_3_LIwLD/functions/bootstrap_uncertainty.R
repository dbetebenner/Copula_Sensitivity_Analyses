...
bootstrap_regime <- function(
  ...
  if ((is_paired || is_partial) && n_u != n_v) {
    stop(
      "linkage_fraction > 0 requires length(u_sample) == length(v_sample) ",
      "(got ",
      n_u,
      " vs ",
      n_v,
  ...
  run_one_replicate <- function(b) {
    ...
    if (is_paired) {
      ...
      warning(
        "replicate_weights is not implemented in STEP 3 yet; falling back to srs_bootstrap"
      )
  ...
  if (isTRUE(use_mirai) && n_boot > 1L) {
    ...
    if (mirai_ok) {
      ...
      cat(
        "  mirai bootstrap requested but no daemons detected; falling back to sequential.\n"
      )
      cat("  Diagnostic: ", diag, "\n")
    }
  }
  ...
  if (mirai_ok) {
    if (verbose) {
      cat("  Running bootstrap via mirai (", n_daemons, " daemons)\n", sep = "")
    }
    ...
    if (isTRUE(push_ok)) {
      ...
      boot_lambda <- function(task) {
        ...
        if (.BOOT_PAIRING) {
          ...
          u_linked <- .BOOT_U_SAMPLE[linked_idx]
          v_linked <- .BOOT_V_SAMPLE[linked_idx]
          uw_linked <- .BOOT_U_WEIGHTS[linked_idx]
          vw_linked <- .BOOT_V_WEIGHTS[linked_idx]
          ...
          u_boot <- .BOOT_U_SAMPLE[u_idx]
          v_boot <- .BOOT_V_SAMPLE[v_idx]
          uw_boot <- .BOOT_U_WEIGHTS[u_idx]
          vw_boot <- .BOOT_V_WEIGHTS[v_idx]
        } else {
          u_idx <- sample.int(n_u, n_u, replace = TRUE)
          v_idx <- sample.int(n_v, n_v, replace = TRUE)
          u_boot <- .BOOT_U_SAMPLE[u_idx]
      ...
      for (b in seq_along(draw_list)) {
        if (
          inherits(draw_list[[b]], "miraiError") ||
            inherits(draw_list[[b]], "errorValue")
        ) {
          draw_list[[b]] <- list(
            regime_param_hat = rep(NA_real_, 2),
            median_sgpc = NA_real_,
  ...
  if (verbose) {
    ...
    if (sum(valid) > 2) {
      cat(
        "  Median SGPc: ",
        round(median(median_sgpc[valid]), 1),
        " [",
  ...
  list(
    ...
    ci_median_sgpc = if (sum(valid) > 2) {
      quantile(median_sgpc[valid], c(0.025, 0.975))
    } else {
      c(NA, NA)
    },
    ci_mean_sgpc = if (sum(valid) > 2) {
      quantile(mean_sgpc[valid], c(0.025, 0.975))
    } else {
      c(NA, NA)
    },
...
copula_uncertainty <- function(
  ...
  if (verbose) {
    cat("Copula parameter uncertainty:", n_draws, "draws\n")
  }
  ...
  if (verbose && sum(valid) > 2) {
    cat(
      "  Median SGPc range due to copula uncertainty: [",
...
cat(
  "  Functions: bootstrap_regime, copula_uncertainty, generate_copula_draws\n"
)
