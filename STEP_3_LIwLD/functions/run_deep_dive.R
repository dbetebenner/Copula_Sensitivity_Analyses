...
run_deep_dive <- function(
  ...
  output_dir <- output_dir %||%
    {
      if (exists("RESULTS_DIR", inherits = TRUE)) {
        get("RESULTS_DIR", inherits = TRUE)
      } else {
        file.path(step3_root, "results")
      }
    }
  ...
  if (is.null(dataset_id) || !nzchar(dataset_id)) {
    stop(
      "run_deep_dive(): dataset_id is NULL/empty and config$validation$dataset_id is not set."
    )
  }
  ...
  if (!exists("DATASETS", inherits = TRUE)) {
    stop(
      "run_deep_dive(): DATASETS object not found. Source dataset_configs.R first."
    )
  }
  ...
  if (!file.exists(data_path)) {
    stop(
      "Data file not found for dataset '",
      dataset_id,
      "'. Checked:\n  ",
  ...
  if (exists(state_data_name)) {
    ...
    dt_names <- ls()[sapply(ls(), function(x) {
      data.table::is.data.table(get(x))
    })]
    if (length(dt_names) == 0) {
      stop("No data.table found after loading ", data_path)
    }
  ...
  if (is.null(condition_id)) {
    ...
    if (length(conditions) == 0) {
      stop("No Phase 1 conditions found for ", dataset_id)
    }
    ...
    if (length(one_year_preferred) > 0) {
      ...
      log_msg(
        "Auto-selected 1-year preferred content condition: ",
        condition_id,
        "\n"
      )
      ...
      log_msg(
        "Preferred content not found in 1-year spans; using first 1-year condition: ",
        condition_id,
        "\n"
      )
    } else {
      ...
      log_msg(
        "No 1-year conditions available; using first available condition: ",
        condition_id,
        "\n"
      )
    }
  ...
  if (is.null(sg_col) || !sg_col %in% names(pairs)) {
    log_msg(
      "  WARNING: Column '",
      sg_col %||% "NULL",
      "' not found. Trying SCHOOL_NUMBER.\n"
    )
  ...
  if (exists("compute_churn_bookkeeping", mode = "function")) {
    ...
    churn_bk <- tryCatch(
      ...
      error = function(e) {
        log_msg("  WARNING: Churn bookkeeping failed: ", e$message, "\n")
        NULL
      }
    )
    if (!is.null(churn_bk)) {
    ...
    marginal_comp <- tryCatch(
      ...
      error = function(e) {
        log_msg("  WARNING: Marginal comparison failed: ", e$message, "\n")
        NULL
      }
    )
    if (!is.null(marginal_comp)) {
      ...
      log_msg(
        ...
        marginal_comp$compositionally_ignorable,
        "\n"
      )
      if (is.finite(marginal_comp$asymmetry_ratio)) {
        log_msg(
          "  Asymmetry ratio (Gamma_V / Gamma_U) = ",
          marginal_comp$asymmetry_ratio,
          "\n"
        )
      }
      # Upgrade churn classification if compositional drift detected
      if (
        !is.null(churn_bk) && !isTRUE(marginal_comp$compositionally_ignorable)
      ) {
        churn_bk$condition$churn_type <- paste0(
          churn_bk$condition$churn_type,
          "+compositional"
        )
  ...
  if (identical(copula_mode, "canonical_only")) {
    ...
    if (!is.null(p1) && !is.null(p1$best_fit_copula)) {
      ...
      if (identical(canon_class, alt_fam)) {
        canon_params <- tryCatch(
          copula::getTheta(p1_copula, freeOnly = FALSE, named = TRUE),
          error = function(e) NULL
        )
        alt_params <- tryCatch(
          copula::getTheta(alt_copula, freeOnly = FALSE, named = TRUE),
          error = function(e) NULL
        )
        if (
          !is.null(canon_params) &&
            !is.null(alt_params) &&
            length(canon_params) == length(alt_params) &&
            all(abs(canon_params - alt_params) < 1e-4)
        ) {
          log_msg(
            "  NOTE: Canonical and best-fit copulas are identical — comparison will show zero delta.\n"
          )
        }
      }
    } else {
      log_msg(
        "  WARNING: No Phase 1 best-fit copula found. Comparison mode degrades to canonical_only.\n"
      )
  ...
  if (subgroup_id == "ALL") {
    ...
    sg_idx_a <- which(
      as.character(pairs[[sg_col]]) == as.character(subgroup_id)
    )
  ...
  if (identical(truth_source, "step2_empirical")) {
    step2_dir_raw <- cfg$systematic$step2_results_dir %||%
      ...
      if (nrow(s2_cond) > 0 && "sgpc_emp" %in% names(s2_cond)) {
        aligned <- merge(
          data.table::data.table(
            ID = pairs$ID[sg_idx_a],
            row_idx = seq_along(sg_idx_a)
          ),
          s2_cond[, .(ID, sgpc_emp)],
          by = "ID",
          all.x = TRUE,
        ...
        if (pct_matched >= 0.95) {
          ...
          log_msg(sprintf(
            "  STEP 2 truth loaded: %d matched (%.1f%%)\n",
            sum(!is.na(aligned$sgpc_emp)),
            pct_matched * 100
          ))
        }
      }
  ...
  log_msg(
    "  True SGPc for full condition pool: n = ",
    length(true_sgpc_full),
    ", mean = ",
    round(mean(true_sgpc_full, na.rm = TRUE), 1),
    "\n\n"
  )
  ...
  u_cut <- cut(
    ...
    breaks = unique(as.numeric(quantile(
      u_sg,
      probs = seq(0, 1, length.out = u_bins + 1),
      na.rm = TRUE
    ))),
    include.lowest = TRUE
  )
  ...
  flag_independence_violation <- ((is.finite(spearman_rho) &&
    abs(spearman_rho) > assump_cfg$max_abs_spearman) ||
  ...
  diag_summary <- data.table::data.table(
    ...
    value = c(
      ...
      kw_p,
      u_bins,
      as.numeric(flag_independence_violation)
    )
  )
  ...
  log_msg(
    "  Note: These are independent samples — no student-level linkage.\n\n"
  )
  ...
  log_msg(
    "  Kernel cache built: ",
    kernel_cache$copula_family,
    " (",
    paste(
      names(kernel_cache$copula_params),
      "=",
      round(unlist(kernel_cache$copula_params), 3),
      collapse = ", "
    ),
    ")\n"
  )
  log_msg(
    "  Grid: ",
    cfg$kernel$u_grid_size,
    "x",
    cfg$kernel$v_grid_size,
    "\n\n"
  )
  ...
  if (is.null(optimize_metric)) {
    optimize_metric <- cfg$distance$primary
  }
  log_msg(
    "A.6  Estimating growth regime (optimize=",
    optimize_metric,
    ", copula=",
  ...
  log_msg(
    "  Estimated median SGPc: ",
    round(best_est$regime$median * 100, 1),
    "\n"
  )
  log_msg(
    "  True median SGPc:      ",
    round(median(true_sgpc, na.rm = TRUE), 1),
    "\n"
  )
  log_msg(
    "  Difference:            ",
  ...
  log_msg(
    "  Estimated mean SGPc:   ",
    round(best_est$regime$mean * 100, 1),
    "\n"
  )
  log_msg(
    "  True mean SGPc:        ",
    round(mean(true_sgpc, na.rm = TRUE), 1),
    "\n"
  )
  log_msg(
    "  Difference:            ",
  ...
  if (!is.null(best_est$alt_metrics) && !is.null(best_est$alt_metrics$cvm)) {
    ...
    log_msg(
      "  CvM median SGPc:  ",
      round(best_est_cvm$regime$median * 100, 1),
      "\n"
    )
    log_msg(
      "  CvM mean SGPc:    ",
      round(best_est_cvm$regime$mean * 100, 1),
      "\n"
    )
    median_delta <- abs(best_est$regime$median - best_est_cvm$regime$median) *
      100
    mean_delta <- abs(best_est$regime$mean - best_est_cvm$regime$mean) * 100
    log_msg(
      "  W1 vs CvM delta:  |median|=",
    ...
    !is.null(best_est$alt_metrics) &&
      !is.null(best_est$alt_metrics$wasserstein1)
  ) {
    ...
    log_msg(
      "  W1 median SGPc:   ",
      round(best_est_w1_alt$regime$median * 100, 1),
      "\n"
    )
    log_msg(
      "  W1 mean SGPc:     ",
      round(best_est_w1_alt$regime$mean * 100, 1),
      "\n\n"
    )
  }
  ...
  if (identical(copula_mode, "comparison") && !is.null(alt_copula)) {
    log_msg(
      "A.6b Estimating growth regime under alternative copula (",
      alt_copula_label,
      ")...\n\n"
    )
    a6b_start <- Sys.time()
    ...
    log_msg(
      "  Alt kernel cache built: ",
      alt_kernel_cache$copula_family,
      " (",
      paste(
        names(alt_kernel_cache$copula_params),
        "=",
        round(unlist(alt_kernel_cache$copula_params), 3),
        collapse = ", "
      ),
      ")\n"
    )
    ...
    if (
      !is.null(alt_best_est$alt_metrics) &&
        !is.null(alt_best_est$alt_metrics$cvm)
    ) {
      alt_best_est_cvm <- alt_best_est$alt_metrics$cvm
      log_msg(
        "  Alt copula CvM-optimised: median=",
        round(alt_best_est_cvm$regime$median * 100, 1),
        "  mean=",
        round(alt_best_est_cvm$regime$mean * 100, 1),
        "\n"
      )
    }
    ...
    copula_sensitivity <- list(
      ...
      delta_median_sgpc = (alt_best_est$regime$median -
        best_est$regime$median) *
        100,
    ...
    log_msg(
      "    Median SGPc: ",
      round(copula_sensitivity$primary_median_sgpc, 1),
      "  (diff from truth: ",
      round(copula_sensitivity$primary_median_diff, 1),
      ")\n"
    )
    log_msg(
      "    Mean SGPc:   ",
      round(copula_sensitivity$primary_mean_sgpc, 1),
      "  (diff from truth: ",
      round(copula_sensitivity$primary_mean_diff, 1),
      ")\n"
    )
    log_msg(
      "    W1: ",
      round(copula_sensitivity$primary_w1, 6),
      "  CvM: ",
      round(copula_sensitivity$primary_cvm, 6),
      "\n"
    )
    log_msg("  Alternative (", alt_copula_label, "):\n")
    log_msg(
      "    Median SGPc: ",
      round(copula_sensitivity$alt_median_sgpc, 1),
      "  (diff from truth: ",
      round(copula_sensitivity$alt_median_diff, 1),
      ")\n"
    )
    log_msg(
      "    Mean SGPc:   ",
      round(copula_sensitivity$alt_mean_sgpc, 1),
      "  (diff from truth: ",
      round(copula_sensitivity$alt_mean_diff, 1),
      ")\n"
    )
    log_msg(
      "    W1: ",
      round(copula_sensitivity$alt_w1, 6),
      "  CvM: ",
      round(copula_sensitivity$alt_cvm, 6),
      "\n"
    )
    log_msg("  Delta (alt - primary):\n")
    log_msg(
      "    |Median|: ",
      round(abs(copula_sensitivity$delta_median_sgpc), 2),
      " SGP points\n"
    )
    log_msg(
      "    |Mean|:   ",
      round(abs(copula_sensitivity$delta_mean_sgpc), 2),
      " SGP points\n"
    )
  ...
  log_msg(
    "  Bootstrap median SGPc 95% CI (independent): ",
    paste0(
      "[",
      round(boot_results$ci_median_sgpc[1], 1),
      ", ",
  ...
  log_msg(
    "  Bootstrap mean SGPc 95% CI (independent):   ",
    paste0(
      "[",
      round(boot_results$ci_mean_sgpc[1], 1),
      ", ",
  ...
  log_msg(
    "  Bootstrap median SGPc 95% CI (paired):      ",
    paste0(
      "[",
      round(boot_paired$ci_median_sgpc[1], 1),
      ", ",
  ...
  log_msg(
    "  Bootstrap mean SGPc 95% CI (paired):        ",
    paste0(
      "[",
      round(boot_paired$ci_mean_sgpc[1], 1),
      ", ",
  ...
  phase_a_linkage_premium <- list(
    ...
    median = list(
      ...
      ci_ratio = if (is.finite(ci_w_paired_median) && ci_w_paired_median > 0) {
        round(ci_w_indep_median / ci_w_paired_median, 2)
      } else {
        NA_real_
      },
      se_ratio = if (
        is.finite(boot_paired$se_median_sgpc) && boot_paired$se_median_sgpc > 0
      ) {
        round(boot_results$se_median_sgpc / boot_paired$se_median_sgpc, 2)
      } else {
        NA_real_
      }
    ),
    mean = list(
      ...
      ci_ratio = if (is.finite(ci_w_paired_mean) && ci_w_paired_mean > 0) {
        round(ci_w_indep_mean / ci_w_paired_mean, 2)
      } else {
        NA_real_
      },
      se_ratio = if (
        is.finite(boot_paired$se_mean_sgpc) && boot_paired$se_mean_sgpc > 0
      ) {
        round(boot_results$se_mean_sgpc / boot_paired$se_mean_sgpc, 2)
      } else {
        NA_real_
      }
    ),
    description = paste0(
      "Linkage premium at N=",
      nrow(pairs_sg),
      ": independent-bootstrap CI width / paired-bootstrap CI width."
    )
  )
  ...
  if (isTRUE(cfg$validation$precision_sweep)) {
    log_msg(
      "A.9  Precision sweep from condition pool (subsampling w/o replacement)...\n\n"
    )
    if (!exists("run_precision_sweep", mode = "function")) {
      log_msg("  WARNING: run_precision_sweep() not loaded. Skipping A.9.\n\n")
    } else {
      a9_start <- Sys.time()
      sweep_n_buckets <- cfg$validation$sweep_n_buckets %||%
        cfg$systematic$n_buckets
      ...
      log_msg(
        "  Pool N = ",
        precision_sweep_results$n_pool,
        ", replicates = ",
        nrow(precision_sweep_results$replicates),
        "\n\n"
      )

      # Write CSV outputs
      data.table::fwrite(
        precision_sweep_results$replicates,
        file.path(output_dir, "phase_a_precision_sweep.csv")
      )
      data.table::fwrite(
        precision_sweep_results$summary,
        file.path(output_dir, "phase_a_precision_sweep_summary.csv")
      )
      log_msg(
        "  Saved: phase_a_precision_sweep.csv, phase_a_precision_sweep_summary.csv\n"
      )
      ...
      tryCatch(
        {
          plot_precision_sweep(
            ...
            output_dir = viz_dir,
            filename = phasea_fig$precision_sweep %||%
              "phasea_08_precision_sweep"
          )
  ...
  if (
    exists("estimate_regime_all_students", mode = "function") &&
      !is.null(churn_bk) &&
      churn_bk$condition$n_leavers + churn_bk$condition$n_entrants > 0
  ) {
    ...
    regime_contrast <- tryCatch(
      ...
      error = function(e) {
        log_msg("  WARNING: Regime contrast failed: ", e$message, "\n")
        NULL
      }
    )
    if (!is.null(regime_contrast)) {
      log_msg(
        ...
        regime_contrast$median_sgpc_all,
        ", mean = ",
        regime_contrast$mean_sgpc_all,
        "\n"
      )
      log_msg(
        "  Stayer regime:      median = ",
        regime_contrast$median_sgpc_stayer,
        ", mean = ",
        regime_contrast$mean_sgpc_stayer,
        "\n"
      )
      log_msg(
        "  Delta median = ",
  ...
  if (
    exists("theoretical_linkage_premium", mode = "function") &&
      !is.null(churn_bk)
  ) {
    log_msg("A.7d Theoretical partial-linkage premium...\n")
    cop_rho <- tryCatch(as.numeric(p1_copula@param[1]), error = function(e) {
      NA_real_
    })
    ...
    log_msg(
      "  alpha = ",
      theoretical_prem$alpha,
      ", rho = ",
      theoretical_prem$rho,
      "\n"
    )
    log_msg(
      "  Theoretical mean-scale premium = ",
      theoretical_prem$mean_scale,
      "\n"
    )
    log_msg(
      "  Theoretical CDF-scale premium = ",
      theoretical_prem$cdf_scale,
      "\n"
    )
    log_msg(
      "  Empirical bootstrap premium (median CI ratio) = ",
  ...
  best_est$w1_uniform <- wasserstein1(
    best_est$F_uniform,
    best_est$F_obs,
    best_est$v_grid
  )
  ...
  if (!is.null(copula_sensitivity) && !is.null(alt_best_est)) {
    ...
    alt_best_est$F_uniform <- predict_marginal_cdf(
      v_grid = alt_best_est$v_grid,
      u_sample = u_cross,
      regime = regime_beta(0.5, 2),
      kernel_cache = alt_kernel_cache
    )
    alt_best_est$F_tamp <- ecdf(u_cross)(alt_best_est$v_grid)
    alt_best_est$w1_uniform <- wasserstein1(
      alt_best_est$F_uniform,
      alt_best_est$F_obs,
      alt_best_est$v_grid
    )
    ...
    plot_observed_vs_predicted_cdf(
      ...
      output_dir = viz_dir,
      filename = phasea_fig$grid_w1_canonical_cdf
    )
    ...
    plot_regime_shape(
      ...
      output_dir = viz_dir,
      filename = phasea_fig$grid_w1_canonical_regime
    )
    ...
    plot_observed_vs_predicted_cdf(
      ...
      output_dir = viz_dir,
      filename = phasea_fig$grid_w1_bestfit_cdf
    )
    ...
    plot_regime_shape(
      ...
      output_dir = viz_dir,
      filename = phasea_fig$grid_w1_bestfit_regime
    )
    ...
    if (!is.null(best_est_cvm)) {
      ...
      plot_observed_vs_predicted_cdf(
        ...
        output_dir = viz_dir,
        filename = phasea_fig$grid_cvm_canonical_cdf
      )
      plot_regime_shape(
        best_est_cvm$regime,
        true_sgpc,
        ...
        output_dir = viz_dir,
        filename = phasea_fig$grid_cvm_canonical_regime
      )
    }
    ...
    if (!is.null(alt_best_est_cvm)) {
      ...
      plot_observed_vs_predicted_cdf(
        ...
        output_dir = viz_dir,
        filename = phasea_fig$grid_cvm_bestfit_cdf
      )
      plot_regime_shape(
        alt_best_est_cvm$regime,
        true_sgpc,
        title = paste0("Regime: CvM-Optimised — ", alt_subtitle),
        output_dir = viz_dir,
        filename = phasea_fig$grid_cvm_bestfit_regime
      )
    }
    ...
    plot_observed_vs_predicted_cdf(
      ...
      output_dir = viz_dir,
      filename = phasea_fig$copula_alt_forward_cdf
    )
    plot_regime_shape(
      alt_best_est$regime,
      true_sgpc,
      title = paste0(
        "C. Inferred Regime Density — ",
        sg_label,
        "\n",
        alt_subtitle
      ),
      output_dir = viz_dir,
      filename = phasea_fig$copula_alt_regime_density
    )
    plot_recovery_summary(
      alt_best_est,
      true_sgpc,
      ...
      output_dir = viz_dir,
      filename = phasea_fig$copula_alt_recovery_summary
    )
    ...
    if (exists("generate_metric_copula_grid_latex", mode = "function")) {
      ...
      tryCatch(
        ...
        error = function(e) {
          log_msg(
            "  WARNING: LaTeX summary grid generation failed: ",
            e$message,
            "\n"
          )
        }
  ...
  phase_a_results <- list(
    ...
    copula_used = list(
      family = kernel_cache$copula_family,
  ...
  w1_red <- if (is.finite(best_est$w1_uniform) && best_est$w1_uniform > 0) {
    round(
      100 * (1 - (best_est$all_distances$wasserstein1 / best_est$w1_uniform)),
      2
    )
  } else {
    NA_real_
  ...
  cvm_median_inferred <- if (!is.null(best_est_cvm)) {
    round(best_est_cvm$regime$median * 100, 2)
  } else {
    NA_real_
  }
  cvm_mean_inferred <- if (!is.null(best_est_cvm)) {
    round(best_est_cvm$regime$mean * 100, 2)
  } else {
    NA_real_
  }
  cvm_median_diff <- if (!is.null(best_est_cvm)) {
    round(best_est_cvm$regime$median * 100 - median(true_sgpc, na.rm = TRUE), 2)
  } else {
    NA_real_
  }
  cvm_mean_diff <- if (!is.null(best_est_cvm)) {
    round(best_est_cvm$regime$mean * 100 - mean(true_sgpc, na.rm = TRUE), 2)
  } else {
    NA_real_
  }
  cvm_at_cvm_opt <- if (!is.null(best_est_cvm)) {
    round(best_est_cvm$all_distances$cramer_von_mises, 6)
  } else {
    NA_real_
  }
  w1_at_cvm_opt <- if (!is.null(best_est_cvm)) {
    round(best_est_cvm$all_distances$wasserstein1, 6)
  } else {
    NA_real_
  }
  ...
  cs_alt_median_inferred <- if (!is.null(cs)) {
    round(cs$alt_median_sgpc, 2)
  } else {
    NA_real_
  }
  cs_alt_mean_inferred <- if (!is.null(cs)) {
    round(cs$alt_mean_sgpc, 2)
  } else {
    NA_real_
  }
  cs_alt_median_diff <- if (!is.null(cs)) {
    round(cs$alt_median_diff, 2)
  } else {
    NA_real_
  }
  cs_alt_mean_diff <- if (!is.null(cs)) round(cs$alt_mean_diff, 2) else NA_real_
  cs_delta_median <- if (!is.null(cs)) {
    round(cs$delta_median_sgpc, 2)
  } else {
    NA_real_
  }
  ...
  cs_alt_cvm_median <- if (!is.null(alt_cvm_est_local)) {
    round(alt_cvm_est_local$regime$median * 100, 2)
  } else {
    NA_real_
  }
  cs_alt_cvm_mean <- if (!is.null(alt_cvm_est_local)) {
    round(alt_cvm_est_local$regime$mean * 100, 2)
  } else {
    NA_real_
  }
  cs_alt_cvm_median_diff <- if (!is.null(alt_cvm_est_local)) {
    round(
      alt_cvm_est_local$regime$median * 100 - median(true_sgpc, na.rm = TRUE),
      2
    )
  } else {
    NA_real_
  }
  cs_alt_cvm_mean_diff <- if (!is.null(alt_cvm_est_local)) {
    round(
      alt_cvm_est_local$regime$mean * 100 - mean(true_sgpc, na.rm = TRUE),
      2
    )
  } else {
    NA_real_
  }
  cs_alt_cvm_cvm <- if (!is.null(alt_cvm_est_local)) {
    round(alt_cvm_est_local$all_distances$cramer_von_mises, 6)
  } else {
    NA_real_
  }
  cs_alt_cvm_w1 <- if (!is.null(alt_cvm_est_local)) {
    round(alt_cvm_est_local$all_distances$wasserstein1, 6)
  } else {
    NA_real_
  }
  ...
  summary_row <- data.frame(
    ...
    median_diff = round(
      best_est$regime$median * 100 - median(true_sgpc, na.rm = TRUE),
      2
    ),
    mean_diff = round(
      best_est$regime$mean * 100 - mean(true_sgpc, na.rm = TRUE),
      2
    ),
    w1_uniform = round(best_est$w1_uniform, 6),
    w1_reduction_pct = w1_red,
    max_abs_residual = round(
      max(abs(best_est$F_pred - best_est$F_obs), na.rm = TRUE),
      6
    ),
    mean_abs_residual = round(
      mean(abs(best_est$F_pred - best_est$F_obs), na.rm = TRUE),
      6
    ),
    ...
    flag_independence_violation = flag_independence_violation,
    # Churn diagnostics
    n_prior_all = if (!is.null(churn_bk)) {
      churn_bk$condition$n_prior_all
    } else {
      NA_integer_
    },
    n_current_all = if (!is.null(churn_bk)) {
      churn_bk$condition$n_current_all
    } else {
      NA_integer_
    },
    n_stayers = if (!is.null(churn_bk)) {
      churn_bk$condition$n_stayers
    } else {
      NA_integer_
    },
    n_leavers = if (!is.null(churn_bk)) {
      churn_bk$condition$n_leavers
    } else {
      NA_integer_
    },
    n_entrants = if (!is.null(churn_bk)) {
      churn_bk$condition$n_entrants
    } else {
      NA_integer_
    },
    alpha_retention = if (!is.null(churn_bk)) {
      churn_bk$condition$alpha
    } else {
      NA_real_
    },
    beta_retention = if (!is.null(churn_bk)) {
      churn_bk$condition$beta
    } else {
      NA_real_
    },
    churn_type = if (!is.null(churn_bk)) {
      churn_bk$condition$churn_type
    } else {
      NA_character_
    },
    gamma_prior = if (!is.null(marginal_comp)) {
      marginal_comp$gamma_prior
    } else {
      NA_real_
    },
    gamma_current = if (!is.null(marginal_comp)) {
      marginal_comp$gamma_current
    } else {
      NA_real_
    },
    compositionally_ignorable = if (!is.null(marginal_comp)) {
      marginal_comp$compositionally_ignorable
    } else {
      NA
    },
    regime_delta_median = if (!is.null(regime_contrast)) {
      regime_contrast$delta_median
    } else {
      NA_real_
    },
    regime_delta_mean = if (!is.null(regime_contrast)) {
      regime_contrast$delta_mean
    } else {
      NA_real_
    },
    theoretical_premium_mean = if (!is.null(theoretical_prem)) {
      theoretical_prem$mean_scale
    } else {
      NA_real_
    },
    theoretical_premium_cdf = if (!is.null(theoretical_prem)) {
      theoretical_prem$cdf_scale
    } else {
      NA_real_
    },
  ...
  se_median <- if (!is.null(boot_results$se_median_sgpc)) {
    boot_results$se_median_sgpc
  } else {
    NA_real_
  }
  se_mean <- if (!is.null(boot_results$se_mean_sgpc)) {
    boot_results$se_mean_sgpc
  } else {
    NA_real_
  }
  se_paired_median <- if (!is.null(boot_paired$se_median_sgpc)) {
    boot_paired$se_median_sgpc
  } else {
    NA_real_
  }
  se_paired_mean <- if (!is.null(boot_paired$se_mean_sgpc)) {
    boot_paired$se_mean_sgpc
  } else {
    NA_real_
  }
  ...
  data.table::fwrite(
    precision_anchor,
    file.path(output_dir, "phase_a_precision_anchor.csv")
  )
  ...
  if (!dir.exists(export_phase_a_dir)) {
    dir.create(export_phase_a_dir, recursive = TRUE)
  }
  data.table::fwrite(
...
cat(
  "  Function: run_deep_dive(dataset_id, condition_id, subgroup_id, output_dir, ...)\n"
)
