...
if (!exists("RESULTS_DIR")) {
  if (grepl("STEP_3_LIwLD$", getwd())) {
    ...
    stop(
      "Cannot determine STEP3_ROOT. Run from STEP_3_LIwLD directory or project root."
    )
  }
...
# Phase A exports
kernel_slices_csv <- file.path(
  RESULTS_DIR,
  "exports/phase_a/step3_kernel_slices.csv"
)
quantile_slices_csv <- file.path(
  RESULTS_DIR,
  "exports/phase_a/step3_quantile_slices.csv"
)
...
if (exists("phase_b") && nrow(phase_b) > 0) {
  pb <- copy(phase_b)
  if (!"content_area" %in% names(pb)) {
    pb[, content_area := "Unknown"]
  }
  pb[,
    pool_type := fifelse(grepl("CLUSTER", subgroup_id), "Cluster", "District")
  ]
  ...
  rng_all <- range(
    c(
      pb$median_sgpc_inferred,
      pb$median_sgpc_true,
      pb$mean_sgpc_inferred,
      pb$mean_sgpc_true
    ),
    na.rm = TRUE
  )
  ...
  make_k1_panel <- function(pb, x_col, y_col, diff_col, stat_label, ttl, sub) {
    ...
    n_range <- paste0(
      "N range: ",
      scales::comma(min(pb$n_subgroup, na.rm = TRUE)),
      "\u2013",
    ...
    ggplot(pb, aes(x = .data[[x_col]], y = .data[[y_col]])) +
      ...
      geom_point(
        aes(color = content_area, shape = pool_type, size = n_subgroup),
        alpha = 0.80
      ) +
      # Colour scale
      scale_color_manual(
        values = c(
          "WRITING" = ZISSOU1_BASE[1],
      ...
      labs(
        ...
        y = sprintf(
          "Inferred %s SGPc (from unlinked cross-sections)",
          stat_label
        )
      ) +
      theme_publication() +
  ...
  save_plot_multi(
    p_k1_median,
    "panel_k1_median_validation_scatter",
    enhanced_dir,
    width = 8,
    height = 8
  )
  save_plot_multi(
    p_k1_mean,
    "panel_k1_mean_validation_scatter",
    enhanced_dir,
    width = 8,
    height = 8
  )
  save_plot_multi(
    p_k1_combined,
    "panel_k1_combined_validation_scatter",
    enhanced_dir,
    width = 8,
    height = 15
  )
...
if (
  exists("phase_b_precision") &&
    nrow(phase_b_precision) > 0 &&
    exists("phase_a")
) {
  # Error 2 (bridge cost) from Phase A
  bridge_cost <- abs(
    phase_a$best_estimate$regime$median *
      100 -
  ...
  p_k2 <- ggplot() +
    ...
    geom_col(
      data = df_decomp,
      aes(
        x = factor(format(n_bucket, big.mark = ",")),
        y = value,
        fill = component
      ),
      width = 0.6
    ...
    geom_point(
      data = df_combined,
      aes(x = factor(format(n_bucket, big.mark = ",")), y = rss),
      shape = 18,
      size = 4,
      color = "grey20"
    ) +
    geom_line(
      data = df_combined,
      aes(x = factor(format(n_bucket, big.mark = ",")), y = rss, group = 1),
      linetype = "solid",
      linewidth = 0.7,
      color = "grey20"
    ) +
    ...
    geom_point(
      data = df_combined,
      aes(x = factor(format(n_bucket, big.mark = ",")), y = additive),
      shape = 1,
      size = 3.5,
      color = "grey45"
    ) +
    ...
    annotate(
      "text",
      x = 2,
      y = max(df_combined$additive) * 0.95,
      label = "NAEP\nrange",
      color = ZISSOU1_BASE[3],
      size = 2.8,
      fontface = "italic"
    ) +
    # Scales
    scale_fill_manual(
      values = c(
        "Error 1: Sampling" = ZISSOU1_BASE[4],
    ...
    labs(
      ...
      subtitle = paste0(
        ...
        sprintf(
          "Bridge cost (Phase A): %.2f SGPc | Diamonds = RSS combined | Circles = additive envelope",
          bridge_cost
        )
      ),
      x = "Sample Size (N)",
...
if (exists("phase_b_precision") && nrow(phase_b_precision) > 0) {
  ...
  build_mae_panel <- function(dt, metric = c("median", "mean"), ylim = D_YLIM) {
    ...
    agg[,
      pool_type := fifelse(
        grepl("cluster", pool_type, ignore.case = TRUE),
        "Cluster",
        "District"
      )
    ]
    ...
    ggplot(agg, aes(x = n_bucket, y = mae_avg, color = pool_type)) +
      ...
      geom_ribbon(
        aes(ymin = mae_q25, ymax = mae_q75, fill = pool_type),
        alpha = 0.15,
        color = NA
      ) +
      geom_ribbon(
        aes(ymin = mae_q10, ymax = mae_q90, fill = pool_type),
        alpha = 0.07,
        color = NA
      ) +
      ...
      scale_color_manual(
        values = c("District" = ZISSOU1_BASE[1], "Cluster" = ZISSOU1_BASE[4]),
        name = "Pool Type"
      ) +
      scale_fill_manual(
        values = c("District" = ZISSOU1_BASE[1], "Cluster" = ZISSOU1_BASE[4]),
      ...
      labs(
        title = paste0("Accuracy: MAE for ", label, " SGPc by Sample Size"),
        subtitle = paste0(
          "How close is the cross-sectional ",
          tolower(label),
  ...
  build_ci_panel <- function(
    ...
    p <- ggplot(agg, aes(x = n_bucket, y = ci_avg, color = pool_type)) +
      ...
      geom_ribbon(
        aes(ymin = ci_q25, ymax = ci_q75, fill = pool_type),
        alpha = 0.15,
        color = NA
      ) +
      geom_line(linewidth = 0.9) +
      ...
      labs(
        title = paste0(
          "Precision: Bootstrap 95% CI Width for ",
          label,
          " SGPc"
        ),
        subtitle = paste0(
  ...
  annotate_d_row <- function(p_mae, p_ci, label) {
    (p_mae | p_ci) +
      plot_annotation(
        title = paste0(
          "Recovery Accuracy and Precision vs Sample Size \u2014 ",
          label,
          " SGPc"
        ),
        subtitle = paste0(
  ...
  p_ci_median <- build_ci_panel(
    phase_b_precision,
    "median",
    sampling_context = .subsample_caption
  )
  p_mae_mean <- build_mae_panel(phase_b_precision, "mean")
  p_ci_mean <- build_ci_panel(
    phase_b_precision,
    "mean",
    sampling_context = .subsample_caption
  )
  ...
  save_plot_multi(
    p_d_combined,
    "panel_d_combined_precision_vs_n",
    enhanced_dir,
    width = 14,
    height = 13
  )
...
if (file.exists(quantile_slices_csv)) {
  ...
  p_k3a <- ggplot(
    qs_steps,
    aes(x = p, y = Q0, color = factor(round(u, 3)), group = slice)
  ) +
    ...
    labs(
      title = expression(paste(
        "Transition Kernel: Conditional Quantile Curves ",
        Q[0](p ~ "|" ~ u)
      )),
      subtitle = paste0(
...
cat("K3b: Transition Kernel (full surface)... ")
{
  ...
  p_k3b <- ggplot() +
    ...
    geom_tile(data = surf_grid, aes(x = u, y = v, fill = F0)) +
...
if (
  ...
  K4_SCALE_COLORS <- colorRampPalette(c("#D6EEF5", ZISSOU1_BASE[1], "#1A5B72"))(
    100
  )
  ...
  p_k4_mae <- ggplot(
    heat_dt,
    aes(x = n_label, y = span_label, fill = median_mae)
  ) +
    geom_tile(color = "white", linewidth = 0.8) +
    geom_text(
      aes(label = sprintf("%.2f", median_mae)),
      color = K4_TEXT_COLOR,
      fontface = "bold",
      size = 4
    ) +
  ...
  p_k4_ci <- ggplot(
    heat_dt,
    aes(x = n_label, y = span_label, fill = ci_width_95)
  ) +
    geom_tile(color = "white", linewidth = 0.8) +
    geom_text(
      aes(label = sprintf("%.2f", ci_width_95)),
      color = K4_TEXT_COLOR,
      fontface = "bold",
      size = 4
    ) +
...
if (exists("phase_b_copula") && nrow(phase_b_copula) > 0) {
  ...
  cop_amean <- round(
    mean(abs(phase_b_copula$delta_median_vs_base), na.rm = TRUE),
    2
  )
  ...
  p_j1 <- ggplot(phase_b_copula, aes(x = delta_median_vs_base)) +
    geom_histogram(
      ...
      bins = 15,
      fill = alpha(STEP3_COLORS$predicted, 0.3),
  ...
  p_j2 <- ggplot(phase_b_indep, aes(x = delta_median)) +
    geom_histogram(
      aes(y = after_stat(density)),
      bins = 15,
      fill = alpha(STEP3_COLORS$actual, 0.3),
    ...
    geom_rug(
      data = phase_b_indep[k3_changed == TRUE],
      aes(x = delta_median),
      sides = "b",
      color = ZISSOU1_BASE[5],
      alpha = 0.9,
...
if (exists("dsg") && nrow(dsg) > 0) {
  ...
  metrics$status <- c(
    ifelse(
      abs(row$mean_sgpc_inferred - row$mean_sgpc_true) < 2,
      "good",
      ifelse(
        abs(row$mean_sgpc_inferred - row$mean_sgpc_true) < 5,
        "warn",
        "bad"
      )
    ),
    ifelse(
      abs(row$median_sgpc_inferred - row$median_sgpc_true) < 2,
      "good",
      ifelse(
        abs(row$median_sgpc_inferred - row$median_sgpc_true) < 5,
        "warn",
        "bad"
      )
    ),
    ifelse(
      row$w1_reduction_pct > 70,
      "good",
      ifelse(row$w1_reduction_pct > 40, "warn", "bad")
    ),
    ifelse(
      row$max_abs_residual < 0.02,
      "good",
    ...
    ifelse(
      !is.na(row$bootstrap_converged) &&
        row$bootstrap_converged / row$bootstrap_n > 0.90,
      "good",
      ifelse(
        !is.na(row$bootstrap_converged) &&
          row$bootstrap_converged / row$bootstrap_n > 0.80,
        "warn",
        "bad"
      )
    ),
    ifelse(
      row$k3_consistency > 0.70,
      "good",
...
if (exists("phase_b_precision") && nrow(phase_b_precision) > 0) {
  ...
  prec_bias[,
    stratum := fifelse(
      grepl("CLUSTER_LOW", pool_id),
      "Low Growth",
      fifelse(
        grepl("CLUSTER_TYPICAL", pool_id),
        "Typical Growth",
  ...
  prec_bias[,
    stratum := factor(
      stratum,
      levels = c("Low Growth", "Typical Growth", "High Growth", "District")
    )
  ]
...
if (exists("phase_b") && nrow(phase_b) > 0 && "year_span" %in% names(phase_b)) {
  ...
  make_e_panel <- function(
    ...
    p <- ggplot(dt, aes(x = span_label, y = .data[[y_col]])) +
      geom_violin(
        fill = alpha(STEP3_COLORS$predicted, 0.12),
        color = STEP3_COLORS$predicted,
        linewidth = 0.4,
        trim = TRUE,
        scale = "width"
      ...
      geom_jitter(
        aes(color = pool_type, shape = pool_type),
        width = 0.12,
        size = 2.2,
        alpha = 0.7
      ) +
      ...
      labs(title = ttl, subtitle = sub, x = "Year Span", y = y_label) +
      theme_publication() +
      theme(legend.position = "top")
  ...
  save_plot_multi(
    p_e_median,
    "panel_e_median_recovery_by_span",
    enhanced_dir,
    width = 12,
    height = 7
  )
  ...
  save_plot_multi(
    p_e_combined,
    "panel_e_combined_recovery_by_span",
    enhanced_dir,
    width = 12,
    height = 13
  )
