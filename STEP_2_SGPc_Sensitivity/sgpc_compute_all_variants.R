  ...
  tryCatch(
    {
      ...
      cat(sprintf(
        "    Conditions: %d across %d datasets\n",
        manifest$metadata$n_conditions,
        manifest$metadata$n_datasets
      ))
      cat(sprintf(
        "    Year spans tested: %s\n",
      ...
      if (
        nrow(canonical_params) > 0 &&
          "overall_stability" %in% names(canonical_params)
      ) {
        n_high <- sum(
          canonical_params$overall_stability == "HIGH",
          na.rm = TRUE
        )
        n_med <- sum(
          canonical_params$overall_stability == "MEDIUM",
          na.rm = TRUE
        )
        n_low <- sum(canonical_params$overall_stability == "LOW", na.rm = TRUE)
        cat(sprintf(
          "    Canonical stability: %d HIGH, %d MEDIUM, %d LOW\n",
          n_high,
          n_med,
          n_low
        ))
...
.subset_conditions <- function(
  conditions,
  max_n = NULL,
  strategy = "stratified",
  seed = 42
) {
  if (is.null(max_n) || max_n >= length(conditions)) {
    return(conditions)
  }
...
.compute_worker_count <- function(
  ...
  if (is.null(mem_per_worker_gb)) {
    ...
    if (!is.null(ds_config)) {
      use_sgp <- exists("USE_SGP_DATA", envir = .GlobalEnv) &&
        isTRUE(get("USE_SGP_DATA", envir = .GlobalEnv))
...
compute_sgpc_variants <- function(
  ...
  if (
    ...
    if (!is.null(dataset_config)) {
      ...
      if (use_sgp && !is.null(dataset_config$local_path_sgp)) {
        ...
        if (use_sgp) {
          cat(
            "\n[WORKER] WARNING: SGP data not configured for",
            dataset_id,
            ", using base data\n"
          )
        }
  ...
  if (
    !is.null(phase1_results$pseudo_observations) &&
  ...
  result <- data.table(
    ...
    grade_current = cond_meta$grade_current,
    ID = pairs$ID,
    SCHOOL_NUMBER = if ("SCHOOL_NUMBER" %in% names(pairs)) {
      pairs$SCHOOL_NUMBER
    } else {
      NA_character_
    },
    DISTRICT_NUMBER = if ("DISTRICT_NUMBER" %in% names(pairs)) {
      pairs$DISTRICT_NUMBER
    } else {
      NA_character_
    },
  ...
  cat(sprintf(
    " %.1fs",
    as.numeric(difftime(Sys.time(), sgpc_start, units = "secs"))
  ))
  ...
  gaussian_cop <- normalCopula(
    param = cor(u, v, method = "kendall", use = "complete.obs") * sin(pi / 2)
  )
  result[,
  ...
  sgpc_start <- Sys.time()
  result[,
    sgpc_comonotonic := sgpc_engine(u, v, "comonotonic", scale = "percentile")
  ]
  cat(sprintf(
    " %.1fs",
    as.numeric(difftime(Sys.time(), sgpc_start, units = "secs"))
...
for (dataset_id in DATASETS_TO_PROCESS) {
  ...
  if (!exists("STATE_DATA_LONG")) {
    ...
    if (!is.null(ds_config)) {
      ...
      if (use_sgp && !is.null(ds_config$local_path_sgp)) {
        ...
        if (use_sgp) {
          cat("  WARNING: SGP data not configured, using base data\n")
        }
      }
  ...
  if (length(conditions) < length(conditions_all)) {
    cat(sprintf(
      "  SUBSET MODE: %d of %d conditions selected (strategy=%s, seed=%d)\n",
      length(conditions),
  ...
  perf_dataset$phase1_load_secs <- as.numeric(difftime(
    Sys.time(),
    phase1_load_start,
    units = "secs"
  ))
  ...
  if (USE_PARALLEL) {
    if (!requireNamespace("mirai", quietly = TRUE)) {
      stop(
        "mirai package required for parallel processing. Install with: install.packages('mirai')"
      )
    }
    ...
    cat(sprintf(
      "  SELECTED:        %d workers  (limited by %s)\n",
      n_workers,
      winfo$limiting_factor
    ))
    ...
    for (i in seq_along(all_condition_results)) {
      ...
      if (i %% 10 == 0) {
        cat(sprintf(
          "  Processed: %d/%d (%.1f%%) - Success: %d, Errors: %d\n",
          i,
          length(conditions),
          100 * i / length(conditions),
          n_success,
          n_errors
        ))
      }
    ...
    for (i in seq_along(conditions)) {
      ...
      if (i %% 10 == 1 || i == 1) {
        ...
        remaining_est <- if (i > 1) {
          elapsed / (i - 1) * (length(conditions) - i + 1)
        } else {
          NA
        }
        cat(sprintf(
    ...
    cat(sprintf(
      "SEQUENTIAL PROCESSING COMPLETE: %d/%d successful (%.1f%%), %d skipped, %d errors\n",
      n_success,
      length(conditions),
      100 * n_success / length(conditions),
      n_skipped,
      n_errors
    ))
    cat(sprintf(
  ...
  # Record compute time
  perf_dataset$compute_secs <- as.numeric(difftime(
    Sys.time(),
    compute_start,
    units = "secs"
  ))
  ...
  perf_dataset$save_secs <- as.numeric(difftime(
    Sys.time(),
    save_start,
    units = "secs"
  ))
  perf_dataset$total_secs <- as.numeric(difftime(
    Sys.time(),
    dataset_wall_start,
    units = "secs"
  ))
  ...
  cat(sprintf(
    "  Compute:       %6.1f s  (%.1f min)\n",
    perf_dataset$compute_secs,
    perf_dataset$compute_secs / 60
  ))
  cat(sprintf("  Save:          %6.1f s\n", perf_dataset$save_secs))
  cat(sprintf(
    "  TOTAL:         %6.1f s  (%.1f min)\n",
    perf_dataset$total_secs,
    perf_dataset$total_secs / 60
  ))
  cat(sprintf(
    "  Throughput:    %.2f conditions/min\n",
