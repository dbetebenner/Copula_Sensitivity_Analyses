...
if (!requireNamespace("wesanderson", quietly = TRUE)) {
  stop(
    "Package 'wesanderson' required but not installed. Install with: install.packages('wesanderson')"
  )
}
require(wesanderson)
...
accuracy_background_layers <- function(comps) {
  ...
  if (n_accuracy == 0 || n_consistency == 0) {
    return(list())
  }
...
xaxis_accuracy_colors <- function(comps) {
  ifelse(
    comps %in% CONSISTENCY_COMPARISONS,
    CONSISTENCY_LABEL_COLOR,
    ACCURACY_LABEL_COLOR
  )
}
...
plot_individual_ecdf <- function(
  ...
  available <- intersect(
    comparisons,
    unique(enhanced_stats$individual_stats$all_ecdf_data$comparison)
  )
  if (length(available) == 0) {
    stop("No valid comparisons found in data.")
  }
  ...
  p <- ggplot(
    ecdf_data,
    aes(x = delta, y = cumulative_pct, color = comparison)
  ) +
    geom_line(linewidth = 0.78) +
    geom_vline(
      xintercept = reference_lines,
      linetype = "dashed",
      color = "gray50",
      linewidth = 0.5
    ) +
    ...
    scale_color_manual(
      values = COMPARISON_COLORS[comparisons],
      name = "Comparison"
    ) +
    scale_x_continuous(
      name = bquote(
        "Absolute Difference (|" * Delta * "| in percentile points)"
      ),
      breaks = seq(0, 50, 5),
      limits = c(0, 50)
    ) +
    ...
    labs(
      ...
      caption = sprintf(
        ...
        if ("dataset_id" %in% names(enhanced_stats$rank_agreement)) {
          uniqueN(enhanced_stats$rank_agreement[, paste(
            dataset_id,
            condition_id,
            sep = "__"
          )])
...
plot_group_ecdf <- function(
  ...
  if (group_level == "district" && !is.null(enhanced_stats$district_stats)) {
    ...
    if (is.null(enhanced_stats$group_stats)) {
      stop(
        "Group statistics not available. Ensure SCHOOL_NUMBER/DISTRICT_NUMBER are in data."
      )
    }
  ...
  if (is.null(title)) {
    title <- sprintf(
      "%s-Level Aggregation (%s): Differences Shrink Dramatically",
      level_label,
      agg_label
    )
  }
  ...
  ecdf_key_mean <- if (group_level == "district") {
    "all_district_ecdf_data_mean"
  } else {
    "all_group_ecdf_data_mean"
  }
  ecdf_key_median <- if (group_level == "district") {
    "all_district_ecdf_data_median"
  } else {
    "all_group_ecdf_data_median"
  }
  ecdf_key_compat <- if (group_level == "district") {
    "all_district_ecdf_data"
  } else {
    "all_group_ecdf_data"
  }
  ...
  if (length(available) == 0) {
    stop("No valid comparisons found in group data.")
  }
  comparisons <- available
  ecdf_data <- ecdf_data[comparison %in% comparisons]
  ecdf_data <- thin_ecdf(ecdf_data, max_pts = 10000)
  ...
  delta_key <- if (agg_method == "median") {
    "median_group_delta_median"
  } else {
    "median_group_delta_mean"
  }
  delta_key_compat <- "median_group_delta"
  grp_medians <- sapply(comparisons, function(comp) {
    s <- grp$by_comparison[[comp]]
    if (!is.null(s) && !is.null(s[[delta_key]])) {
      s[[delta_key]]
    } else if (!is.null(s) && !is.null(s[[delta_key_compat]])) {
      s[[delta_key_compat]]
    } else {
      NA_real_
    }
  ...
  p <- ggplot(
    ecdf_data,
    aes(x = delta_group, y = cumulative_pct, color = comparison)
  ) +
    geom_line(linewidth = 0.78) +
    geom_vline(
      xintercept = reference_lines,
    ...
    labs(
      title = title,
      subtitle = sprintf(
        "Aggregation via %s | Does averaging students to %s level wash out copula-choice differences?\nAggregation sharply reduces sensitivity | %s",
        agg_label,
        tolower(level_label),
        comparison_text
      ),
      caption = sprintf(
        "%ss with >= %s students | Total groups: ~%s | Aggregation: %s\n(%ss as surrogates for TIMSS countries / NAEP states)",
        level_label,
        min_n_label,
        n_groups_str,
        agg_label,
  ...
  if (add_inset && !is.null(enhanced_stats$group_size_analysis)) {
    ...
    if (length(inset_data_list) > 0) {
      ...
      delta_col_inset <- if (
        agg_method == "median" && "delta_group_median" %in% names(inset_data)
      ) {
        "delta_group_median"
      ...
      if (!delta_col_inset %in% names(inset_data)) {
        inset_data[, delta_group := delta_group_mean]
      }
      ...
      if (length(rep_comp) > 0) {
        ...
        p_inset <- ggplot(inset_data, aes(x = n, y = get(delta_col_inset))) +
          ...
          geom_smooth(
            method = "loess",
            color = COMPARISON_COLORS[rep_comp[1]],
            se = FALSE,
            linewidth = 1
          ) +
          scale_x_log10(
...
plot_condition_dots <- function(
  ...
  for (comp_name in comparisons) {
    ...
    mad_by_cols <- if ("dataset_id" %in% names(sgpc_data)) {
      c("dataset_id", "condition_id", "year_span", "content_area")
    } else {
      c("condition_id", "year_span", "content_area")
    }
    mad_data <- sgpc_data[,
      .(
  ...
  bottom_content <- tail(
    sort(unique(as.character(mad_by_condition$content_area))),
    1
  )
  ...
  p <- ggplot(mad_by_condition, aes(x = comparison, y = mad)) +
    ...
    geom_point(
      ...
      size = 2,
      shape = 18,
      color = rgb(247, 247, 247, maxColorValue = 255)
    ) +
    geom_errorbar(
      data = summary_stats,
    ...
    facet_grid(
      content_area ~ year_span,
      labeller = labeller(
        year_span = function(x) paste(x, "year"),
...
plot_rank_agreement <- function(
  ...
  available <- intersect(
    comparisons,
    unique(enhanced_stats$rank_agreement$comparison)
  )
  if (length(available) == 0) {
    stop("No valid comparisons found in rank agreement data.")
  }
  comparisons <- available
...
plot_decile_stability <- function(
  ...
  if (
    !is.null(enhanced_stats$individual_bucket_stability) &&
  ...
  if (length(available) == 0) {
    stop("No valid comparisons found in decile data.")
  }
  comparisons <- available

  # Filter to requested comparisons and K values
  class_data <- class_data[
    comparison %in% comparisons & n_buckets %in% bucket_values
  ]
  if (nrow(class_data) == 0) {
    stop("No matching rows after filtering by comparison and n_buckets.")
  }
  ...
  if (stratify_by == "year_span") {
    ...
    class_data <- class_data[
      stratum %in% c("Overall", "MATHEMATICS", "READING")
    ]
  ...
  como_label <- grep(
    "Comonotonic",
    levels(decile_long$comparison),
    value = TRUE
  )
  como_missing <- decile_long[
    comparison %in%
      como_label &
  ...
  p <- ggplot(
    decile_long,
    aes(x = comparison, y = proportion, fill = category)
  ) +
    accuracy_background_layers(levels(class_data$comparison)) +
    geom_col(position = "stack", width = 0.7) +
    geom_text(
      data = decile_long[
        category == "Exact match" & !is.na(proportion) & proportion > 0
      ],
    ...
    labs(
      title = title,
      subtitle = sprintf(
        "Do individual student classifications change when the copula model changes?\nRows: K=3/5/10 | Stratified by %s | \u03ba = Cohen\u2019s weighted kappa (linear/quadratic weights)",
        facet_label
      ),
      caption = paste0(
        "X-axis label colour: ",
  ...
  kappa_anno_bottom <- kappa_anno[
    bucket_label == "Individual SGPc Deciles (K=10)"
  ]
  ...
  xaxis_label_data_bottom <- xaxis_label_data[
    bucket_label == "Individual SGPc Deciles (K=10)"
  ]
  p <- p +
    geom_text(
    ...
    p <- p +
      geom_text(
        ...
        aes(
          x = comparison,
          y = 0.5,
          label = "Category Classification not Possible (Bimodal: Values are 1 or 99 only)"
        ),
        inherit.aes = FALSE,
        angle = 90,
        size = 1.3,
...
plot_group_bucket_stability <- function(
  ...
  if (is.null(gbs) || nrow(gbs) == 0) {
    ...
    return(
      ggplot() + theme_void() + ggtitle("Panel D2: No group-level bucket data")
    )
  }
  ...
  if (is.null(title)) {
    title <- paste0(
      "Classification Accuracy/Precision: Group-Level ",
      agg_label,
      " Agreement"
    )
  }
  ...
  if (length(available) == 0) {
    ...
    return(
      ggplot() + theme_void() + ggtitle("Panel D2: No matching comparisons")
    )
  }
  ...
  xaxis_label_data_d2[,
    xcolor := xaxis_accuracy_colors(as.character(comparison))
  ]
  ...
  if ("kappa_w" %in% names(plot_dt)) {
    ...
    kappa_anno_d2 <- kappa_anno_d2[
      !is.na(kappa_w) & bucket_label == bucket_labels[["10"]]
    ]
  ...
  xaxis_label_data_bottom <- xaxis_label_data_d2[
    bucket_label == bucket_labels[["10"]]
  ]
...
plot_group_transition_matrices <- function(
  ...
  if (is.null(tm) || nrow(tm) == 0 || is.null(tmet) || nrow(tmet) == 0) {
    ...
    return(
      ggplot() + theme_void() + ggtitle("Panel D3: No transition matrix data")
    )
  }

  if (length(n_buckets) != 1L) {
    stop(
      "plot_group_transition_matrices() now expects a single K per figure (e.g., 5, 7, or 10)."
    )
  }
  ...
  if (is.null(title)) {
    title <- paste0(
      "Classification Pathways: Group-Level ",
      agg_label,
      " Transition Matrices (K=",
      selected_k,
      ")"
    )
  }
  ...
  if (nrow(tm_filt) == 0) {
    available_comps <- unique(tm[
      agg_method == selected_agg & n_buckets == selected_k,
      comparison
    ])
    if (length(available_comps) == 0) {
      warning("No transition matrix rows for selected aggregation method.")
      return(
        ggplot() + theme_void() + ggtitle("Panel D3: No matching transitions")
      )
    }
  ...
  if (length(available_comparisons) == 0) {
    warning(
      "No requested comparisons available for selected K and aggregation."
    )
    return(
      ggplot() + theme_void() + ggtitle("Panel D3: No matching comparisons")
    )
  }
  ...
  tm_filt[,
    comparison_label := factor(comparison, levels = available_comparisons)
  ]
  tm_filt[,
    label_txt := fifelse(
      !is.na(row_prop),
      sprintf(".%02.0f", row_prop * 100),
      ""
    )
  ]
  tm_filt[, label_txt := fifelse(label_txt == ".100", "1.0", label_txt)]
  tm_filt[,
    label_col := fifelse(!is.na(row_prop) & row_prop >= 0.45, "white", "gray20")
  ]
  ...
  met_filt[,
    comp_short := fifelse(
      comparison %in% names(comp_short),
      comp_short[comparison],
      comparison
    )
  ]
  ...
  stats_dt <- met_filt[,
    {
      ...
      stats_label <- paste(
        ...
        sprintf(
          "  P(Cbot|Ebot) = %.1f%%",
          100 * d$p_comp_bottom_given_emp_bottom
        ),
        sprintf(
          "  P(Ebot|Cbot) = %.1f%%",
          100 * d$p_emp_bottom_given_comp_bottom
        ),
        sep = "\n"
      )
      list(
  ...
  p <- ggplot(
    tm_filt,
    aes(x = comparison_bucket, y = empirical_bucket, fill = row_prop)
  ) +
    geom_tile(color = "white", linewidth = 0.25) +
    geom_text(
      aes(label = label_txt, color = label_col),
      size = 1.9,
      show.legend = FALSE
    ) +
    geom_label(
    ...
    labs(
      title = title,
      subtitle = paste0(
        "How do school/district growth-bucket classifications shift under different comparison models at K=",
        selected_k,
        "?\n",
        "Rows: comparison groups | Columns: School/District | Aggregation: ",
        agg_label,
...
plot_prior_quartile_sensitivity <- function(
  ...
  if (
    is.null(enhanced_stats$prior_quartile_stats) ||
      nrow(enhanced_stats$prior_quartile_stats$raw) == 0
  ) {
    stop(
      "Prior quartile statistics not available. Recompute enhanced statistics."
    )
  }
  ...
  if (length(available) == 0) {
    stop("No valid comparisons found in quartile data.")
  }
  comparisons <- available
...
plot_cross_dataset_comparison <- function(
  ...
  if (
    is.null(enhanced_stats$by_dataset_stats) ||
  ...
  if (length(available) == 0) {
    stop("No valid comparisons found in dataset stats.")
  }
  comparisons <- available
  ...
  p <- ggplot(
    ds_data,
    aes(x = comparison, y = mad, color = dataset_label, group = dataset_label)
  ) +
    accuracy_background_layers(comps) +
    ...
    labs(
      ...
      caption = sprintf(
        "n = %d total observations | %d datasets | %d conditions\nX-axis label colour: purple = classification accuracy (vs empirical) | green = inter-model consistency",
        sum(ds_data$n),
...
plot_multilevel_aggregation <- function(
  ...
  if (is.null(title)) {
    title <- sprintf(
      "Multi-Level Aggregation (%s): Individual to School to District",
      agg_label
    )
  }
  ...
  if (!is.null(enhanced_stats$individual_stats$all_ecdf_data)) {
    ...
    if (length(available_ind) > 0) {
      ...
      if ("delta" %in% names(ind_ecdf)) {
        setnames(ind_ecdf, "delta", "abs_diff")
      }
      ecdf_list[["individual"]] <- ind_ecdf[, .(
        comparison,
        abs_diff,
        cumulative_pct,
        level
  ...
  sch_ecdf_key <- if (agg_method == "median") {
    "all_group_ecdf_data_median"
  } else {
    "all_group_ecdf_data_mean"
  }
  ...
  if (is.null(sch_ecdf_src)) {
    sch_ecdf_src <- enhanced_stats$group_stats$all_group_ecdf_data
  }
  if (!is.null(sch_ecdf_src)) {
    ...
    if (length(available_sch) > 0) {
      ...
      if ("delta_group" %in% names(sch_ecdf)) {
        setnames(sch_ecdf, "delta_group", "abs_diff")
      }
      ecdf_list[["school"]] <- sch_ecdf[, .(
        comparison,
        abs_diff,
        cumulative_pct,
        level
  ...
  dist_ecdf_key <- if (agg_method == "median") {
    "all_district_ecdf_data_median"
  } else {
    "all_district_ecdf_data_mean"
  }
  ...
  if (is.null(dist_ecdf_src)) {
    dist_ecdf_src <- enhanced_stats$district_stats$all_district_ecdf_data
  }
  if (!is.null(dist_ecdf_src)) {
    ...
    if (length(available_dist) > 0) {
      ...
      if ("delta_group" %in% names(dist_ecdf)) {
        setnames(dist_ecdf, "delta_group", "abs_diff")
      }
      ecdf_list[["district"]] <- dist_ecdf[, .(
        comparison,
        abs_diff,
        cumulative_pct,
        level
  ...
  if (length(ecdf_list) < 2) {
    stop(
      "Need at least 2 aggregation levels for multi-level plot. Check that school/district stats are available."
    )
  }

  all_ecdf <- rbindlist(ecdf_list)
  all_ecdf[,
    level := factor(level, levels = c("Individual", "School", "District"))
  ]
  all_ecdf <- thin_ecdf(
    all_ecdf,
    max_pts = 10000,
    by_col = c("level", "comparison")
  )
  ...
  p <- ggplot(
    all_ecdf,
    aes(x = abs_diff, y = cumulative_pct, color = level, linetype = level)
  ) +
    geom_line(linewidth = 0.78) +
    geom_vline(
      xintercept = c(5, 10),
    ...
    labs(
      ...
      caption = sprintf(
        "Individual = student-level | School = n>=10 | District = n>=30 | Aggregation: %s",
        agg_label
      )
    ) +
    theme_publication(base_size = 9) +
...
plot_error_decomposition <- function(
  ...
  p_ribbon <- ggplot(
    cell,
    aes(x = sample_size, y = mad_mean, color = comparison, fill = comparison)
  ) +
    geom_ribbon(
      aes(ymin = mad_q05, ymax = mad_q95),
      alpha = 0.12,
      linewidth = 0
    ) +
    geom_ribbon(
      aes(ymin = mad_q25, ymax = mad_q75),
      alpha = 0.22,
      linewidth = 0
    ) +
    ...
    labs(
      ...
      subtitle = paste0(
        ...
        sampling_results$metadata$B,
        " replicates | Narrow ribbons confirm robustness at small N"
      )
    ) +
  ...
  if (!is.finite(sd_scale) || sd_scale <= 0) {
    sd_scale <- 1
  }

  p_share <- ggplot(
    per_ss_long,
    aes(x = sample_size, y = share, fill = source_label)
  ) +
    ...
    scale_y_continuous(
      ...
      sec.axis = sec_axis(
        ~ . * sd_scale,
        name = "Total MAD SD (percentile points)"
      )
    ) +
...
save_plot_multi_panel <- function(
  plot_list,
  name,
  dir,
  width = 14,
...
plot_condition_n_vs_mad <- function(
  ...
  if (is.null(cond_stats) || nrow(cond_stats) == 0) {
    ...
    return(
      ggplot() + theme_void() + ggtitle("Panel J: No condition-level data")
    )
  }
  ...
  if (nrow(plot_dt) == 0) {
    ...
    return(
      ggplot() + theme_void() + ggtitle("Panel J: No matching comparisons")
    )
  }
  ...
  p <- ggplot(
    plot_dt,
    aes(x = n, y = mad, color = comparison, fill = comparison)
  ) +
    ...
    labs(
      ...
      caption = bquote(
        "MAD = mean |" *
          Delta *
          "SGPc| between copula variants within each condition"
      )
    ) +
    theme_publication(base_size = 10) +
    theme(
...
plot_group_rank_stability <- function(
  ...
  if (length(available) == 0) {
    ...
    return(
      ggplot() + theme_void() + ggtitle("Panel K: No matching comparisons")
    )
  }

  # Select the appropriate rho column; fall back to 'rho' for backward compat
  if (!rho_col %in% names(gra) && "rho" %in% names(gra)) {
    rho_col <- "rho"
  }
  ...
  p <- ggplot(plot_dt, aes(x = comparison, y = rho_plot)) +
    ...
    labs(
      ...
      subtitle = paste0(
        ...
        tolower(agg_label),
        " growth rankings preserved across copula models?\n",
        "Each dot represents Spearman \u03c1 for ",
        tolower(agg_label),
      ...
      caption = sprintf(
        "School (n\u226510 students) | District (n\u226530 students) | %d conditions",
        n_conditions
      )
    ) +
    theme_publication(base_size = 9) +
...
plot_diag_quartile_focused <- function(
  ...
  if (is.null(raw_data) || nrow(raw_data) == 0) {
    stop(
      "Prior quartile raw data not available. Recompute enhanced statistics."
    )
  }

  available <- intersect(comparisons, unique(raw_data$comparison))
  if (length(available) == 0) {
    stop("No valid comparisons in quartile data.")
  }
...
plot_diag_quartile_summary <- function(
  ...
  if (is.null(summary_data) || nrow(summary_data) == 0) {
    stop(
      "Prior quartile summary data not available. Recompute enhanced statistics."
    )
  }

  available <- intersect(comparisons, unique(summary_data$comparison))
  if (length(available) == 0) {
    stop("No valid comparisons in quartile summary.")
  }
...
plot_diag_delta_vs_prior <- function(
  ...
  if (
    !"SCALE_SCORE_PRIOR" %in% names(sgpc_data) ||
  ...
  for (comp_name in names(comparison_pairs)) {
    ...
    var1 <- vars[1]
    var2 <- vars[2]
    if (!var1 %in% names(sgpc_data) || !var2 %in% names(sgpc_data)) {
      next
    }

    sub <- sgpc_data[
      !is.na(get(var1)) & !is.na(get(var2)) & !is.na(SCALE_SCORE_PRIOR)
    ]
    if (nrow(sub) == 0) {
      next
    }
    ...
    delta_list[[comp_name]] <- sub[, .(
      prior_pctile = frank(SCALE_SCORE_PRIOR, ties.method = "average") /
        .N *
        100,
...
plot_diag_mean_vs_median_stability <- function(
  ...
  has_school <- "SCHOOL_NUMBER" %in%
    names(sgpc_data) &&
    sum(!is.na(sgpc_data$SCHOOL_NUMBER)) > 0
  has_district <- "DISTRICT_NUMBER" %in%
    names(sgpc_data) &&
  ...
  compute_agg_rho <- function(dt, group_var, min_n, pairs, level_label) {
    ...
    for (comp_name in names(pairs)) {
      ...
      var1 <- vars[1]
      var2 <- vars[2]
      if (!var1 %in% names(dt) || !var2 %in% names(dt)) {
        next
      }
  ...
  if (nrow(rho_dt) == 0) {
    stop("No valid group-level data found.")
  }
  ...
  p <- ggplot(
    rho_long,
    aes(x = comparison, y = rho, color = aggregation, shape = aggregation)
  ) +
    geom_point(
      alpha = 0.25,
      size = 1,
...
plot_diag_composition_bias <- function(
  ...
  has_school <- "SCHOOL_NUMBER" %in%
    ...
    sum(!is.na(sgpc_data$SCHOOL_NUMBER)) > 0
  if (!has_school) {
    stop("SCHOOL_NUMBER not found or all NA.")
  }
  if (
    !"SCALE_SCORE_PRIOR" %in% names(sgpc_data) ||
      all(is.na(sgpc_data$SCALE_SCORE_PRIOR))
  ...
  if (length(agg_list) == 0) {
    stop("No valid school-level data found.")
  }
  ...
  for (i in seq_len(nrow(r_summary))) {
    cat(sprintf(
      "    %s: median r = %.3f  [IQR: %.3f, %.3f]  (%d conditions)\n",
  ...
  agg_dt[,
    mean_prior_pctile := frank(mean_prior, ties.method = "average") / .N * 100,
    by = comparison
  ]
