...
for (ds_id in BENCHMARK_DATASETS) {
  ...
  fobj <- if (use_sgp) {
    ds_config$rdata_object_name_sgp
  } else {
    ds_config$rdata_object_name
  }
  ...
  cat(sprintf(
    "  Loaded %s rows in %.1f s\n",
    format(nrow(STATE_DATA_LONG), big.mark = ","),
    load_secs
  ))
  ...
  for (n_sub in BENCHMARK_SUBSETS) {
    ...
    perf_file <- file.path(
      "STEP_2_SGPc_Sensitivity/results/perf",
      paste0("step2_perf_", ds_id, ".json")
    )
    perf <- if (file.exists(perf_file)) {
      jsonlite::fromJSON(perf_file)
    } else {
      list()
    }

    n_ok <- if (!is.null(perf$n_ok)) perf$n_ok else 0
    compute_secs <- if (!is.null(perf$compute_secs)) {
      perf$compute_secs
    } else {
      bench_secs
    }
    ...
    cat(sprintf(
      "  Completed: %d conditions in %.1f s (%.2f s/condition)\n",
      n_ok,
      compute_secs,
...
if (length(results) > 0) {
  ...
  cat(sprintf(
    "Observed throughput: %.2f seconds/condition (median across subsets)\n\n",
    med_rate
  ))
  ...
  for (ec2_cores in c(16, 32, 64, 96, 192)) {
    ...
    cat(sprintf(
      "  %3d cores: ~%.1f hours  (speedup %.1fx vs %d local cores)\n",
      ec2_cores,
      est_hours,
      speedup,
      local_cores
  ...
  cat(sprintf(
    "  dataset_1 file: %.2f GB (in-memory ~%.1f GB per worker)\n",
    bench_df$file_gb[1],
    bench_df$file_gb[1] * 7 + 0.5
  ))
  cat("  Recommended EC2 instance:\n")
  cat(
    "    r8g.24xlarge  (96 vCPU, 768 GB) — safe for all datasets in parallel\n"
  )
  cat(
    "    r8g.12xlarge  (48 vCPU, 384 GB) — adequate, may need fewer workers for dataset_1\n"
  )
