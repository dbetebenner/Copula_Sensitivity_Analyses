...
p_emp_best <- ggplot(
  all_data[!is.na(sgpc_emp) & !is.na(sgpc_best)],
  ...
  labs(
    ...
    subtitle = sprintf(
      ...
      format(
        sum(!is.na(all_data$sgpc_emp) & !is.na(all_data$sgpc_best)),
        big.mark = ","
      )
    ),
...
p_emp_avg <- ggplot(
  all_data[!is.na(sgpc_emp) & !is.na(sgpc_avg)],
  ...
  labs(
    ...
    subtitle = sprintf(
      ...
      format(
        sum(!is.na(all_data$sgpc_emp) & !is.na(all_data$sgpc_avg)),
        big.mark = ","
      )
    ),
    x = "Empirical SGPc (Non-parametric Truth)",
...
p_emp_gaussian <- ggplot(
  all_data[!is.na(sgpc_emp) & !is.na(sgpc_gaussian)],
  ...
  labs(
    ...
    subtitle = sprintf(
      ...
      format(
        sum(!is.na(all_data$sgpc_emp) & !is.na(all_data$sgpc_gaussian)),
        big.mark = ","
      )
    ),
    x = "Empirical SGPc (Non-parametric Truth)",
...
p_emp_comon <- ggplot(
  all_data[!is.na(sgpc_emp) & !is.na(sgpc_comonotonic)],
  ...
  labs(
    ...
    subtitle = sprintf(
      ...
      format(
        sum(!is.na(all_data$sgpc_emp) & !is.na(all_data$sgpc_comonotonic)),
        big.mark = ","
      )
    ),
    x = "Empirical SGPc (Non-parametric Truth)",
...
if (
  "sgpc_gumbel" %in% names(all_data) && sum(!is.na(all_data$sgpc_gumbel)) > 0
) {
  p_emp_gumbel <- ggplot(
    all_data[!is.na(sgpc_emp) & !is.na(sgpc_gumbel)],
    ...
    labs(
      ...
      subtitle = sprintf(
        ...
        format(
          sum(!is.na(all_data$sgpc_emp) & !is.na(all_data$sgpc_gumbel)),
          big.mark = ","
        )
      ),
      x = "Empirical SGPc (Non-parametric Truth)",
...
if ("sgpc_frank" %in% names(all_data) && sum(!is.na(all_data$sgpc_frank)) > 0) {
  p_emp_frank <- ggplot(
    all_data[!is.na(sgpc_emp) & !is.na(sgpc_frank)],
    ...
    labs(
      ...
      subtitle = sprintf(
        ...
        format(
          sum(!is.na(all_data$sgpc_emp) & !is.na(all_data$sgpc_frank)),
          big.mark = ","
        )
      ),
      x = "Empirical SGPc (Non-parametric Truth)",
...
if (
  "sgp_traditional" %in%
    names(all_data) &&
    sum(!is.na(all_data$sgp_traditional)) > 0
) {
  p_emp_trad <- ggplot(
    all_data[!is.na(sgpc_emp) & !is.na(sgp_traditional)],
    ...
    labs(
      ...
      subtitle = sprintf(
        ...
        format(
          sum(!is.na(all_data$sgpc_emp) & !is.na(all_data$sgp_traditional)),
          big.mark = ","
        )
      ),
  ...
  p_best_trad <- ggplot(
    all_data[!is.na(sgpc_best) & !is.na(sgp_traditional)],
    ...
    labs(
      ...
      subtitle = sprintf(
        ...
        format(
          sum(!is.na(all_data$sgpc_best) & !is.na(all_data$sgp_traditional)),
          big.mark = ","
        )
      ),
...
mad_by_stratum <- all_data[,
  {
    ...
    if ("sgpc_gumbel" %in% names(.SD)) {
      result$mad_emp_gumbel <- mean(abs(sgpc_emp - sgpc_gumbel), na.rm = TRUE)
    }
    if ("sgpc_frank" %in% names(.SD)) {
      result$mad_emp_frank <- mean(abs(sgpc_emp - sgpc_frank), na.rm = TRUE)
    }
    if ("sgp_traditional" %in% names(.SD)) {
      result$mad_emp_trad <- mean(abs(sgpc_emp - sgp_traditional), na.rm = TRUE)
    }
    result
  },
...
p_heatmap_best <- ggplot(
  mad_by_stratum,
  aes(x = factor(year_span), y = content_area, fill = mad_emp_best)
) +
  geom_tile(color = "white") +
  geom_text(
    aes(label = sprintf("%.1f", mad_emp_best)),
    color = "white",
    fontface = "bold"
  ) +
...
p_heatmap_avg <- ggplot(
  mad_by_stratum,
  aes(x = factor(year_span), y = content_area, fill = mad_emp_avg)
) +
  geom_tile(color = "white") +
  geom_text(
    aes(label = sprintf("%.1f", mad_emp_avg)),
    color = "white",
    fontface = "bold"
  ) +
  scale_fill_gradient2(
...
if (nrow(all_data) > VIOLIN_MAX) {
  cat(sprintf(
    "  Subsampling to %s observations for violin KDE...\n",
...
if ("sgpc_gumbel" %in% names(all_data)) {
  violin_cols$diff_emp_gumbel <- c("sgpc_emp", "sgpc_gumbel")
}
if ("sgpc_frank" %in% names(all_data)) {
  violin_cols$diff_emp_frank <- c("sgpc_emp", "sgpc_frank")
}
if ("sgp_traditional" %in% names(all_data)) {
  violin_cols$diff_emp_trad <- c("sgpc_emp", "sgp_traditional")
}
...
for (cn in names(violin_cols)) {
  ...
  if (v1 %in% names(violin_data) && v2 %in% names(violin_data)) {
    violin_prep[,
      (cn) := violin_data[!is.na(prior_quartile)][[v1]] -
        violin_data[!is.na(prior_quartile)][[v2]]
    ]
  }
}
...
# Empirical vs Best-Fit
ba_data_best <- all_data[
  !is.na(sgpc_emp) & !is.na(sgpc_best),
  .(
...
p_ba_best <- ggplot(ba_data_best, aes(x = mean, y = diff)) +
  ...
  geom_hline(
    yintercept = mean(ba_data_best$diff, na.rm = TRUE),
    color = "blue",
    linetype = "solid",
    linewidth = 1
  ) +
  geom_hline(
    yintercept = mean(ba_data_best$diff, na.rm = TRUE) +
      1.96 * sd(ba_data_best$diff, na.rm = TRUE),
    color = "blue",
    linetype = "dotted",
    linewidth = 0.8
  ) +
...
if (
  ...
  ba_data_trad <- all_data[
    !is.na(sgpc_emp) & !is.na(sgp_traditional),
    .(
  ...
  p_ba_trad <- ggplot(ba_data_trad, aes(x = mean, y = diff)) +
    ...
    geom_hline(
      yintercept = mean(ba_data_trad$diff, na.rm = TRUE),
      color = "blue",
      linetype = "solid",
      linewidth = 1
    ) +
    geom_hline(
      yintercept = mean(ba_data_trad$diff, na.rm = TRUE) +
        1.96 * sd(ba_data_trad$diff, na.rm = TRUE),
      color = "blue",
      linetype = "dotted",
      linewidth = 0.8
    ) +
    ...
    labs(
      ...
      subtitle = sprintf(
        "Mean diff = %.2f | SD = %.2f | Capstone: copula-based vs B-spline QR",
  ...
  save_plot(
    p_ba_trad,
    "bland_altman_emp_vs_traditional",
    width = 10,
    height = 7
  )
}
...
if (
  ...
  ba_data_gumbel <- all_data[
    !is.na(sgpc_emp) & !is.na(sgpc_gumbel),
    .(
  ...
  p_ba_gumbel <- ggplot(ba_data_gumbel, aes(x = mean, y = diff)) +
    ...
    geom_hline(
      yintercept = mean(ba_data_gumbel$diff, na.rm = TRUE),
      color = "blue",
      linetype = "solid",
      linewidth = 1
    ) +
    geom_hline(
      yintercept = mean(ba_data_gumbel$diff, na.rm = TRUE) +
        1.96 * sd(ba_data_gumbel$diff, na.rm = TRUE),
      color = "blue",
      linetype = "dotted",
      linewidth = 0.8
    ) +
...
if (
  "sgpc_gaussian" %in%
    names(all_data) &&
    sum(!is.na(all_data$sgpc_gaussian)) > 0
) {
  ba_data_gaussian <- all_data[
    !is.na(sgpc_emp) & !is.na(sgpc_gaussian),
    .(
  ...
  p_ba_gaussian <- ggplot(ba_data_gaussian, aes(x = mean, y = diff)) +
    ...
    geom_hline(
      yintercept = mean(ba_data_gaussian$diff, na.rm = TRUE),
      color = "blue",
      linetype = "solid",
      linewidth = 1
    ) +
    geom_hline(
      yintercept = mean(ba_data_gaussian$diff, na.rm = TRUE) +
        1.96 * sd(ba_data_gaussian$diff, na.rm = TRUE),
      color = "blue",
      linetype = "dotted",
      linewidth = 0.8
    ) +
...
cat("Files created:\n")
cat(
  "  - scatter_emp_vs_*.{pdf,svg,png} (including gumbel, frank, traditional)\n"
)
