...
n_conditions <- uniqueN(sgpc_data[, paste(
  dataset_id,
  condition_id,
  sep = "__"
)])
cat(sprintf(
  "Combined dataset: %s observations, %d conditions (across %d datasets)\n\n",
...
if (!has_school && !has_district) {
  warning(
    "SCHOOL_NUMBER and DISTRICT_NUMBER not found or all NA.\n",
  ...
  skip_panel_b <- FALSE
  cat(sprintf(
    "✓ Group identifiers found: SCHOOL_NUMBER=%s, DISTRICT_NUMBER=%s\n\n",
    has_school,
    has_district
  ))
}
...
if (file.exists(enhanced_stats_file) && !FORCE_RECOMPUTE) {
  ...
  if (n_pairs < 8 || !has_d3_tables) {
    if (n_pairs < 8) {
      cat(sprintf(
        "  Cache has %d comparison pairs (need 8). Recomputing...\n",
        n_pairs
      ))
    } else {
  ...
  cat(
    "Computing enhanced statistics (8 comparison pairs, this may take a few minutes)...\n"
  )
  enhanced_stats <- compute_enhanced_statistics(sgpc_data)
...
if (!skip_panel_b) {
  for (agg_m in c("mean", "median")) {
    ...
    cat(sprintf(
      "Generating Panel B (School-level ECDF, %s, with sqrt(n) inset)...\n",
      agg_lbl
    ))
    plots[[paste0("panel_b_", agg_m)]] <- plot_group_ecdf(
      enhanced_stats,
      add_inset = TRUE,
      group_level = "school",
      agg_method = agg_m
    ...
    save_plot_multi(
      plots[[paste0("panel_b_", agg_m)]],
      paste0("panel_b_school_ecdf_", agg_m),
      VIZ_DIR,
      width = PLOT_WIDTH,
      height = PLOT_HEIGHT
    )
    ...
    plots[[paste0("panel_b2_", agg_m)]] <- plot_group_ecdf(
      enhanced_stats,
      add_inset = FALSE,
      group_level = "district",
      agg_method = agg_m
    )
    save_plot_multi(
      plots[[paste0("panel_b2_", agg_m)]],
      paste0("panel_b2_district_ecdf_", agg_m),
      VIZ_DIR,
      width = PLOT_WIDTH,
      height = PLOT_HEIGHT
    )
...
if (!skip_panel_b) {
  for (agg_m in c("mean", "median")) {
    agg_lbl <- if (agg_m == "mean") "Mean SGPc" else "Median SGPc"
    cat(sprintf(
      "Generating Panel D2 [%s] (Group-level bucket stability: K=3,5,10)...\n",
      agg_lbl
    ))
    tryCatch(
      {
        ...
        save_plot_multi(
          plots[[paste0("panel_d2_", agg_m)]],
          paste0("panel_d2_group_bucket_stability_", agg_m),
          VIZ_DIR,
          width = PANEL_CLASSIFICATION_PAGE_WIDTH,
          height = PANEL_CLASSIFICATION_PAGE_HEIGHT
        )
...
if (!skip_panel_b) {
  ...
  for (agg_m in c("mean", "median")) {
    ...
    for (k in d3_bucket_sizes) {
      cat(sprintf(
        "Generating Panel D3 [%s, K=%d] (6x2 comparison x level grid)...\n",
        agg_lbl,
        k
      ))
      tryCatch(
        {
          ...
          file_key <- paste0(
            "panel_d3_group_transition_matrices_k",
            k,
            "_",
            agg_m
          )
        ...
        error = function(e) {
          cat(sprintf(
            "  WARNING: Panel D3 [%s, K=%d] failed: %s\n\n",
            agg_lbl,
            k,
            e$message
          ))
...
if (!skip_panel_b) {
  for (agg_m in c("mean", "median")) {
    agg_lbl <- if (agg_m == "mean") "Mean SGPc" else "Median SGPc"
    cat(sprintf(
      "Generating Panel H (Multi-level aggregation, %s: Individual -> School -> District)...\n",
      agg_lbl
    ))
    tryCatch(
      {
        plots[[paste0("panel_h_", agg_m)]] <- plot_multilevel_aggregation(
          enhanced_stats,
          agg_method = agg_m
        )
        save_plot_multi(
          plots[[paste0("panel_h_", agg_m)]],
          paste0("panel_h_multilevel_aggregation_", agg_m),
          VIZ_DIR,
          width = PLOT_WIDTH,
          height = PLOT_HEIGHT
        )
...
if (file.exists(sampling_results_file) && !FORCE_RECOMPUTE) {
  ...
  if (!identical(cached_sample_sizes, target_sample_sizes)) {
    cat(sprintf(
      "  Cached sample sizes (%s) differ from target (%s); recomputing.\n",
    ...
    cat(sprintf(
      "  Cached sampling sensitivity to: %s\n\n",
      sampling_results_file
    ))
  } else {
    cat(sprintf(
      "  Loaded (%d replicate rows, %d comparison pairs, %d sample sizes)\n\n",
...
cat(
  "Generating Panel I1 & I2 (Error Decomposition: Comparison vs Sampling)...\n"
)
tryCatch(
  {
    ...
    if (is.list(panel_i_plots) && !inherits(panel_i_plots, "gg")) {
      ...
      save_plot_multi(
        plots$panel_i2,
        "panel_i2_variance_decomposition",
        VIZ_DIR,
        width = PLOT_WIDTH,
        height = PLOT_HEIGHT
      )
...
if (!skip_panel_b) {
  for (agg_m in c("mean", "median")) {
    agg_lbl <- if (agg_m == "mean") "Mean SGPc" else "Median SGPc"
    cat(sprintf(
      "Generating Panel K [%s] (Group-Level Rank Stability)...\n",
      agg_lbl
    ))
    tryCatch(
      {
        ...
        save_plot_multi(
          plots[[paste0("panel_k_", agg_m)]],
          paste0("panel_k_group_rank_stability_", agg_m),
          VIZ_DIR,
          width = PLOT_WIDTH,
          height = PLOT_HEIGHT
        )
...
if ("sgpc_best" %in% names(sgpc_data)) {
  ...
  plots$ba_emp_best <- make_bland_altman(
    sgpc_data,
    "sgpc_emp",
    "sgpc_best",
...
if ("sgpc_gaussian" %in% names(sgpc_data)) {
  ...
  plots$ba_emp_gaussian <- make_bland_altman(
    sgpc_data,
    "sgpc_emp",
    "sgpc_gaussian",
...
if ("sgpc_gumbel" %in% names(sgpc_data)) {
  ...
  plots$ba_emp_gumbel <- make_bland_altman(
    sgpc_data,
    "sgpc_emp",
    "sgpc_gumbel",
    "Empirical vs Gumbel"
...
if ("sgp_traditional" %in% names(sgpc_data)) {
  ...
  plots$ba_emp_trad <- make_bland_altman(
    sgpc_data,
    "sgpc_emp",
    "sgp_traditional",
  ...
  tryCatch(
    {
      ...
      save_plot_multi(
        diag_plots$diag_n,
        "diag_n_mean_vs_median_stability",
        VIZ_DIR,
        width = 12,
        height = 7
      )
...
if (!is.null(diag_plots$diag_fa)) {
  diag_composite_panels$fa <- diag_plots$diag_fa
}
if (!is.null(diag_plots$diag_fb)) {
  diag_composite_panels$fb <- diag_plots$diag_fb
}
if (!is.null(diag_plots$diag_l)) {
  diag_composite_panels$l <- diag_plots$diag_l
}
if (!is.null(diag_plots$diag_m)) {
  diag_composite_panels$m <- diag_plots$diag_m
}
  ...
  tryCatch(
    {
      save_plot_multi_panel(
        ...
        "gaussian_misfit_diagnostic_grid",
        VIZ_DIR,
        width = 16,
        height = diag_height,
        ncol = diag_ncol,
        widths = rep(1, diag_ncol)
      )
    },
    error = function(e) {
...
if (!is.null(plots$panel_d3_median)) {
  panel_files[
    "panel_d3_median"
  ] <- "panel_d3_group_transition_matrices_median.pdf"
}
