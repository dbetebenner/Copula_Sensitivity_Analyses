...
compute_weighted_kappa <- function(x1, x2, K = 10, weights = "linear") {
  ...
  x1 <- as.integer(x1[valid])
  x2 <- as.integer(x2[valid])
  if (length(x1) == 0) {
    return(NA_real_)
  }
...
compute_enhanced_statistics <- function(
  ...
  if (has_dataset_id) {
    ...
    cat(
      "  Conditions:",
      uniqueN(sgpc_data[, paste(dataset_id, condition_id, sep = "__")]),
      "\n"
    )
  } else {
    cat(
  ...
  for (comp_name in names(comparison_pairs)) {
    ...
    cat(sprintf(
      "  %s: median=%.1f, Q95=%.1f, P(>5)=%.1f%%, P(>10)=%.1f%%\n",
      comp_name,
      quantiles["50%"],
      quantiles["95%"],
      exceedance_5 * 100,
  ...
  if (!has_school && !has_district) {
    ...
    cat(
      "  Re-run Step 2.1 with updated sgpc_compute_all_variants.R to include these IDs.\n\n"
    )
  }
  ...
  build_group_aggregate_cache <- function(dt, group_var, level_label) {
    ...
    for (comp_name in names(comparison_pairs)) {
      ...
      agg <- dt[
        !is.na(get(group_var)) & !is.na(get(var1)) & !is.na(get(var2)),
        .(
  ...
  if (has_school) {
    cat(
      "  Precomputing school-level group aggregates (reused across sections)...\n"
    )
    school_group_cache <- build_group_aggregate_cache(
      sgpc_data,
      "SCHOOL_NUMBER",
      "school"
    )
  ...
  if (has_district) {
    cat(
      "  Precomputing district-level group aggregates (reused across sections)...\n"
    )
    district_group_cache <- build_group_aggregate_cache(
      sgpc_data,
      "DISTRICT_NUMBER",
      "district"
    )
  ...
  if (has_school) {
    ...
    for (comp_name in names(comparison_pairs)) {
      ...
      if (nrow(group_agg_filtered) > 0) {
        # Mean-based ECDF
        q_mean <- quantile(
          group_agg_filtered$delta_group_mean,
        ...
        # Median-based ECDF
        q_median <- quantile(
          group_agg_filtered$delta_group_median,
          probs = c(0.50, 0.90, 0.95),
          na.rm = TRUE
        )
        ...
        cat(sprintf(
          "    %s: %d schools, median Delta (mean agg)=%.1f, median Delta (median agg)=%.1f (vs individual %.1f)\n",
          comp_name,
          nrow(group_agg_filtered),
          q_mean["50%"],
          q_median["50%"],
    ...
    if (length(group_stats) > 0) {
      all_group_ecdf_data <- rbindlist(lapply(group_stats, function(x) {
        x$group_ecdf_data_mean
      }))
      all_group_ecdf_data_mean <- all_group_ecdf_data
      all_group_ecdf_data_median <- rbindlist(lapply(group_stats, function(x) {
        x$group_ecdf_data_median
      }))
    } else {
    ...
    if (length(group_size_analysis) > 0) {
      size_summary_all <- rbindlist(lapply(
        names(group_size_analysis),
        function(cn) {
  ...
  if (has_district) {
    cat(
      "  Computing district-level aggregates (min n=30, TIMSS/NAEP surrogate)...\n"
    )
    ...
    for (comp_name in names(comparison_pairs)) {
      ...
      if (nrow(dist_agg_filtered) > 0) {
        # Mean-based ECDF
        dq_mean <- quantile(
          dist_agg_filtered$delta_group_mean,
          probs = c(0.50, 0.90, 0.95),
          na.rm = TRUE
        )
        ...
        # Median-based ECDF
        dq_median <- quantile(
          dist_agg_filtered$delta_group_median,
          probs = c(0.50, 0.90, 0.95),
          na.rm = TRUE
        )
        ...
        cat(sprintf(
          "    %s: %d districts, median Delta (mean agg)=%.1f, median Delta (median agg)=%.1f (vs school %.1f, individual %.1f)\n",
          comp_name,
          nrow(dist_agg_filtered),
          dq_mean["50%"],
          dq_median["50%"],
          if (!is.null(group_stats[[comp_name]])) {
            group_stats[[comp_name]]$median_group_delta_mean
          } else {
            NA
          },
          individual_stats[[comp_name]]$median
    ...
    if (length(district_stats) > 0) {
      all_district_ecdf_data <- rbindlist(lapply(district_stats, function(x) {
        x$group_ecdf_data_mean
      }))
      all_district_ecdf_data_mean <- all_district_ecdf_data
      all_district_ecdf_data_median <- rbindlist(lapply(
        district_stats,
        function(x) x$group_ecdf_data_median
      ))
    } else {
  ...
  cat(
    "Computing individual-level rank agreement (Spearman rho by condition)...\n"
  )
  ...
  if (has_school) {
    ...
    if (nrow(school_rank_rho) > 0) {
      ...
      for (cn in unique(school_rank_rho$comparison)) {
        ...
        if (nrow(sub) > 0) {
          cat(sprintf(
            "    %s: median school-rank rho=%.4f, min=%.4f (%d conditions)\n",
            cn,
            median(sub$rho),
            min(sub$rho),
            nrow(sub)
  ...
  if (has_district) {
    ...
    district_rank_rho <- compute_group_rank_rho(
      district_group_cache,
      30,
      "district"
    )
    if (nrow(district_rank_rho) > 0) {
      ...
      for (cn in unique(district_rank_rho$comparison)) {
        ...
        if (nrow(sub) > 0) {
          cat(sprintf(
            "    %s: median district-rank rho=%.4f, min=%.4f (%d conditions)\n",
            cn,
            median(sub$rho),
            min(sub$rho),
            nrow(sub)
  ...
  for (var in bucket_vars) {
    if (var %in% names(sgpc_with_buckets)) {
      ...
      for (K in individual_bucket_sizes) {
        ...
        bucket_by <- if (has_dataset_id) {
          c("dataset_id", "condition_id")
        } else {
          "condition_id"
        }
        sgpc_with_buckets[,
        ...
        if (K == 10L) {
          ...
          if (n_na > 0) {
            ...
            cat(sprintf(
              "  NOTE: %s classification failed for %d obs (%.1f%%) across %d conditions\n",
              var,
              n_na,
              100 * n_na / nrow(sgpc_with_buckets),
              length(failed_conditions)
            ))
            cat(sprintf(
              "        (Bimodal distribution: values are 1 or 99 only in: %s)\n",
  ...
  for (comp_name in names(comparison_pairs)) {
    ...
    for (K in individual_bucket_sizes) {
      ...
      if (
        bucket1 %in%
          names(sgpc_with_buckets) &&
          bucket2 %in% names(sgpc_with_buckets)
      ) {
        # Overall
        stability_overall <- sgpc_with_buckets[, {
          d1 <- as.integer(get(bucket1))
          d2 <- as.integer(get(bucket2))
          ...
          list(
            ...
            kappa_w_quad = compute_weighted_kappa(
              d1,
              d2,
              K = K,
              weights = "quadratic"
        ...
        stability_by_year <- sgpc_with_buckets[,
          {
            ...
            list(
              ...
              off_by_2plus = mean(bucket_diff >= 2, na.rm = TRUE),
              n = sum(!is.na(bucket_diff)),
              kappa_w = compute_weighted_kappa(
                d1,
                d2,
                K = K,
                weights = "linear"
        ...
        stability_by_year[, `:=`(
          comparison = comp_name,
          stratum = paste0("Year_", year_span),
          n_buckets = K
        )]
        ...
        stability_by_content[, `:=`(
          comparison = comp_name,
          stratum = content_area,
          n_buckets = K
        )]
        ...
        if (K == 10L) {
          cat(sprintf(
            "  %s: %.1f%% exact, %.1f%% ±1, %.1f%% ≥2 deciles different | \u03ba_w=%.3f\n",
            comp_name,
  ...
  individual_bucket_stability <- rbindlist(
    individual_bucket_stability_list,
    fill = TRUE
  )

  # Backward-compatible decile-only table for existing consumers
  decile_misclass <- individual_bucket_stability[
    n_buckets == 10,
    .(
      comparison,
      stratum,
  ...
  compute_bucket_stability <- function(group_cache, min_n, level_label, K) {
    ...
    for (comp_name in names(group_cache)) {
      ...
      grp_bucket_by <- if ("dataset_id" %in% names(group_agg)) {
        c("dataset_id", "condition_id")
      } else {
        "condition_id"
      }
        ...
        ga_copy[,
          bucket1 := {
            ...
            if (length(brks) < 2) {
              ...
              as.integer(cut(get(v1_col), breaks = brks, include.lowest = TRUE))
            }
          },
          by = grp_bucket_by
        ]
        ...
        ga_copy[,
          bucket2 := {
            ...
            if (length(brks) < 2) {
              ...
              as.integer(cut(get(v2_col), breaks = brks, include.lowest = TRUE))
            }
          },
          by = grp_bucket_by
        ]
      ...
      res_median <- run_bucket_stability(
        group_agg,
        "median1",
        "median2",
        "median"
      ...
      comp_results <- rbindlist(list(res_mean, res_median)[
        !sapply(list(res_mean, res_median), is.null)
      ])
  ...
  if (has_school) {
    ...
    for (K in bucket_sizes) {
      ...
      if (nrow(res) > 0) {
        ...
        if (nrow(ovr) > 0) {
          cat(sprintf(
            "    K=%d: median exact match = %.1f%% across %d comparisons\n",
            K,
            median(ovr$exact_match) * 100,
            nrow(ovr)
          ))
        }
      }
  ...
  if (!has_school && !has_district) {
    cat(
      "  WARNING: No SCHOOL_NUMBER or DISTRICT_NUMBER. Skipping bucket stability.\n"
    )
  }
  ...
  cat(
    "Computing group-level transition matrices (K=3,5,7,10; school>=250, district>=500)...\n"
  )
  ...
  compute_group_transitions <- function(group_cache, min_n, level_label, K) {
    ...
    for (comp_name in names(group_cache)) {
      ...
      run_transition <- function(ga, v1_col, v2_col, method_label) {
        ...
        if (nrow(ga_copy) == 0) {
          return(list(trans = NULL, metrics = NULL))
        }
        ...
        trans <- ga_copy[,
          .(n = .N),
          by = .(empirical_bucket, comparison_bucket)
        ]
        grid <- CJ(empirical_bucket = 1:K, comparison_bucket = 1:K)
        trans <- merge(
          grid,
          trans,
          by = c("empirical_bucket", "comparison_bucket"),
          all.x = TRUE
        )
        trans[is.na(n), n := 0L]
        ...
        metrics <- data.table(
          ...
          spearman_rho = tryCatch(
            cor(
              as.double(ga_copy[[v1_col]]),
              as.double(ga_copy[[v2_col]]),
              method = "spearman",
              use = "complete.obs"
            ),
            error = function(e) NA_real_
          ...
          p_comp_top_given_emp_top = ifelse(
            den_top > 0,
            tp_top / den_top,
            NA_real_
          ),
          p_emp_top_given_comp_top = ifelse(
            den_cmp_top > 0,
            tp_top / den_cmp_top,
            NA_real_
          ),
          p_comp_bottom_given_emp_bottom = ifelse(
            den_bot > 0,
            tp_bot / den_bot,
            NA_real_
          ),
          p_emp_bottom_given_comp_bottom = ifelse(
            den_cmp_bot > 0,
            tp_bot / den_cmp_bot,
            NA_real_
          )
      ...
      if (!is.null(res_mean$trans)) {
        trans_results[[paste0(comp_name, "_mean")]] <- res_mean$trans
      }
      if (!is.null(res_median$trans)) {
        trans_results[[paste0(comp_name, "_median")]] <- res_median$trans
      }
      if (!is.null(res_mean$metrics)) {
        metric_results[[paste0(comp_name, "_mean")]] <- res_mean$metrics
      }
      if (!is.null(res_median$metrics)) {
        metric_results[[paste0(comp_name, "_median")]] <- res_median$metrics
      }
    }

    list(
      transitions = if (length(trans_results) > 0) {
        rbindlist(trans_results, fill = TRUE)
      } else {
        data.table()
      },
      metrics = if (length(metric_results) > 0) {
        rbindlist(metric_results, fill = TRUE)
      } else {
        data.table()
      }
    )
  ...
  if (has_school) {
    ...
    for (K in transition_bucket_sizes) {
      ...
      if (nrow(res$transitions) > 0) {
        group_transition_matrices_list[[paste0(
          "school_K",
          K
        )]] <- res$transitions
      }
      ...
      ovr <- res$metrics[
        comparison == "Empirical – Canonical" & agg_method == "mean"
      ]
      if (nrow(ovr) > 0) {
        cat(sprintf(
          "    K=%d: canonical exact match = %.1f%% (n_groups=%s)\n",
          K,
          ovr$exact_match[1] * 100,
          format(ovr$n_groups[1], big.mark = ",")
        ))
      }
    }
  }
  ...
  if (!has_school && !has_district) {
    cat(
      "  WARNING: No SCHOOL_NUMBER or DISTRICT_NUMBER. Skipping transition matrices.\n"
    )
  }
  ...
  if (
    ...
    cat(
      "  WARNING: SCALE_SCORE_PRIOR not available. Skipping quartile analysis.\n\n"
    )
  } else {
    ...
    for (comp_name in names(comparison_pairs)) {
      ...
      q_stats <- sgpc_data[
        !is.na(prior_quartile) & !is.na(get(var1)) & !is.na(get(var2)),
        {
  ...
  for (comp_name in names(comparison_pairs)) {
    ...
    cat(sprintf(
      ...
      comp_name,
      paste(
        sprintf("%s:%.1f", ds_stats$dataset_id, ds_stats$mad),
        collapse = " | "
      )
    ))
  }
  ...
  if (nrow(condition_level_stats) > 0) {
    ...
    n_unique_conditions <- if ("dataset_id" %in% names(condition_level_stats)) {
      uniqueN(condition_level_stats[, paste(
        dataset_id,
        condition_id,
        sep = "__"
      )])
    ...
    cat(sprintf(
      "  Conditions: %d unique, N range: %s - %s\n",
      n_unique_conditions,
    ...
    for (i in seq_len(nrow(summ))) {
      cat(sprintf(
        "  %s: median condition MAD=%.2f (median n=%s)\n",
        summ$comparison[i],
        summ$median_mad[i],
  ...
  result <- list(
    ...
    # Group-size stratification (sqrt(n) averaging effect)
    group_size_analysis = if (length(group_size_analysis) > 0) {
      group_size_analysis
    } else {
      NULL
    },
    ...
    classification_issues = if (length(classification_issues) > 0) {
...
compute_sampling_sensitivity <- function(
  ...
  for (nm in names(comparison_pairs)) {
    cat(sprintf(
      "  %s: %s vs %s\n",
      nm,
      comparison_pairs[[nm]][1],
      comparison_pairs[[nm]][2]
    ))
  }
  ...
  if (is.null(conditions)) {
    conditions <- eligible$condition_id[seq_len(min(
      max_conditions,
      nrow(eligible)
    ))]
  } else {
    ...
    if (length(missing) > 0) {
      cat(sprintf(
        "  Dropping conditions too small: %s\n",
  ...
  total_cells <- length(conditions) *
    length(comparison_pairs) *
    length(sample_sizes) *
    B
  cat(sprintf(
    "Total bootstrap units: %s\n\n",
  ...
  cell_summary <- replicate_results[
    !is.na(mad),
    {
      list(
        n_replicates = .N,
  ...
  cell_details <- merge(
    cell_means,
    valid_reps[, .N, by = .(comparison, sample_size)],
  ...
  per_ss_decomposition <- valid_reps[,
    {
      ...
      list(
        ...
        total_var = var(mad),
        comparison_share = comparison_var /
          (comparison_var + within_var + 1e-12),
  ...
  for (i in seq_len(nrow(per_ss_decomposition))) {
    ...
    cat(sprintf(
      "  %6s  %10.3f  %10.3f  %10.3f  %7.1f%%  %7.1f%%\n",
      format(r$sample_size, big.mark = ","),
      r$comparison_var,
      r$sampling_var,
      r$total_var,
      r$comparison_share * 100,
      r$sampling_share * 100
    ))
  }
