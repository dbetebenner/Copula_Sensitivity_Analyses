...
if (!file.exists(comparison_file)) {
  stop(
    "STEP_1 comparison file not found: ",
    comparison_file,
...
cat(sprintf(
  "Loaded STEP_1 comparison data: %s rows, %d conditions\n",
...
cat(sprintf(
  "Best-fit families identified for %d conditions\n",
  nrow(best_fits)
))
...
cat(sprintf(
  "  Strata where t-copula is majority (>=50%%): %d\n",
  sum(stratum_family_dist$t_majority)
))
cat(sprintf(
  "  Strata where t-copula is NOT majority: %d\n",
  sum(!stratum_family_dist$t_majority)
))
...
for (ds_id in dataset_ids) {
  ...
  if (file.exists(rds_file)) {
    ...
    cat(sprintf(
      "  Loaded %s: %s observations, %d conditions\n",
      ds_id,
...
if (length(all_variants) == 0) {
  ...
  cat(
    "CANONICAL VALIDATION: PHASE 1 COMPLETE (Phase 2 pending STEP_2 results)\n"
  )
  cat("====================================================================\n")
  ...
  stop(
    "STEP_2 results not yet available. Re-run after sgpc_compute_all_variants.R completes."
  )
}
...
cat(sprintf(
  "  STEP_1 best-family lookup: %d unique grade_pair x content_area patterns\n",
...
cat(sprintf(
  "Family info merged: %d/%d observations matched (%.1f%%)\n",
  n_matched,
  nrow(variants_dt),
  100 * n_matched / nrow(variants_dt)
))

if (n_matched == 0) {
  cat("\nWARNING: No condition_id matches between STEP_1 and STEP_2.\n")
  cat(
    "STEP_1 descriptive IDs: ",
    paste(head(condition_best_family$descriptive_id, 3), collapse = ", "),
    "\n"
  )
  cat(
    "STEP_2 condition IDs:   ",
...
cat(sprintf(
  "Valid observations (with empirical SGPc): %s\n",
...
stratum_validation <- condition_summary[,
  .(
    ...
    mad_canonical_t_best = round(
      mean(mad_canonical[best_family_aic == "t"], na.rm = TRUE),
      2
    ),
    mad_canonical_frank_best = round(
      mean(mad_canonical[best_family_aic == "frank"], na.rm = TRUE),
      2
    ),

    # Frank accuracy for Frank-best conditions (does Frank actually beat canonical there?)
    mad_frank_frank_best = round(
      mean(mad_frank[best_family_aic == "frank"], na.rm = TRUE),
      2
    ),
...
stratum_validation[,
  canonical_gap := round(mad_canonical_mean - mad_best_mean, 2)
]

# Does Frank beat canonical for Frank-best conditions?
stratum_validation[,
  frank_beats_canonical_for_frank_conds := mad_frank_frank_best <
    mad_canonical_frank_best
]

# Report
cat("\nPer-stratum validation results:\n\n")
cat(sprintf(
  "  %-25s  n_cond  MAD_canon  MAD_best  MAD_frank  gap   canon_frank_best  frank_frank_best  frank_wins?\n",
  "stratum"
))
for (i in seq_len(nrow(stratum_validation))) {
  row <- stratum_validation[i]
  cat(sprintf(
    "  %-25s  %3d     %5.2f      %5.2f     %5.2f    %5.2f     %5.2f             %5.2f             %s\n",
    row$stratum_id,
    row$n_conditions,
    row$mad_canonical_mean,
    row$mad_best_mean,
    row$mad_frank_mean,
    row$canonical_gap,
    row$mad_canonical_frank_best,
    row$mad_frank_frank_best,
    ifelse(
      is.na(row$frank_beats_canonical_for_frank_conds),
      "N/A",
...
canonical_csv_file <- file.path(
  STEP1_RESULTS_DIR,
  "canonical_copula_parameters.csv"
)
...
global_mad_canonical <- round(
  mean(condition_summary$mad_canonical, na.rm = TRUE),
  2
)
...
global_cor_canonical <- round(
  mean(condition_summary$cor_canonical, na.rm = TRUE),
  4
)
...
if (nrow(frank_best_conds) > 0) {
  mad_canonical_on_frank <- round(
    mean(frank_best_conds$mad_canonical, na.rm = TRUE),
    2
  )
  mad_frank_on_frank <- round(mean(frank_best_conds$mad_frank, na.rm = TRUE), 2)
  cor_canonical_on_frank <- round(
    mean(frank_best_conds$cor_canonical, na.rm = TRUE),
    4
  )
} else {
...
frank_improvement <- if (
  !is.na(mad_canonical_on_frank) && !is.na(mad_frank_on_frank)
) {
...
if (!is.na(frank_improvement)) {
  cat(sprintf(
    "  Frank improvement on Frank-best conditions: %.2f SGPc points\n",
    frank_improvement
  ))
}
...
report_lines <- c(
  ...
  "",
  paste0(
    "The canonical copula (t-copula with stratum-specific median parameters from STEP_1) ",
    "was validated against empirical SGPc values across all STEP_2 conditions."
  ),
  "",
  sprintf(
    "- **Overall MAD (canonical vs empirical):** %.2f SGPc percentile points",
    global_mad_canonical
  ),
  sprintf(
    "- **Overall MAD (best-fit vs empirical):** %.2f SGPc percentile points",
    global_mad_best
  ),
  sprintf(
    "- **Canonical gap (excess MAD over best-fit):** %.2f points",
    global_gap
  ),
  sprintf("- **Canonical/Best-fit MAD ratio:** %.3f", canonical_ratio),
  sprintf(
    "- **Mean correlation (canonical vs empirical):** %.4f",
    global_cor_canonical
  ),
  "",
  sprintf(
...
if (!is.na(mad_canonical_on_frank)) {
  report_lines <- c(
    report_lines,
    sprintf(
      "| Metric | t-best conditions (n=%d) | Frank-best conditions (n=%d) |",
      nrow(t_best_conds),
      nrow(frank_best_conds)
    ),
    "|--------|-------------------------|------------------------------|",
    sprintf(
      "| MAD (canonical vs emp) | %.2f | %.2f |",
      mad_canonical_on_t,
      mad_canonical_on_frank
    ),
    sprintf(
      "| MAD (Frank vs emp) | %.2f | %.2f |",
      round(mean(t_best_conds$mad_frank, na.rm = TRUE), 2),
      mad_frank_on_frank
    ),
  ...
  if (frank_improvement > 0.5) {
    report_lines <- c(
      report_lines,
      sprintf(
        "**Note:** Frank copula outperforms canonical by %.2f MAD points on Frank-best conditions.",
        frank_improvement
      ),
    ...
    report_lines <- c(
      report_lines,
      sprintf(
        "**Note:** Frank improvement (%.2f points) is modest. The t-canonical is robust even for Frank-best conditions.",
        frank_improvement
      ),
      ""
    )
...
if (is_validated) {
  report_lines <- c(
    ...
    "",
    sprintf(
      "1. The canonical-to-empirical MAD (%.2f) is close to the best-fit-to-empirical MAD (%.2f), ratio = %.3f",
      global_mad_canonical,
      global_mad_best,
      canonical_ratio
    ),
    sprintf(
      "2. The canonical-empirical correlation (%.4f) indicates near-perfect rank agreement",
      global_cor_canonical
    ),
    sprintf(
      "3. The canonical gap (%.2f SGPc points) represents minimal practical impact on student percentile classifications",
      global_gap
    ),
    ""
  )
  ...
  if (!is.na(frank_improvement) && frank_improvement > 0) {
    report_lines <- c(
      ...
      "",
      sprintf(
        "- The canonical MAD increases modestly (%.2f vs %.2f for t-best conditions)",
        mad_canonical_on_frank,
        mad_canonical_on_t
      ),
      sprintf(
        "- Frank copula would improve MAD by %.2f points for these conditions",
        frank_improvement
      ),
