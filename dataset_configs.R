...
DATASETS <- list(
  ...
  dataset_3 = list(
    ...
    scaling_by_year = data.frame(
      ...
      scaling_type = c(
        "vertical",
        "vertical", # 2013, 2014 (before transition)
        "non_vertical",
        "non_vertical",
        "non_vertical"
      ), # 2015-2017 (after transition)
      stringsAsFactors = FALSE
    ),
...
get_scaling_type <- function(dataset_config, year) {
  ...
  if (nrow(match_row) == 0) {
    warning(
      "Year ",
      year_num,
      " not found in scaling_by_year for dataset ",
      dataset_config$id,
      ". Returning NA."
    )
...
crosses_transition <- function(dataset_config, year_prior, year_current) {
  ...
  return(
    year_prior_num < transition_year && year_current_num >= transition_year
  )
}
...
get_scaling_transition_type <- function(
  dataset_config,
  year_prior,
  year_current
) {
